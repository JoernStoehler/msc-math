...
\begin{remark}[Degeneracy and orbit switching]
\label{rem:orbit-switching}
The EHZ capacity is $c_{\mathrm{EHZ}}(h) = \min_{(S,\sigma)} A(S,\sigma;h)$.
Each orbit's action $A(S,\sigma;\cdot)$ is a smooth function of $h$
(by the envelope theorem, as long as the KKT problem for that orbit is
non-degenerate). The capacity, being a minimum of smooth functions, is
continuous but generally only piecewise smooth: it is smooth on regions
where the minimizing orbit is unique, and non-smooth at boundaries where
two orbits achieve the same minimum action.

At such boundaries the capacity derivative has a one-sided discontinuity:
the gradient computed from the current minimizer may not predict the
behavior of $c_{\mathrm{EHZ}}$ after a finite step, because a different
orbit may become the new minimizer. This \emph{orbit switching} is
structural to the piecewise-smooth optimization problem and is observed
in our dataset (some polytopes worsen despite a favorable gradient).
\end{remark}
...
\item \emph{Orbit switching limits finite steps.}
  As noted in Remark~\ref{rem:orbit-switching}, a different orbit may
  become the minimizer after a finite step, causing the actual
  $\mathrm{sys}$ change to differ from the gradient's prediction.
