...
\begin{proof}
\emph{Claim (a):} Closure follows from $z_{\alpha,b}(\beta T_z) = \alpha z(T_z) + b = \alpha z(0) + b = z_{\alpha,b}(0)$.
...
$\dot{z}$. Under the rescaling $z(t) \mapsto \alpha z(t/\beta)$, the velocity
transforms as $\dot{z}(t) \mapsto (\alpha/\beta) \dot{z}(t/\beta)$. Since $I_K$
...
\[
  I_K(z_{\alpha,b})
  = \frac{1}{4} \int_0^{\beta T_z} h_K^2(-J_0 \dot{z}_{\alpha,b}(t))\, dt
  = \frac{1}{4} \int_0^{\beta T_z} h_K^2\bigl( -J_0 (\alpha/\beta) \dot{z}(t/\beta) \bigr)\, dt.
\]
Since $h_K$ is 1-homogeneous:
\[
  = \frac{1}{4} \int_0^{\beta T_z} (\alpha/\beta)^2 h_K^2(-J_0\dot{z}(t/\beta))\, dt
  = \frac{\alpha^2}{\beta^2} \cdot \frac{\beta}{4} \int_0^{T_z} h_K^2(-J_0\dot{z}(s))\, ds
  = \frac{1}{4} \int_0^{T_z} h_K^2(-J_0\dot{z}(s))\, ds = I_K(z),
\]
using $\beta = \alpha^2$ and substitution $s = t/\beta$ with $dt = \beta\, ds$.

\emph{Claim (c):} Similarly, $A(z) = \frac{1}{2}\int_0^T \langle -J_0\dot{z}, z\rangle\, dt$
gives
\[
  A(z_{\alpha,b})
  = \frac{1}{2} \int_0^{\beta T_z} \langle -J_0(\alpha/\beta)\dot{z}(t/\beta),\, \alpha z(t/\beta) + b \rangle\, dt.
\]
Since $\int_0^{\beta T_z} \langle -J_0\dot{z}(t/\beta), b\rangle\, dt = 0$ (using $J_0$ skew-symmetry
and closure of $z$), we get
\[
  = \frac{\alpha^2}{\beta} \cdot \frac{1}{2} \int_0^{\beta T_z} \langle -J_0\dot{z}(t/\beta), z(t/\beta)\rangle\, dt
  = \beta \cdot \frac{1}{2} \int_0^{T_z} \langle -J_0\dot{z}(s), z(s)\rangle\, ds
  = \beta\, A(z).
\]

\emph{Claim (d):} If $A(z) = T_z$, then $A(z_{\alpha,b}) = \beta A(z) = \beta T_z = T_{\alpha,b}$.
\end{proof}
