...
- For subset S ⊆ {1,...,F}, |S| = m ≥ 2, cyclic permutation σ of S:
  - **Action matrix** H(σ) ∈ R^{m×m}: for i < j, H_ij = H_ji = ω₀(a_σ(i), a_σ(j)); H_ii = 0.
    ...
    - Note: ω₀ is antisymmetric, but H is symmetric because H uses the (i<j) convention: the double sum Σ_{i<j} β_i β_j ω₀(a_σ(i), a_σ(j)) = ½ β^T H β (lem:H-quadratic, proven in kkt/math.tex)
  ...
  - Origin of the constraints:
    ...
    - See lem:dual-vertex-qp in kkt/math.tex (Jörn-approved) for the complete derivation.
...
- **Adjacency pruning** (lem:numerical-transition-feasibility in kkt/math.tex):
...
[SCOPE: Include the full proof of lem:numerical-transition-feasibility here, or just state and cite? The proof is in kkt/math.tex and is ~90 lines. It's self-contained and geometrically instructive but not on the critical path of the numerical error story.]
...
### 3a: Notation and general structure (rem:qp-setup in error-bounds/math.tex)
...
### 3c: Critical-point classification (prop:critical-points in error-bounds/math.tex)
...
2. **H' singular, g ⊥ ker(H')** (i.e., g ∈ im(H')): affine subspace of critical points: α*_part + ker(H').
   - All critical points give the same Q value (lem:well-defined, proven in kkt/math.tex, Jörn-approved).
...
### 3d: Boundary vs. interior maximum (prop:boundary-vs-interior in error-bounds/math.tex)
...
- **Classification of the per-σ problem:**
  ...
  - Some γ̃_j ∈ [−ε_γ, ε_γ] → **INDETERMINATE**: proceed as if case I2 (search flat eigendirections for β > 0 via LP, rem:near-null-lp-search in kkt/math.tex)
...
- **The bound η_i decomposes into three sources** (eq:beta-error-decomposition in error-bounds/math.tex):
...
- **Computable formula** (eq:eta-computable in error-bounds/math.tex):
...
- **Empirical performance** (from error-bounds/logbook.md, 44,808 natural polytope σ-nodes):
...
### Lazy INDETERMINATE resolution (rem:trinary-beta in error-bounds/math.tex)
...
### Empirical validation summary (from error-bounds/logbook.md)
...
5. **Transition feasibility lemma** — cited in Part 2 but formal statement+proof lives in kkt/math.tex (lem:numerical-transition-feasibility); [SCOPE: reproduce vs cite?]
