...
\begin{enumerate}
\item whenever a physical facet transition \(F_i\to F_j\) with \(i\ne j\)
  exists,
  \(\omega_0(a_i,a_j)\ne0\);
...
a strict positive-time representative cannot use that triple: by
...
times and Lemma~\ref{lem:fg-local-transition-regularity-positive-sign} force
the positive transition signs of every adjacent triple.  Hence
...
Fix a facet count \(F\).  On the open dense sufficient generic locus from
...
not analyze f64 rounding.  The correctness theorem uses the local
physical-transition nonzero-\(\omega_0\) condition from
Definition~\ref{def:fg-nondegenerate-facet-presentation}; the generic
corollary still records a stronger open-dense sufficient denominator locus.
