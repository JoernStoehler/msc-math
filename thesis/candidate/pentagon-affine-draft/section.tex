...
Every equality body in this family is a positive dilation of a linear
symplectic image of the HKO body. Indeed, when $M=tR_\alpha U$, the symplectic
map $\operatorname{diag}(G^{-1},G^T)$ sends the product to
\[
 P\times M^{-T}P=P\times t^{-1}R_\alpha UP.
\]
The symplectic rescaling $(q,p)\mapsto(t^{-1/2}q,t^{1/2}p)$ then gives
$t^{-1/2}(P\times R_\alpha UP)$. Every orthogonal symmetry $U$ of the
decagon sends $P$ to $P$ or $-P$, so the remaining regular product has
relative angle $\alpha$ or $\alpha+\pi$. Modulo the pentagon period
$2\pi/5$, both $\alpha+\pi$ and the HKO angle $-\pi/2$ equal $-\alpha$;
simultaneous reflection in both planes exchanges $\alpha$ and $-\alpha$
symplectically. Conversely, dilation and
linear symplectic maps preserve the systolic ratio, so every such HKO image
in this family attains equality.
