...
The determinant-generic corollary is also not yet the final relative-genericity
statement one would want for thesis prose.  To upgrade it, one still has to
prove or cite the local openness of the valid irredundant \(H\)-presentation
space in the dual-vertex coordinates used here, then intersect that open
presentation space with the all-pair nonzero-\(\omega_0\), linear-independence,
primitive-denominator, and determinant-nonzero loci.  Without that extra
presentation-space argument, the formal text only proves genericity relative
to the ambient primitive-denominator domain used in
Fact~\ref{fact:fg-finite-orbit-regular-open-dense}.
