...
\item \emph{Symplectic signs.}
  ...
  numerical error.
  Here $\omega_0(u, v) = u_1 v_3 - u_3 v_1
  + u_2 v_4 - u_4 v_2$ is the standard symplectic
  form in coordinates $(q_1, q_2, p_1, p_2)$.
\end{enumerate}
\end{remark}
...
\begin{proof}
We check each direction by tracking which
halfspace constraints $\langle n_k, \cdot \rangle \leq h_k$
the approach and departure can violate.
...
\leq \langle n_k, x \rangle \leq h_k$
for $t \geq 0$.
If no blocking facets exist, any $\varepsilon > 0$
suffices.
Otherwise, let $\delta$ be the minimum of
$h_k - \langle n_k, x \rangle > 0$
over all blocking~$k$, and choose
...
coordinates is \emph{numerically $\varepsilon$-robust}
if the following conditions hold,
where all quantities are computed exactly
over~$\mathbb{Q}$:
\begin{enumerate}
