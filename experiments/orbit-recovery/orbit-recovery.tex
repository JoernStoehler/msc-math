...
\end{itemize}
These metrics verify three of the four conditions in
Definition~\ref{def:reeb-orbit-polytope}: closure checks
$\gamma(0) = \gamma(T)$, on-facet error and inequality violation check
$\operatorname{Im}(\gamma) \subset \partial K$, and action error checks
$A(\gamma) = T$. The remaining condition --- that the velocity is a
convex combination of incident Reeb vectors --- holds by construction,
since the orbit is reconstructed from~$(S, \beta)$ using the facet Reeb
vectors~$R_{\sigma(k)}$.
