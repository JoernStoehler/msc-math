...
Unlike the previous two predicates, near-singular
dismissal (Algorithm~\ref{alg:near-singular-handling})
is not a three-valued predicate but a binary decision:
the algorithm either dismisses the pair (provably covered
by a smaller pair, up to a bounded error from
Lemma~\ref{lem:dismissal-error-bound}) or uses the
computed~$\beta_0$ directly.
In the latter case---whether the system is well-conditioned
or the near-null space is confined to the multiplier
block---the $\beta_0$ is well-determined and the
three-valued predicates above apply to it as usual, with
no additional uncertainty from the near-singularity
