...
% TODO: verify exact theorem number in Higham Ch. 5
\begin{fact}[SVD backward stability;
  {Higham~(2002), Ch.~5}]\label{fact:prefilter-svd}
...
% TODO: verify exact section numbers in Higham
\begin{fact}[SVD-based solve backward stability;
  {Higham~(2002), Ch.~3, 5}]\label{fact:prefilter-solve}
...
% TODO: verify exact corollary number in GVL Ch. 8
\begin{fact}[Weyl's singular value perturbation theorem;
  {Golub \& Van~Loan~(2013), Ch.~8}]\label{fact:prefilter-weyl}
For any $B, B' \in \mathbb{R}^{n \times n}$:
$|\sigma_i(B) - \sigma_i(B')| \le \|B - B'\|$.
\end{fact}

% TODO: verify exact lemma number in Higham Ch. 2
\begin{fact}[Banach perturbation lemma;
  {Higham~(2002), Ch.~2}]\label{fact:prefilter-banach}
...
% TODO: verify exact theorem number in Higham Ch. 3
\begin{fact}[Dot product rounding;
  {Higham~(2002), Ch.~3}]\label{fact:prefilter-dot}
...
% [TODO: JÖRN - The proposition requires $\emach\kappa \le 1/4$ on the
%   TRUE condition number $\kappa = \kappa_2(\hat{A})$, but the algorithm
%   (step 3) can only check $\emach\hat\kappa \le 1/4$ on the COMPUTED
%   $\hat\kappa$. Since $\hat\kappa$ can underestimate $\kappa$ (Weyl
%   allows $\hat\sigma_{\min} > \sigma_{\min}(\hat{A})$), the check does
%   not imply the hypothesis. Closing this gap requires a concrete value
%   of $c_{\mathrm{svd}}$ for $n = 4$: the algorithm's threshold should
%   be $\emach\hat\kappa \le 1/(4 + c_{\mathrm{svd}})$.]
