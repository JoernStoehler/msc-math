...
fn cyclic_invariance_check() {
    ...
    for rot in 0..m {
        ...
        match solve_kkt(normals, heights, &rotated) {
            Some((beta, q)) => {
                let action = 0.5 / q;
                eprintln!("  rot={}: {:?} → action={:.10} Q={:.10} β={:.6?}", rot, rotated, action, q, beta);
            }
            None => {
...
fn svd_null_space_debug() {
    ...
    for i in 0..m {
        for j in (i + 1)..m {
            let val = geom::symplectic::omega0(&normals[perm[i]], &normals[perm[j]]);
            kkt[(i, j)] = val;
            kkt[(j, i)] = val;
        }
    }
    ...
    let q0 = crate::q_from_beta(normals, perm, &beta0);
    eprintln!("Q(β₀) = {:.10}", q0);
    ...
    for &alpha in &[-2.0, -1.0, -0.5, 0.0, 0.5, 1.0, 2.0] {
        ...
        let q = crate::q_from_beta(normals, perm, &beta);
        let all_pos = beta.iter().all(|&b| b > 1e-12);
    ...
    for i in 0..m {
        for j in (i + 1)..m {
            let val = geom::symplectic::omega0(&normals[perm_rot[i]], &normals[perm_rot[j]]);
            kkt2[(i, j)] = val;
            kkt2[(j, i)] = val;
        }
    }
    ...
    let q0_rot = crate::q_from_beta(normals, perm_rot, &beta0_rot);
    ...
    match solve_kkt(normals, heights, perm) {
        Some((beta, q)) => {
            let action = 0.5 / q;
            eprintln!("  action={:.10} Q={:.10} β={:.8?}", action, q, beta);
            // Check: does this β satisfy the constraints?
            let n_constraint: Vec<f64> = (0..4).map(|d| {
                (0..m).map(|i| beta[i] * normals[perm[i]][d]).sum::<f64>()
            }).collect();
            let eta_constraint: f64 = (0..m).map(|i| beta[i] * heights[perm[i]]).sum();
    ...
    let q_r9 = crate::q_from_beta(normals, perm, &beta0_r9);
    ...
    for &angle in &[40.0_f64, 42.0, 43.0, 44.5] {
        ...
        if let Ok(Some(ref r)) = bil {
            ...
            match solve_kkt(normals, heights, &r.best_permutation) {
                Some((beta, q)) => {
                    let action = 0.5 / q;
                    eprintln!("    action={:.10} beta={:.6?}", action, beta);
                }
        ...
        if let Some(ref r) = hk {
            ...
            for rot in 0..m {
                ...
                for i in 0..m {
                    for j in (i + 1)..m {
                        let val = geom::symplectic::omega0(&normals[rotated[i]], &normals[rotated[j]]);
                        kkt_mat[(i, j)] = val;
                        kkt_mat[(j, i)] = val;
                    }
                }
                ...
                match solve_kkt(normals, heights, &rotated) {
                    Some((beta, q)) => {
                        ...
                        eprintln!("    rot={}: {:?} → action={:.10} Q={:.10} rank={}/{} min_sv={:.2e} β={:.6?}",
                            rot, rotated, action, q, rank, size, min_sv, beta);
                    }
                    None => {
            ...
            if let Ok(Some(ref r)) = bil {
                ...
                for rot in 0..m {
                    ...
                    match solve_kkt(normals, heights, &rotated) {
                        Some((beta, q)) => {
                            let action = 0.5 / q;
                            let min_beta = beta.iter().cloned().fold(f64::INFINITY, f64::min);
...
fn solve_kkt_trace() {
    ...
    match solve_kkt(normals, heights, perm) {
        Some((beta, q)) => {
            let action = 0.5 / q;
            eprintln!("  RETURNED Some: action={:.10}, Q={:.10}", action, q);
            eprintln!("  beta={:.8?}", beta);
            // Verify: does this beta come from the SVD path?
            let q_check = crate::q_from_beta(normals, perm, &beta);
            eprintln!("  Q(beta) = {:.10} (matches? {})", q_check, (q - q_check).abs() < 1e-12);
            // Is this beta equal to beta0?
            let diff: f64 = beta.iter().zip(beta0.iter()).map(|(a, b)| (a - b).abs()).sum();
