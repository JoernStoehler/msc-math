...
the current active-branch diagnostic has 150 active rows, split into
44 nonsingular and 106 singular rows; and the current theorem route uses
explicit feasible beta sections for the singular rows instead of treating them
as nearby smooth optimizing branches.
...
The smooth-only rank-defect summary
`experiments/hko-local-maximum/smooth-only-rank-defect/summary.json`
records the current route-choice bookkeeping: the 44 nonsingular rows have
projected rank 23 in the 25-dimensional quotient, while the feasible-section
route reaches rank 25 in the same f64 diagnostic.
...
The older floating-point first-order run grouped 44 near-optimal
orbits into 10 visited-set groups.  This grouping is historical context only;
the current theorem route does not use a visited-set classification as proof
input.
...
that classification.  No current theorem-facing symmetry claim uses a
replacement visited-subset classification.
...
The current diagnostic records per-row height-gradient vectors
(Section~\ref{sec:sys-optimization}).  These vectors are numerical
route-choice evidence, not the theorem certificate.

Among the per-orbit height-gradient vectors:
\begin{itemize}
...
\item \textbf{Mixed sensitivity in height space.}
  ...
  The older 10-gradient symmetry-average argument is historical context only;
  the current theorem-facing route does not use it.
