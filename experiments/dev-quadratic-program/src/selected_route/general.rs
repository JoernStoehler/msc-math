//! Instrumentable experiment copy of the selected general QP route.
//!
//! Keep the mathematical semantics in sync with
//! `crates/symplectic/src/algorithms/capacity_4d/general.rs`. This copy retains
//! route variants and counters used by the ablation producer; production fixes
//! the chosen LBLT/hybrid route and omits those experiment controls. The
//! correspondence suite must compare capacity bounds after a semantic change
//! to either file.
...
// This branch exists only in the copied experiment enum and is never selected
// by production. Keeping the signature makes the faithful source comparison
// local while avoiding import of the experiment's unsafe empirical heuristic.
...
pub fn solve_selected_general(
    duals: &[Vector4<f64>],
    words: Vec<Vec<usize>>,
) -> Option<(f64, f64)> {
    let cases = vec![(String::new(), duals.to_vec(), words)];
    let result = run_route_with_guard(
        &cases,
        Some(usize::MAX),
        FactorKind::Lblt,
        GuardKind::HybridAnalyticEnvelope,
    ...
    result
        .stats
        .best_action_lower
        .zip(result.stats.best_action_upper)
}
