...
\begin{remark}[The standard solve-step bound does not
  apply here]%
\label{rem:v2-solve-step-warning}
% QC: This remark is our analysis, not from the literature.
The bound in Remark~\ref{rem:v2-eigen-accuracy}(3)
...
\begin{equation}\label{eq:v2-q-expansion}
...
\begin{equation}\label{eq:v2-substitution-remainder}
  |r_2^\top\delta\mu + r_3\,\delta\xi|
...
gives
\begin{equation}\label{eq:v2-block-identity}
...
\]
Combining with~\eqref{eq:v2-delta-bound}
...
%   This uses the structural identity (eq:v2-block-identity):
...
\emph{Conclusion.}
From~\eqref{eq:v2-q-expansion},
\eqref{eq:v2-substitution-remainder},
and Step~4:
\[
  |Q(\beta_0) - \tilde Q|
  \leq 2\,\frac{\|r\|^2}{|\lambda_{\min}|}
...
(Remark~\ref{rem:v2-eigen-accuracy}),
...
(identity~\eqref{eq:v2-block-identity})
...
Lemma~\ref{lem:v2-q-interval} satisfies
