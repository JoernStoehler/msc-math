...
bounded if and only if, for every triple~$(i,j,k)$ whose normals have a
1-dimensional kernel direction~$d$, some normal has positive inner product
with~$d$ and some normal has negative inner product with~$d$.
\end{proposition}
...
\emph{Case~1: $\operatorname{rec}(K)$ is pointed} (contains no line). Then it
has an extreme ray~$r$. An extreme ray is a 1-dimensional face, and a
$k$-dimensional face of a polyhedral cone in~$\mathbb{R}^4$ lies on at least
$4 - k$ linearly independent tight constraints. For $k = 1$, this gives three
linearly independent normals~$n_i, n_j, n_k$ with $n_i \cdot r = n_j \cdot r
= n_k \cdot r = 0$, so $r \in \ker(n_i, n_j, n_k)$. Since $r \in
\operatorname{rec}(K)$, we have $n_\ell \cdot r \le 0$ for all~$\ell$, and no
normal has positive inner product with~$r$. Triple~$(i,j,k)$ fails.

\emph{Case~2: $\operatorname{rec}(K)$ contains a line.} There exists a nonzero
$v$ with $n_\ell \cdot v = 0$ for all~$\ell$ (both $v$ and $-v$ lie in
$\operatorname{rec}(K)$, forcing all inner products to zero). All normals lie
in~$v^\perp$, a 3-dimensional subspace. Take three linearly independent
normals~$n_i, n_j, n_k$; they span~$v^\perp$, so $\ker(n_i, n_j, n_k) =
(v^\perp)^\perp = \operatorname{span}(v)$. The kernel direction $w = v$
satisfies $n_\ell \cdot w = 0$ for all~$\ell$, so no normal has positive inner
product with~$w$. Triple~$(i,j,k)$ fails.
\end{proof}
