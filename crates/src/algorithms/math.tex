...
% algorithm environment (unnumbered, matching thesis style)
\newtheorem*{algorithm*}{Algorithm}
\newenvironment{algorithm}[1][]{\begin{algorithm*}[#1]}{\end{algorithm*}}
...
both facets, meaning $\langle q, \hat{n}_1^q \rangle = h_1^q$
and $\langle q, \hat{n}_2^q \rangle = h_2^q$: the point $q$
...
\begin{proof}
By CH2021, the minimum-action combinatorial Reeb orbit
...
\item For each pair $(i,j)$: check if $F_i \cap F_j$ is a $2$-face
  and compute $\omega_0(n_i, n_j)$. Record directed edges $F_i \to F_j$
  where $\omega_0(n_i, n_j) > 0$.
...
\begin{enumerate}
\item \textbf{Prune.} Discard if:
  $\operatorname{Start} = \emptyset$ or $\operatorname{End} = \emptyset$
  (Lemma~\ref{lem:prune-empty}), or
  $\min_{y \in \operatorname{End}} a(y) > c^*$
