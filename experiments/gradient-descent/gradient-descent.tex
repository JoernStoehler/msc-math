...
\subsubsection*{Distribution of sys}

The empirical distributions of $\mathrm{sys}$ are well described
by Beta distributions (Figure~\ref{fig:gd-density}).
Recall that $\mathrm{Beta}(a,b)$ has density
$f(x) \propto x^{a-1}(1-x)^{b-1}$ on~$[0,1]$;
for $a, b > 1$ the density is unimodal and vanishes at both
endpoints, with mean $a/(a+b)$ and the mode
shifted right as $a/b$ increases.
For general polytopes, the fit is excellent:
starting values follow $\mathrm{Beta}(1.7,\, 2.2)$
(KS test $p = 0.40$),
and after optimization $\mathrm{Beta}(3.0,\, 2.7)$
($p = 0.99$).
Gradient ascent shifts the first parameter from
$a \approx 1.7$ to $a \approx 3.0$
while $b$ stays near~$2.5$:
the left tail becomes lighter (fewer low-$\mathrm{sys}$ polytopes
remain), but the right cutoff below~$1$ persists.

For Lagrangian products, the aggregate distribution is a mixture
of three splits with different shapes, so a single Beta
fits poorly ($p = 0.01$).
Per-split fits are better
(e.g.\ the $4 \times 6$ split fits $\mathrm{Beta}(2.2,\, 1.3)$
with $p = 0.55$),
but the $3 \times 7$ and $5 \times 5$ splits show
deviations from Beta.

\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{../experiments/gradient-descent/gradient_descent_density.png}
\caption{Beta distribution fits to $\mathrm{sys}$ before and after
  gradient ascent.
  Top: general polytopes, with excellent Beta fits
  ($p = 0.40$ starting, $p = 0.99$ final).
  Bottom: Lagrangian products (all splits aggregated),
  where the mixture structure causes poor single-Beta fits
  ($p = 0.01$).}
