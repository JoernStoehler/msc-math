% omega-obstacle.tex — \input'd from experiments/experiments.tex
%
...
%   - Plot script: experiments/omega-obstacle/omega_obstacle.py
%   - Experiment notes: experiments/omega-obstacle/README.md
%
% Structure:
%   - Hypothesis and mechanism
%   - Phase A: observational (scatter plots, orbit vs non-orbit ω)
%   - Phase B: gradient analysis (⟨∇sys, ∇ω⟩ dot products)
%   - Known polytopes
%   - Conclusion
...
optimal orbit traverses several ridges with $\omega_0(n_i, n_j) = 0$
(Lagrangian ridges).
This motivates the hypothesis that near-Lagrangian ridges --- facet
pairs with small $|\omega_0(n_i, n_j)|$ --- act as ``obstacles'' that
...
shared 2-faces), and $\omega_0(n_i, n_j)$ for every ridge pair.
...
smallest $|\omega_0(n_i, n_j)|$ over all ridge pairs $(i,j)$
against~$\mathrm{sys}$.
...
Neither the orbit minimum $\min_k |\omega_0(n_{\sigma(k)}, n_{\sigma(k+1)})|$
...
$\mathrm{sys}$-increasing direction in normal space tend to
\emph{decrease} $\omega_0$?

For each facet~$k$ and each ridge-neighbor~$i$, we compute:
\begin{enumerate}
\item $\nabla_{n_k}\mathrm{sys}\big|_{T_{n_k}S^3}$ ---
  the tangential gradient of $\mathrm{sys}$ with respect to~$n_k$,
  using the analytical derivatives from
  Corollary~\ref{cor:sys-derivative} and
  Lemmas~\ref{lem:vol-derivative-normal}--\ref{lem:cap-derivative-normal}.
  For orbit facets, both volume and capacity contribute; for non-orbit
  facets, only the volume term is nonzero
  (since $\partial c_{\mathrm{EHZ}}/\partial n_k = 0$ when
  $k \notin S$).

\item $\nabla_{n_k}\omega_0(n_k, n_i)\big|_{T_{n_k}S^3}$ ---
  the tangential gradient of $\omega_0(n_k, n_i)$ with respect
  to~$n_k$.
  Since $\omega_0(v, w) = (J_0\,v) \cdot w$,
  the full gradient is $J_0\,n_i$.
  Projected to $T_{n_k}S^3$:
  \[
    \nabla_{n_k}\omega_0(n_k, n_i) = J_0\,n_i - (J_0\,n_i \cdot n_k)\,n_k.
  \]

\item The dot product
  $d = \langle \nabla_{n_k}\mathrm{sys},\;
        \nabla_{n_k}\omega_0(n_k, n_i) \rangle$.
\end{enumerate}
...
median $d = -0.0006$, fraction negative $= 50.5\%$.
...
\caption{Distribution of $\langle \nabla_{n_k}\mathrm{sys},\,
  \nabla_{n_k}\omega_0(n_k, n_i) \rangle$ for orbit facets.
...
\caption{Dot product $\langle \nabla_{n_k}\mathrm{sys},\,
  \nabla_{n_k}\omega_0 \rangle$ vs.\ current $|\omega_0(n_k, n_i)|$
  for orbit facets.
