...
demonstrated it on 140~polytopes with $F \le 10$.
...
The algorithm terminates with $O(1)$ gradient norms---far from
the near-zero gradients expected at a local maximum.
...
correlated with final $\mathrm{sys}$ ($r \approx 0.80$;
Figure~\ref{fig:gd-convergence}, left):
polytopes closest to the Viterbo threshold have the
\emph{strongest} residual gradients.
...
\caption{Convergence diagnostics.
  Left: residual height gradient $\|\nabla_h\,\mathrm{sys}\|$ vs.\
  final $\mathrm{sys}$; the positive correlation ($r \approx 0.80$)
  shows that polytopes near the threshold are gradient-constrained,
  not gradient-satisfied.
  Center: median step size drops exponentially as the polytope
  approaches its combinatorial type boundary.
  Right: survival curve (number of polytopes still improving
  at each iteration).}
\label{fig:gd-convergence}
\end{figure}

The cause is visible in the step size dynamics
(Figure~\ref{fig:gd-convergence}, center):
...
The algorithm always wants to step further, but the combinatorial
type boundary closes in, leaving less and less room for improvement.

\subsubsection*{Interpretation}

The $\mathrm{sys} < 1$ barrier in this experiment is a
\emph{step-bound barrier}, not a gradient obstruction.
The gap between $\mathrm{sys} \approx 0.905$ and the threshold~$1$
reflects a limitation of the optimization method:
\begin{itemize}
