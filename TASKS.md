...
- Current exact Sage finding: on the present widened representative surface, the representative rows annihilate the `Sp(4)` generators but not the current translation/scaling generators; the widened representative row matrix therefore has right-kernel dimension `29`, not the target `15`. Before treating that surface as provisional theorem input, resolve whether this comes from the affine/scaling tangent encoding, the exact row formula, or a convention mismatch between those two packets.
