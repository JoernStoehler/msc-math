...
For fixed $T > 0$, the set of closed, centered, simple
piecewise-Reeb curves with period~$T$ is naturally parameterized
by $S_F \times \Delta^{F-1}$
(permutation and normalized durations).
Since $S_F \times \Delta^{F-1}$ is compact and
$A$, $I_K$ are continuous functions of the parameters,
every sequence has a convergent subsequence preserving
$A$ and~$I_K$.
\end{lemma}
...
\emph{Compactness.}
Rescale each $z_4$ to period exactly~$T$
(Lemma~\ref{lem:rescaling}; the rescaling factor
$T / T^{(4)} \to 1$, so $A$ and $I_K$ change by a
vanishing amount).
By Lemma~\ref{lem:compactness} (with $S_F \times \Delta^{F-1}$
compact), a subsequence converges to a simple
piecewise-Reeb curve~$z_*$ with
$A(z_*) = T$ and $I_K(z_*) = T$.
...
Each curve is encoded by $(\sigma, s) \in S_F \times \Delta^{F-1}$,
where $\sigma$ is the ordering and $s_i = t_i / T$ are normalized
durations.
The map $(\sigma, s) \mapsto z$ is injective (up to centering),
and $A(\sigma, s) = T^2 \sum_{j<i} s_i\, s_j\,
\omega_0(w_i, w_j)$ is a polynomial in~$s$
(by Lemma~\ref{lem:shoelace}).
Since all velocities are pure Reeb vectors,
$I_K = T$ is constant on this space.
\end{proof}
