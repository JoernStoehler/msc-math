...
finished its assigned PCA method-row task. It is an example, not a template.
...
The goal is to improve the method-row orchestration flow for long-term thesis
success. The current README files may provide enough context, but this is hard
to know without asking agents after real runs.
...
- what goal or context should be changed in the prompt for the next method-row
  task to better serve long-term thesis success;
