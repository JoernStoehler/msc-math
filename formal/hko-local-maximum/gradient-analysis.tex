% formal/hko-local-maximum/gradient-analysis.tex — input by formal/main.tex
%
% Symmetry analysis of the HKO2024 counterexample and its sensitivity structure.
%
% Sources:
%   - Generated data: experiments/hko-local-maximum/gradient-analysis/hko-neighborhood-sensitivity.jsonl
%   - Plot script: experiments/hko-local-maximum/gradient-analysis/analyze.py
...
(Algorithm~\ref{alg:billiard}) originally found 364 valid orbits, of which
44 achieved $c_{\mathrm{EHZ}}(K)$ to machine precision
(relative gap $< 5 \times 10^{-14}$).  That older floating-point
bookkeeping is superseded for active-set counts by the exact-Clarke summary
`experiments/hko-local-maximum/exact-clarke/numerical-minima-summary.json`,
which records 150 exact-action orbits, 20 distinct visited-facet subsets,
and 28 distinct height gradients.
...
\item \textbf{28 are distinct}
  in the current exact-Clarke summary.
\item \textbf{All gradient norms are identical}
  in the older 44-orbit floating-point summary
  ($\approx 1.431$, spread $< 2 \times 10^{-13}$);
  re-check this statement before applying it to all 150 current rows.
...
\item \textbf{Mixed sensitivity in height space.}
  ...
  the current 28-gradient exact-Clarke bookkeeping before it is used in the
