...
\begin{proof}
Put $a_i=n_i/h_{0,i}$ and set
\[
  \beta_i=\frac{b_i h_{0,i}}{b\cdot h_0}
          =\alpha_i h_{0,i}=T_i.
\]
Then $\beta\geq0$, $\sum_i\beta_i=1$, and
\[
  \sum_i\beta_i a_i
  =\frac{1}{b\cdot h_0}\sum_i b_i n_i=0.
\]
Thus $(\sigma,\beta)$ is feasible for the active-word HK2017 capacity
problem of Definition~\ref{def:hk2017-problem}.  Moreover its objective is
\[
  Q_\sigma(\beta)
  =\frac{\mathcal Q_\sigma(b)}{(b\cdot h_0)^2}
  =\frac{1}{2A_{\sigma,b}(h_0)}.
\]
Since $(\sigma,b)$ is active, this equals the global maximum
$1/(2c_{\mathrm{EHZ}}(K(h_0)))$.  Fact~\ref{fact:hk2017} therefore gives the
claimed minimum-action simple generalized characteristic after deleting the
zero entries.  Its physical dwell time on facet $i$ is
$c_{\mathrm{EHZ}}(K(h_0))\beta_i$, so its dwell-time fraction is
$\beta_i=T_i$.  The proof of Fact~\ref{fact:hk2017} also shows that every
positive-duration $i$-labelled piece lies on $F_i$.  Finally,
$h_{0,i}>0$ makes $T_i>0$ equivalent to $\alpha_i>0$.  Reversing the word is
needed only to translate the project active traversal order to HK2017's
