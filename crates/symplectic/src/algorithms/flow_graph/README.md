...
| Singular closed-tube fixed-point cases are not silently capacity values. | source-backed by `exact_tube.rs::solve_singular_fixed_tube` and `singular_fixed_polygon_result`; recovered-proof-unreviewed for the nonpositive-action no-orbit side via `lem:fg-nonpositive-fixed-set-no-strict-orbit`; f64 near-singular maps remain a numerical policy | exact closed-word resolution | review the singular-classifier correspondence before theorem-strength thesis use; do not equate it with finite-orbit regularity |
