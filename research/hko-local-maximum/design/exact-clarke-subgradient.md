...
- For HKO2024 in dual-vertex coordinates `a_i`, the Clarke nondecreasing cone
  at the base point should equal the infinitesimal symmetry space generated by
  translation, scaling, and `Sp(4)`.
- Any later `ker(G) = Sym` summary is only a supporting linear-algebra shadow
  of that theorem after an additional argument reduces the cone certificate to
  a common-zero-set statement for the active rows.
