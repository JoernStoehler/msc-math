...
% Historical source: thesis/legacy/lagrangian-product-algorithm-proof.tex
% Current proof owner: thesis/04-haim-kislev-quadratic-program.tex,
%   Theorem thm:lagrangian-product-finite-enumeration.
% The current proof does not identify the simple minimizer supplied by HK2017
% with the short minimizer supplied by the billiard theorem. It starts from the
% short strong billiard and uses dual splitting, merging, and minimality to
% obtain a distinct-facet maximizing word without increasing the block count.
...
\[
  ...
  = \min\bigl\{ \ell_{K_p}(\eta) :
  \eta \text{ is a closed strong $(K_q,K_p)$-Minkowski billiard trajectory}
  \bigr\},
\]
where
\[
  \ell_{K_p}(\eta)
  = \sum_i \mu_{K_p^\circ}(\Delta_i)
  = \sum_i h_{K_p}(\Delta_i)
\]
and $\Delta_i=x_{i+1}-x_i$ are the edge vectors.
\end{theorem}
...
\begin{unverified}
\begin{algorithm}[Mathematical Lagrangian-product enumeration]\label{alg:billiard}
% Source: thesis/lagrangian-product-algorithm.tex
This is the exact optimization specification implied by the restricted word
theorem, not a claim that one KKT solve per word certifies the maximum.
...
\item \textbf{Build blocks.}
  ...
  The case $k=1$ is impossible by the positive closure equation: one normal,
  or a positive combination of two adjacent polygon normals, cannot vanish.
...
\item \textbf{Solve and filter.}
  For each $\sigma$, maximize $Q_\sigma$ over its compact feasible polytope.
  Equivalently, cover all active supports and compare every feasible KKT
  candidate; a positive feasible stationary point alone is not a maximum
  certificate.
\item \textbf{Return} $1/(2Q_{\max})$, where $Q_{\max}$ is the largest
  certified global value among the enumerated words.
\end{enumerate}
\end{algorithm}
\end{unverified}
