...
**Required replacement outcome:** Select material with substantive implications for AI use in mathematical research. If the sign example is mentioned, give it proportionate, very brief treatment; do not build the chapter around it or pad it with more routine examples. Jörn subsequently proposed the substantive direction below. Developing it does not require a new productivity study or experiments.
...
**Jörn's proposed substantive direction:** During the thesis, the models became much stronger: they supplied increasingly useful research ideas and automated longer tasks with fewer mistakes and derailments. His retrospective judgment is that their eventual utility lay chiefly in delegating work with a clearly defined outcome. The chapter can explain that change in research practice and what it taught him about delegation. These are author observations from the project, not results of a controlled model comparison; that does not make them unsuitable for a research-process reflection.

The practical idea to develop is that the initial context should give the agent a target it can understand, plan toward and judge progress against. “Gradeable” should convey an assessable outcome, not impose a requirement that every valuable research task have an automatic checker. Nor does it mean that Jörn must specify every intermediate action: the point of delegation is that the agent can plan the work toward the outcome. This interpretation connects useful independent research ideas and longer autonomous work with the importance of a clear target.

**Possible literature connection:** Jörn suggests relating this experience to reinforcement learning with verifiable rewards (RLVR). [DeepSeek-R1's primary research paper](https://www.nature.com/articles/s41586-025-09422-z) reports that rule-based outcome rewards elicited reasoning behaviors without human-labelled reasoning trajectories. It also describes a broader training pipeline using supervised fine-tuning and model-based preference rewards. This supports discussing how reward design shapes learned behavior; it does not establish that all agent habits chiefly arise from RLVR or that humans have no role in training. The proposed connection from that training to effective task specification in this thesis remains an interpretation, not a demonstrated causal result. The chapter should distinguish automated feedback during training from the success criterion supplied in a user's task context.
...
| AI reflection, pp. 97–99 | The replay supports a specific test-behavior observation | Jörn judges the routine example worth perhaps two lines; he proposes reflection on increasing research capability, longer reliable delegation and clearly defined outcomes, with a carefully supported RLVR connection |
...
| Selecting AI-reflection content | Develop Jörn's proposed account of increasing model capability and outcome-defined delegation; give the routine sign example at most proportionate brief treatment | That more small examples or a new productivity study are needed, or that the proposed RLVR explanation is already established |
...
He then proposed that worthwhile content: models' improving research ideas and ability to execute longer tasks with fewer mistakes and derailments during the project, and the usefulness of delegation with clearly defined outcomes. F11 records that direction and a primary-source lead for the suggested RLVR explanation, while distinguishing his retrospective observations from the proposed training-based interpretation.
