...
| `hko-local-maximum.tex` | Exact computation uses quartic field `Q(tan(pi/5))`, exact dual coordinates, symmetry tangent rank 15, and Sage verification surfaces. | HKO local result; numerics/exactness story | `research/hko-local-maximum-status.md`; `experiments/hko-local-maximum/history/exact-geometry-and-symmetry.md`; `experiments/hko-local-maximum/theorem/`; `experiments/hko-local-maximum/theorem/verification-summary.json` | exact feasible-section certificate verifies | Avoid `Q(sqrt(5))`; say Sage may use larger exact field if needed. | Sage computation subsection | Write the exact verification interface from the feasible-section certificate. | `Jörn math correctness` |
