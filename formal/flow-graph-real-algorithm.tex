...
%
% Status: complete idealized-theorem proof chain, independently agent-reviewed.
% Jörn confirmed on 2026-07-11 that both correctness and chamber genericity
% have been proved and should be retained.  Final thesis/advisor review and
% Rust correspondence are tracked separately.
% Purpose: define the idealized real-number flow-graph/tube algorithm and
% support the corresponding correctness and genericity results in the thesis.
%
...
\]
The corresponding facet-pair section is
\[
...
\[
  P_1\cap\Phi_1^{-1}(P_2),
\]
the concatenated end polygon is the actual image of the surviving start
domain,
\[
  \Phi_2\!\left(\Phi_1(P_1\cap\Phi_1^{-1}(P_2))\right)
  =\Phi_2\!\left(\Phi_1(P_1)\cap P_2\right).
\]
The equality is the elementary identity
\(f(P\cap f^{-1}(Q))=f(P)\cap Q\) and does not require injectivity.
The concatenated affine map is
...
\begin{proof}
Choose the breakpoint at which \(\gamma\) changes from the direction
\(R_{s_1}\) to \(R_{s_2}\) at a point of
\(F_{s_1}\cap F_{s_2}\) as the start point.  Since the orbit has positive dwell
time on every listed facet, its successive breakpoints lie in the facet-pair
sections
...
The breakpoint may lie on additional facets; neither the construction nor this
argument requires it to lie in the relative interior of a two-face.
\end{proof}
\end{unverified}
...
zero polynomial whenever the chart is nonempty.  The common denominator
domain is a nonempty Zariski-open subset of the irreducible affine parameter
space, so a nonzero rational function cannot vanish identically after
restriction to a nonempty coordinate-minor chart.  The nonzero locus is the
complement of the algebraic zero
...
positive-time simple pure-facet closed Reeb trajectory represented by a flow-graph tube
...
positive-time simple pure-facet closed Reeb trajectory with action at most
...
not analyze floating-point rounding.  It also does not cover the exact Rust singular
...
Arbitrary bounded-irredundant input validation and broader Rust correspondence
remain separate implementation obligations. The earlier binary64 FG prototype
did not establish a sound true/false/indeterminate predicate contract and was
retired; it is not a current implementation route. In general, a numerical
frontend would need sound predicates or exact resolution of every
output-relevant predicate before its output could enter this exact theorem
boundary.
