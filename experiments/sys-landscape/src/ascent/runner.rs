...
///   returns** — rayon scheduling determines the append order within the
///   summary, trace, and cache files, and `finalize_ascent_output` is what
///   canonicalizes row order.
/// - `completed` is checked before calling `process`; resume semantics
///   therefore hold across crashes (see `write_seed_result` for the on-disk
...
/// `db` (outside `write_seed_result`, in the per-experiment closure) → `trace`
/// (inside `write_seed_result`) → `cache` (inside `write_seed_result`) →
/// `summary` (inside `write_seed_result`) → `best` (here, after
/// `write_seed_result` returns). Each lock is released before the next is
/// acquired — no nesting — so two threads cannot form a deadlock cycle
/// regardless of which seed each is processing.
///
/// Panic propagation: a panic inside `process` (or inside `write_seed_result`)
