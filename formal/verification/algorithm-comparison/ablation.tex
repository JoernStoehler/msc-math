% formal/verification/algorithm-comparison/ablation.tex — input by formal/main.tex
%
% Status: A-axis complete. Four variants: A0 (unpruned), A1 (vertex adjacency),
%   A2 (directed ω₀), A3 (Reeb-flow feasibility).
% Empirical results (datasets, iteration counts, timing): see
% experiments/verification/algorithm-comparison/ablation/ablation.jsonl and
% analyze.py.
%
...
strictly stronger necessary conditions at each step.
...
This is a linear feasibility problem on the face $F_i \cap F_j$,
...
Let $K = \{x : \langle a_k, x \rangle \le 1,\; k = 1, \ldots, F\}$
be a convex polytope with dual vertices~$a_k$ and facets~$F_k$.
Define the blocking set
$B = \{k \notin \{i,j\} : \omega_0(a_i, a_k) < 0
  \textup{ or } \omega_0(a_j, a_k) > 0\}$.
...
\begin{proof}
Throughout, we use $a_k \cdot V_\ell = \omega_0(a_\ell, a_k)$.

\emph{Necessity.}
Let $\gamma(0) = x \in F_i \cap F_j$ realize the transition.
For the approach ($\gamma(t) = x + t\, V_i$, $t < 0$),
$\langle a_j, \gamma(t) \rangle = 1 + t\, \omega_0(a_i, a_j) \le 1$
forces $\omega_0(a_i, a_j) \ge 0$ (since $t < 0$),
and the departure gives the same by antisymmetry. This is~(1).
For~(2), suppose $k \in B$ with $\omega_0(a_i, a_k) < 0$.
Then $\langle a_k, x + t\, V_i \rangle = \langle a_k, x \rangle + t\,\omega_0(a_i, a_k)$;
since $\omega_0(a_i, a_k) < 0$ and $t < 0$, this equals
$\langle a_k, x \rangle + |t|\,|\omega_0(a_i, a_k)| > \langle a_k, x \rangle$.
If $\langle a_k, x \rangle = 1$, this exceeds~$1$ for any $\varepsilon > 0$,
violating $\gamma(t) \in K$.
Hence $\langle a_k, x \rangle < 1$.
The argument for departure-blocking $k$ (with $\omega_0(a_j, a_k) > 0$)
is analogous.

\emph{Sufficiency.}
Given $x \in F_i \cap F_j$ satisfying~(1) and~(2),
set $\delta = \min_{k \in B} (1 - \langle a_k, x \rangle) > 0$.
For non-blocking $k \notin B \cup \{i,j\}$:
the approach satisfies
$\langle a_k, x + t\, V_i \rangle = \langle a_k, x \rangle + t\,\omega_0(a_i, a_k) \le \langle a_k, x \rangle \le 1$
since $t \le 0$ and $\omega_0(a_i, a_k) \ge 0$;
the departure satisfies
$\langle a_k, x + t\, V_j \rangle = \langle a_k, x \rangle + t\,\omega_0(a_j, a_k) \le \langle a_k, x \rangle \le 1$
since $t \ge 0$ and $\omega_0(a_j, a_k) \le 0$.
For blocking $k \in B$, the slack~$\delta$ absorbs the perturbation:
choosing $\varepsilon < \delta / \max_k(|\omega_0(a_i, a_k)| + |\omega_0(a_j, a_k)|)$
ensures all constraints hold.
The cases $k = i$ and $k = j$ follow from~(1) and
the antisymmetry $\omega_0(a_i, a_i) = 0$.
\end{proof}
\end{unverified}
