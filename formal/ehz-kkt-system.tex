% formal/ehz-kkt-system.tex — kkt module
% Mathematical results supporting library/src/kkt/
%
% Labels defined here:
...
%
% Sources: thesis/legacy/general-case-algorithm.tex,
%   thesis/legacy/general-case-algorithm-proof.tex,
%   thesis/legacy/appendix-numerical.tex
%
...
%
% For a polytope K with dual vertices a_i and an active-traversal pair
% (S, sigma),
...
%   [ 1^T  0    0 ] [ xi   ]   [ 1 ]
%
% See thesis/legacy/general-case-algorithm.tex, Algorithm [alg:ehz].


\begin{notationupdated}
...
% Review target: distinguish the one-word KKT contract from the outer scalar
% capacity argument. The candidate argument follows the proof in
% thesis/legacy/general-case-algorithm-proof.tex.
\begin{remark}[KKT witnesses and the finite capacity algorithm]
\label{rem:kkt-witness-versus-capacity}
For a fixed support and ordered word, the KKT system characterizes stationary
points of the equality-constrained quadratic objective on the relative
interior of that support face.  A solution with $\beta_i>0$ and
$Q_\sigma(\beta)>0$ is therefore an exact feasible KKT witness.  It need not
be the maximum of $Q_\sigma$ on that face.

The scalar finite Haim--Kislev algorithm uses these witnesses as candidates.
Every positive witness is feasible and hence has value at most the global
quantity $Q_{\max}$.  Conversely, a global maximizer becomes an interior KKT
point after deleting its zero coordinates.  Consequently, exact KKT solves
over a complete candidate family suffice for the scalar capacity value,
without requiring every stationary candidate to satisfy a second-order
maximum condition.  Such a condition, or another soundness argument, is
required when a fixed-word maximum or a complete minimizer set is claimed.
An arbitrary KKT witness does not determine a physical orbit.  In contrast, a
\emph{global} HK2017 maximizer already gives a global dual minimizer; Clarke's
dual action correspondence then realizes it as a minimum-action simple
generalized closed Reeb orbit (Fact~\ref{fact:hk2017}).  The translating
constant in that correspondence supplies the primal placement on~$\partial K$;
it is not provided by feasibility or stationarity of the finite KKT system
alone.
\end{remark}
...
\begin{lemma}[Dual-vertex QP formulation]%
\label{lem:dual-vertex-qp}
% Jörn: text approved (82daaaef) — statement and proof
% Source: qp_assembly.rs build_qp_from_dual_vertices
Let $K = \{x : \langle a_i, x \rangle \leq 1\}$ with dual
vertices~$a_i \in \mathbb{R}^4$.
For a cyclic permutation~$\sigma$ of $|S|$~facet indices,
define the constraint matrix~$A$ and action matrix~$H$ as in the
linear system (see header), and let $\mathbf{1} \in \mathbb{R}^{|S|}$
be the all-ones vector.
Then the fixed-word Haim--Kislev problem is
\[
  \max_{\beta}\; Q_\sigma(\beta)
  =\tfrac{1}{2}\,\beta^\top H\,\beta
  \quad\text{s.t.}\quad
  A^\top \beta = 0,\quad
  \mathbf{1}^\top \beta = 1,\quad
  \beta\geq0.
\]
If \(Q_\sigma(\beta)>0\), the associated dual curve has period and
action \(T=1/(2Q_\sigma(\beta))\).  This is a capacity value only when
the candidate is globally maximizing over the complete word family.
In particular, the factorization $a_i = n_i/h_i$
into unit normal~$n_i$ and support value~$h_i$ plays no role:
the constraints and objective depend only on the dual
vertices~$a_i$ and the bilinearity of~$\omega_0$.
\end{lemma}

\begin{proof}
The Reeb vector on facet~$i$ is
$R_i = 2\, J_0\, a_i$, where $J_0$ is the standard
complex structure.
For a closed pure-facet dual curve visiting facets
$\sigma(1), \ldots, \sigma(|S|)$ with dwell times
$T\beta_i$, where $\beta_i\geq0$ and $\sum_i\beta_i=1$:

\emph{Closure.}
The orbit closes iff
$\sum_i T\beta_i\, R_{\sigma(i)} = 0$.
Since $R_i = 2\, J_0\, a_i$ and $J_0$ is invertible, this is equivalent to
$\sum_i a_{\sigma(i)}\, \beta_i = 0$,
which is $A^\top \beta = 0$.

\emph{Normalization.}
The dwell-time fractions sum to one:
$\mathbf{1}^\top \beta = 1$.

\emph{Objective and action.}
For \(j<i\), the pair contribution is
$\beta_i\,\beta_j\,\omega_0(a_{\sigma(j)}, a_{\sigma(i)})$
in the active-word convention.
By Lemma~\ref{lem:H-quadratic}, the total action equals
$2T^2Q_\sigma(\beta)=T^2\beta^\top H\beta$,
where (H) is the symmetric action matrix defined above.
Choosing \(T=1/(2Q_\sigma(\beta))\) makes the action equal to \(T\),
which is the free-period dual normalization used by the Haim--Kislev
formula.  Feasibility or KKT stationarity for this one word does not establish
that \(T=c_{\mathrm{EHZ}}(K)\); that conclusion requires global maximality.

All three ingredients --- closure, normalization, action ---
are stated purely in terms of the dual vertices~$a_i$.
The factorization $a_i = n_i/h_i$ is never used, and
$\|n_i\| = 1$ is never assumed.
\end{proof}
...
% Source: thesis/legacy/appendix-numerical.tex
...
the transition.
For $k \notin \{i,j\}$ with
$\omega_0(a_i, a_k) \geq 0$ and
$\omega_0(a_j, a_k) \leq 0$:
$\langle a_k, x + t\, J_0 a_i \rangle
\leq \langle a_k, x \rangle \leq 1$
for $t \leq 0$,
and
$\langle a_k, x + t\, J_0 a_j \rangle
\leq \langle a_k, x \rangle \leq 1$
for $t \geq 0$.
For the remaining $k \notin \{i,j\}$
(those with $\omega_0(a_i, a_k) < 0$ or
$\omega_0(a_j, a_k) > 0$),
condition~(2) gives
$\langle a_k, x \rangle < 1$.
Let $\delta$ be the minimum of
$1 - \langle a_k, x \rangle > 0$
over these~$k$ (or any $\delta > 0$ if there are none),
and choose
$\varepsilon < \delta / \max_k(
|\omega_0(a_i, a_k)| + |\omega_0(a_j, a_k)|)$.
...
\[
  r \;=\; M\hat x - b
  \;=\; \begin{pmatrix}
    H\hat\beta + A\hat\mu + \mathbf{1}\hat\xi \\
    A^\top\hat\beta \\
    \mathbf{1}^\top\hat\beta - 1
  \end{pmatrix}
...
$H\beta_0 + A\mu_0 + \mathbf{1}\xi_0 = 0$.
Hence
\[
  \delta\beta^\top H\,\beta_0
  = -\delta\beta^\top(A\mu_0 + \mathbf{1}\xi_0)
  = -(A^\top\delta\beta)^\top \mu_0
    - (\mathbf{1}^\top\delta\beta)\,\xi_0.
\]
The constraint residual blocks give
$A^\top\delta\beta = A^\top\hat\beta = r_2$
(since $A^\top\beta_0 = 0$ by the exact constraint)
and similarly $\mathbf{1}^\top\delta\beta = r_3$, hence
\[
  -\delta\beta^\top H\,\beta_0
...
\[
  ...
  = \delta x^\top M\,\delta x
    ...
    - 2\,r_3\,\delta\xi,
\]
where we used $A^\top\delta\beta = r_2$
and $\mathbf{1}^\top\delta\beta = r_3$ from Step~2.
For the first term,
\[
  |\delta x^\top M\,\delta x|
  = |r^\top M^{-1} r|
...
$A^\top (\hat v_j)_\beta = \hat\lambda_j\, (\hat v_j)_\mu$
and
$\mathbf{1}^\top (\hat v_j)_\beta = \hat\lambda_j\, (\hat v_j)_\xi$.
Three types arise:
...
\item \textbf{Type~B} ($\|(\hat v_j)_\beta\| = O(1)$,
  ...
  Since $\mathbf{1}^\top (\hat v_j)_\beta \approx 0$,
  the direction has mixed signs,
...
\item \textbf{Type~C} ($\|(\hat v_j)_\beta\| = O(1)$,
  ...
  The implementation returns \texttt{KktOutcome::TypeCViolation}
  with a warning if a Type~C direction is ever encountered,
  skipping the orbit.
\end{itemize}
\end{remark}
\end{unverified}
