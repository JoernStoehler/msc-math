% formal/lagrangian-product-rotation-symmetry.tex — input by formal/main.tex
%
...
%   - Symmetry lemmas (rotation/reflection domain and factor-swap mirror)
%
% Empirical content (sweep descriptions, figures, observations) lives in
...
% See experiments/sys-landscape/random-product-sample/ for the random product sample experiment.
...
\begin{lemma}[Factor-swap mirror for equal odd regular polygons]
\label{lem:odd-regular-factor-swap-mirror}
Let~$P$ be a centered regular odd $n$-gon. Then
\[
  \mathrm{sys}(P \times_L R(\theta)P)
  =
  \mathrm{sys}(P \times_L R(\pi/n-\theta)P).
\]
For~$n=5$, this reduces the domain~$[0,\pi/5]$ from
Lemma~\ref{lem:rotation-fundamental-domain} to~$[0,\pi/10]$.
\end{lemma}

\begin{proof}
The map~$N=-J_0$ is symplectic and sends~$(q,p)$ to~$(p,-q)$.
Therefore it maps~$P \times_L R(\theta)P$ to
$R(\theta)P \times_L (-P)$. Next apply the simultaneous rotation
$\operatorname{diag}(R(-\theta),R(-\theta))$, which is symplectic. The image is
\[
  P \times_L R(-\theta)(-P).
\]
Since~$P$ is a centered regular odd $n$-gon, one has
$-P=R(\pi)P=R(\pi/n)P$, where the last equality uses the
$2\pi/n$ rotational symmetry of~$P$. Hence the image is
$P \times_L R(\pi/n-\theta)P$. Symplectic invariance of
$\mathrm{sys}$ gives the claimed equality.
\end{proof}
