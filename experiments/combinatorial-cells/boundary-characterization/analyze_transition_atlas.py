...
def main() -> None:
    print(
        "warning: this analysis may be stale after producer, epsilon-policy, "
        "selected-sigma, or schema changes; inspect the semantic checks and "
...
boundary-characterization JSONL artifacts. Input paths, row and byte counts,
and join checks are in `first-boundary-transition-summary.json`. A successful
run does not establish that the copied epsilon policy and exception
interpretation remain appropriate after producer changes.
