% formal/pentagon-rotation-capacity.tex — stale source material, not input by formal/main.tex
%
% Private stale proof draft for the explicit systolic-ratio formula of
% `P_5 x_L R(theta) P_5`.
%
% Status:
%   This file is source material only. It predates the current exact Sage
%   executable proof and still contains historical shortlisted-branch and
%   deleted CAS-witness references. Do not use it as current proof status.
%
% Sources:
%   - Current proof packet:
%     experiments/regular-products/pentagon-rotation-formula-proof/
%   - Current empirical packet:
%     experiments/regular-products/pentagon-rotation-empirics/
%   - HKO source: papers/hko2024/counterexample.tex
%   - Thesis-facing content guide:
...
\]
It was used for agent and Jörn review. It is retained only as stale source
material for calculations that have not all been copied into the thesis
companion.
...
symbolic reduction is part of the exact CAS witness from
Remark~\ref{rem:pentagon-rotation-cas-witness}, which verifies that the switch
equation is solved on $0 \le \theta \le \pi/10$ by
\[
  \lambda(\theta)
  =
  \frac{1}{2} - \frac{\tan(\theta)}{2\tan(\pi/10)},
\]
and that the one-sided slopes are
\[
...
The same exact CAS witness verifies the support values at the minimizing
parameter:
\[
...
\ref{lem:pentagon-rotation-three-bounce-second-family} and
\ref{lem:pentagon-rotation-three-bounce-third-family} are verified exactly by
the script
...
actions recorded in the proofs below and symbolically checks:
\[
  \lambda(\theta), \quad
...
\]
for the finite descriptor list of shortlisted $3$-bounce families, by exact
symbolic simplification. In other words, the trusted statement here is not that
the TeX proof ``reads off'' these identities, but that the external symbolic
witness loops over the declared branch descriptors and verifies the claimed
closed forms from the reduced setup formulas. The developer command is
...
for each declared branch descriptor:\\
\qquad define the reduced formulas for the active support pattern\\
\qquad define the claimed closed form for the branch output\\
\qquad assert \texttt{simplify(}output-claimed\_form\texttt{) = 0}
\end{quote}
\end{remark}
\end{unverified}
...
\[
  A := 1+\cos\left(\frac{\pi}{5}\right).
\]
A direct support evaluation for the named faces reduces this branch to the
system
\[
  a(\theta)\sin\left(\theta+\frac{3\pi}{10}\right) = \frac{\sqrt{5}}{2},
\]
\[
...
\]
The exact CAS witness from Remark~\ref{rem:pentagon-rotation-cas-witness}
starts from these reduced formulas and checks
\[
  A_1(\theta) - g(\theta)
  =
  \frac{
...
\[
  A := 1+\cos\left(\frac{\pi}{5}\right).
\]
Again, a direct support evaluation reduces the branch to
\[
  a(\theta)\sin\left(\frac{3\pi}{10}-\theta\right) = \frac{\sqrt{5}}{2},
\]
\[
...
\[
  ...
  A\,a(\theta) + \frac{\sqrt{5}}{2}b(\theta).
\]
By the exact CAS witness from Remark~\ref{rem:pentagon-rotation-cas-witness},
\[
  A_2(\theta) - g(\theta)
  =
  \frac{
...
\begin{unverified}
\begin{lemma}[The competitive EEE/EEE branch stays above the 2-bounce candidate]
\label{lem:pentagon-rotation-three-bounce-third-family}
Keep the notation of
Lemma~\ref{lem:pentagon-rotation-three-bounce-first-family}, and write
\[
...
\]
which is a convenient representative of the canonical affine signature
\texttt{Q:0-1-3\ \ P:0-2-3}. Let
\[
  x_0 \in e_0,
  \qquad
  x_1 := x_0 - \varphi t_q(\theta)m_0(\theta) \in e_3,
  \qquad
  x_2 := x_1 - t_q(\theta)m_3(\theta) \in e_1,
\]
\[
  y_0 \in R(\theta)e_0,
  \qquad
  y_1 := y_0 + \varphi t_p(\theta)n_3 \in R(\theta)e_3,
  \qquad
  y_2 := y_1 + t_p(\theta)n_1 \in R(\theta)e_2,
\]
with closure
\[
  x_0 = x_2 - t_q(\theta)m_2(\theta),
  \qquad
  y_0 = y_2 + t_p(\theta)n_0.
\]
Then the reduced support equations force
\[
  t_q(\theta) = t_p(\theta)
  =
  \frac{10+2\sqrt{5}}{4\tan\left(\frac{\pi}{5}\right)\sin\theta + 12\cos\theta},
\]
and the affine product action of this branch is
\[
  A_3(\theta)
  =
  \frac{
    \frac{5}{2}\left(2+\sqrt{5}\right)
  }
  {
    \tan\left(\frac{\pi}{5}\right)\sin\theta + 3\cos\theta
  }.
\]
In particular,
\[
  ...
  \text{for } 0 \le \theta < \frac{\pi}{10},
\]
and $A_3(\pi/10)=g(\pi/10)$.
\end{lemma}

\begin{proof}
The vector identities
\[
  \varphi m_0(\theta) + m_3(\theta) + m_2(\theta) = 0,
  \qquad
  \varphi n_3 + n_1 + n_0 = 0
\]
fix the closure ratios in the displayed $q$- and $p$-cycles. The exact CAS
witness from Remark~\ref{rem:pentagon-rotation-cas-witness} starts from these
reduced support relations and checks the common scalar solve
\[
  t_q(\theta) = t_p(\theta)
  =
  \frac{10+2\sqrt{5}}{4\tan\left(\frac{\pi}{5}\right)\sin\theta + 12\cos\theta}
\]
and the action formula above. The same witness also checks
\[
  A_3(\theta) - g(\theta)
  =
  \frac{
    5\bigl(A\cos\theta - B\sin\theta\bigr)
  }
  {
    4\bigl(D\sin\theta + 12\cos\theta\bigr)\cos\theta
  },
\]
where
\[
  A := 2(\sqrt{5}-1),
  \qquad
  B := (\sqrt{2}+\sqrt{10})\sqrt{5-\sqrt{5}},
  \qquad
  D := (\sqrt{10}-\sqrt{2})\sqrt{5-\sqrt{5}}.
\]
It further checks the constant identities
\[
  A = 8\sin\left(\frac{\pi}{10}\right),
  \qquad
  B = 8\cos\left(\frac{\pi}{10}\right),
  \qquad
  D = 4\tan\left(\frac{\pi}{5}\right).
\]
Therefore
\[
  A_3(\theta) - g(\theta)
  =
  \frac{
    10\sin\left(\frac{\pi}{10}-\theta\right)
  }
  {
    \bigl(4\tan\left(\frac{\pi}{5}\right)\sin\theta + 12\cos\theta\bigr)\cos\theta
  }.
\]
On $0 \le \theta < \pi/10$, the numerator is positive, and every denominator
factor is positive. At $\theta=\pi/10$ the numerator vanishes. This proves the
claim.
\end{proof}
\end{unverified}
