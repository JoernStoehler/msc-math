...
| `pca-projection` | PCA projection. Does a low-dimensional linear projection expose a reproducible rule for proposing high-`sys` candidates? | Current retained-dataset run and PC2-high audit completed locally. PC2-high has statistically meaningful descriptive top-1% enrichment among already evaluated retained rows; the signal is product/source-entangled and non-monotone, but still enriches source-local top-1% rows within `gradient_ascent_products`. This work does not define a candidate-generation interface and found no validated new row. | Current mixed evidence with primary label `ran with no candidate-proposer and no new validated row`; secondary labels `supporting evidence only` and `future reopen trigger`. | `pca-projection/analyze.py`; `pca-projection/interpret_pc2_high.py` | `pca-projection/report.md` | Guard: PCA inputs exclude `sys`, capacity columns, endpoint labels, dataset identity, optimizer provenance, and post-hoc `sys` inspection. Thesis use: mixed PCA projection row, not a clean negative descriptive result. |
