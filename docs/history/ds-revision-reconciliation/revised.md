...
Decreasing $R$ does not, however, provide a generally reliable way to increase the systolic ratio. Two rotation families of polygon products, with edge counts $3\times6$ and $4\times4$, exhibit the opposite behavior numerically: both $R$ and the numerical systolic ratio decrease across the four sampled positions in each family. The pentagon identity therefore explains one family, while these paths warn against extrapolating its direction of improvement to other products.
...
[^methods]: The [methods notes](methods-notes.md) record the control exclusions, selection procedure and reproduction details.
