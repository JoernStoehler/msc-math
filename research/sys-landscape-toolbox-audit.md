...
| `M011` feature-block regression | Do cheap feature blocks predict `sys` well enough to transfer from random samples to endpoints? | tabular random and endpoint regimes | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze.py`; feature JSONL/PNG artifacts in the same directory; `research/sys-landscape.md`; `tasks/landscape.md` | Grouped CV by lineage/source; transfer between random and endpoint regimes is the load-bearing guard. | Current notes say endpoint-side features are useful within random data but weak for transfer to endpoints. | Supports the bounded claim that the current feature packet did not produce a reusable global-search heuristic. | `attempted, negative vs random` | Main evidence for the data-science hostile-landscape paragraph. | Reopen if refreshed feature packets show random-to-endpoint transfer or add local-maxima-specific transfer evidence. |
| `M012` regime classification | Can standard classifiers separate endpoint from random regimes, and is the separator geometric rather than provenance-heavy? | grouped tabular regime-separation task | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze_regime_classification.py`; `regime_classification_bars.png` | Grouped split; metadata/provenance fields are an explicit validity caveat. | Analyzer compares logistic regression and random forest across metadata, geometry, orbit, trajectory, and combined blocks. Ledger marks thesis use undecided. | May show regime separation, but metadata-heavy separation does not by itself give a geometry-based search heuristic. | `attempted, negative vs random` / `validity caveat` | Undecided; needs Jörn decision before thesis-facing use. | Reopen after Jörn decides whether classification is claim-bearing, caveat-only, or omitted. |
...
| PCA / global dimensionality reduction | Would a global low-dimensional embedding become a search heuristic? | deferred global feature embedding | No committed hostile-landscape PCA method packet found in the ledger. | A valid version would need lineage-safe fitting plus a rule that maps embeddings back to candidate search actions. | Not attempted as a method program in the current ledger. | Skipping PCA does not weaken the current artifact-backed claim; it remains a standard-toolbox family not covered. | `skipped: expensive to implement` | Explicit omitted-family caveat only. | Reopen if a cheap embedding-to-candidate rule is proposed using existing feature tables. |
| Clustering / manifold learning | Would unsupervised grouping of landscape rows reveal a search-relevant regime? | deferred unsupervised feature packet | No committed clustering or manifold-learning method packet found in the ledger. | A valid version would need lineage-safe feature construction and a non-post-hoc cluster-to-search rule. | Not attempted as a method program in the current ledger. | This is a skipped standard-toolbox family, not negative evidence. | `skipped: expensive to implement` | Explicit omitted-family caveat only. | Reopen if writing needs a named unsupervised comparison or existing feature tables make it cheap. |
