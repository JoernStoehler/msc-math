...
//!         experiments/sys-datascience/produce/ascent-general-endpoints.jsonl
//!         experiments/sys-datascience/produce/ascent-general-trace.jsonl
//!
...
//! `cargo run -p exp-sys-landscape --release --bin sys-dataset-ascent -- --out experiments/sys-datascience/produce/ascent-general-endpoints.jsonl --db-update`
//!
...
fn main() {
    ...
    run_parallel_seeds(&args, &completed, &writers, &best, move |i, seed_i| {
        ...
        let mut polytope_opt: Option<SysLandscapePolytopeCache> = None;
        for _ in 0..MAX_POLYTOPE_ATTEMPTS {
            if let Some(p) =
                SysLandscapePolytopeCache::sample_random(FACET_COUNT, H_MIN, H_MAX, &mut rng_i)
            {
                polytope_opt = Some(p);
                break;
            }
