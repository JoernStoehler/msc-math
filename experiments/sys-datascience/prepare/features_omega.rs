...
pub fn compute_omega_fields(
    ...
    let singular_values = omega_svd
        ...
        .copied()
        .collect::<Vec<_>>();
    let omega_matrix_frobenius_norm = singular_values
        .iter()
        .map(|value| value * value)
        .sum::<f64>()
        .sqrt();
    let omega_matrix_spectral_norm = singular_values.iter().copied().fold(0.0, f64::max);
    let omega_matrix_stable_rank = if omega_matrix_spectral_norm > 0.0 {
        omega_matrix_frobenius_norm.powi(2) / omega_matrix_spectral_norm.powi(2)
    } else {
        0.0
    };
    let omega_matrix_rank_1em10 = singular_values
        .iter()
        .filter(|&&value| value > 1e-10)
        .count() as f64;
    let omega_matrix_nullity_1em10 = facet_count as f64 - omega_matrix_rank_1em10;
    ...
    let ridge_abs_omegas = two_faces
        .iter()
        .map(|two_face| omega0(&duals[two_face.facets[0]], &duals[two_face.facets[1]]).abs())
        .collect::<Vec<_>>();
    ...
    OmegaFields {
        transition,
        omega_matrix_frobenius_norm,
        omega_matrix_spectral_norm,
        omega_matrix_stable_rank,
        omega_matrix_rank_1em10,
        omega_matrix_nullity_1em10,
        omega_sign_out_degree_mean,
        omega_sign_out_degree_std,
        omega_sign_out_degree_min,
        omega_sign_out_degree_max,
        ...
        allpair_abs_omega_min,
        allpair_abs_omega_max,
        allpair_abs_omega_q25: quantile_or_zero(&allpair_abs_omegas, 0.25),
        allpair_abs_omega_median: quantile_or_zero(&allpair_abs_omegas, 0.50),
        allpair_abs_omega_q75: quantile_or_zero(&allpair_abs_omegas, 0.75),
        allpair_abs_omega_q90: quantile_or_zero(&allpair_abs_omegas, 0.90),
        allpair_abs_omega_top3_share: top_k_share(&allpair_abs_omegas, 3),
        ...
        ridge_abs_omega_min,
        ridge_abs_omega_max,
        ridge_abs_omega_q25: quantile_or_zero(&ridge_abs_omegas, 0.25),
        ridge_abs_omega_median: quantile_or_zero(&ridge_abs_omegas, 0.50),
        ridge_abs_omega_q75: quantile_or_zero(&ridge_abs_omegas, 0.75),
        ridge_abs_omega_q90: quantile_or_zero(&ridge_abs_omegas, 0.90),
        ridge_abs_omega_top3_share: top_k_share(&ridge_abs_omegas, 3),
        ...
        ridge_abs_normalized_omega_min,
        ridge_abs_normalized_omega_max,
        ridge_abs_omega_le_1em3_fraction: fraction_at_most(&ridge_abs_omegas, 1e-3),
        ridge_abs_omega_le_1em2_fraction: fraction_at_most(&ridge_abs_omegas, 1e-2),
        ridge_abs_omega_le_1em1_fraction: fraction_at_most(&ridge_abs_omegas, 1e-1),
