#!/usr/bin/env python3
"""Compare pilot features with immutable historical table, preserving all discrepancies."""
...
selection=[json.loads(x) for x in (here.parent/'pilot-inputs.jsonl').read_text().splitlines()]
assert len(new)==36 and len({r['poly_id'] for r in new})==36
assert {r['poly_id'] for r in new}=={r['id'] for r in selection}
