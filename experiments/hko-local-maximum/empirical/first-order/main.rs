...
//! Output Artifacts: experiments/hko-local-maximum/empirical/first-order/hko-neighborhood-sensitivity.jsonl
//!         experiments/hko-local-maximum/empirical/first-order/hko-neighborhood-ascent.jsonl
//!         experiments/hko-local-maximum/empirical/first-order/exact-certification-bank.jsonl
//!
...
fn exact_bank_output_path(base_dir: &Path, canonical: bool) -> PathBuf {
    if canonical {
        base_dir.join("empirical/first-order/exact-certification-bank.jsonl")
    } else {
        base_dir.join("empirical/first-order/smoke-exact-certification-bank.jsonl")
    }
}
...
fn run_phase_a(base_dir: &std::path::Path, smoke: bool) {
    ...
    let sens_path = base_dir.join("empirical/first-order/hko-neighborhood-sensitivity.jsonl");
    ...
    let ascent_path = base_dir.join("empirical/first-order/hko-neighborhood-ascent.jsonl");
...
fn main() {
    ...
    std::fs::create_dir_all(base_dir.join("empirical/first-order"))
        .expect("create empirical/first-order output dir");
...
mod tests {
    ...
    fn exact_bank_output_paths_match_smoke_and_canonical_contract() {
        ...
        assert_eq!(
            ...
            base.join("empirical/first-order/smoke-exact-certification-bank.jsonl")
        );
        assert_eq!(
            exact_bank_output_path(base, true),
            base.join("empirical/first-order/exact-certification-bank.jsonl")
        );
    }
