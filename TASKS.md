...
- Execution status on `capacity-result-api-exec`: Packet 1 scaffold landed as `library/src/algorithms/orbit_search.rs` with shared enums/types for the new result layer. Packet 2 is in progress: there is now a shared `solve_orbit_sigma(...)` primitive on the saddle-point path, a shared `collect_legacy_capacity(...)` seam for the current solve/classify/track/finalize loop, and the first internal exact-fallback / guarantee-mode helpers. The remaining Packet 2 work is the actual `OrbitSearchResult`-returning collector surface above those seams.
...
- Acceptance check for the design session: a short architecture note or `TASKS.md` packet names the proposed public functions/types, migration path for copied experiment instrumentation, verification commands, explicit non-goals for the thesis push, and the required doc updates (`TASKS.md`, `ARCHITECTURE.md`) plus the rerun surface that will demonstrate the refactor actually worked.
