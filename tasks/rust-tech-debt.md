...
| Exact arithmetic replacement branch | `[active]` | mainline thesis if exact validation is cited or exact-code ambiguity blocks main-branch task sessions | agents, Jörn for ambiguous project-role classification, deletion/provenance decisions, and math/proof acceptance | Use `tasks/delete-algebraic-crate-done.md`. Finish the branch inventory, migrate thesis-relevant generic consumers to the new crate, keep domain code in domain crates/experiments, and fill the merge checklist. | `tasks/delete-algebraic-crate-done.md`, `crates/algebraic-numbers/README.md`, `crates/algebraic-numbers/DEVELOPMENT.md`, `research/numerics.md`, `crates/MAP.md` |
