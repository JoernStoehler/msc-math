...
| Where is active maintainability work tracked, and what is the current phase? | `TASKS.md:445-495` and `.codex/reference/repo-maintainability/design/main.md:12-20, 95-107, 230-237` | No gap for status. This is already the home for the program plan, packet queue, and safe resume point. | Tracker packet / no new doc needed |
...
| How does the JSONL cache/data layer work? | `crates/symplectic/src/database.rs:1-54, 114-140` and `TASKS.md:468-482` | The storage contract is clear, but the canonical shared catalog versus mirror versus transient-search distinction is still open. That is an architecture decision, not a wording gap. | Tracker packet first; `ARCHITECTURE.md` after the policy lands |
