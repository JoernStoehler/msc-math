...
| Tracing/instrumentation surface | `[implemented first slice]` | future/follow-up | agents, Jörn only for architecture if a broader setup becomes invasive | Branch `tracing-perf-integration` added the first standard `tracing` slice: opt-in `--trace` subscriber in `profile-pruned-hk2017` and production spans/events/counters around `polar_vertices_exact`. Broaden only when another concrete profiling question needs more spans; keep tracing opt-in, ignorable, and out of return types/stdout data paths. | 2026-05-11 profiling discussion, `crates/symplectic/DEVELOPMENT.md`, `crates/symplectic/src/bin/profile_pruned_hk2017.rs`, `.agents/skills/rust/SKILL.md` |
