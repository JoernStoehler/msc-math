...
- **Figure inclusion (from CLAUDE.md § Figure sizing)**: All formatting in Python, LaTeX is 1:1 pass-through. Flag any `\includegraphics` with `width=`, `height=`, or `scale=` parameters. Flag tables using `\scriptsize` or `\tiny`. Tables should use `booktabs` (`\toprule`/`\midrule`/`\bottomrule`), not `\hline`.
