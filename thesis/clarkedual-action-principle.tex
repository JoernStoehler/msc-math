...
In order to show the main result \ref{thm:main} from \cite{HK2017} we need to
switch from the minimum action optimization problem to a different, dual, optimization problem,
which has several properties that make it easier to prove combinatorial properties of the minimizers.
...
adapted to polytopes), later we will construct a combinatorically simple minimizer via a
five-step simplification proof.
...
so $g_K^2(x) = \lambda^2$.
Then:
% FIXME(hbox): 19.40pt — wide displayed equation with nested \sup; try breaking into align* with = signs aligned, or use \allowdisplaybreaks
\[
  (g_K^2)^*(y)
...
% FIXME(hbox): 6.26pt — long inline formula "$(g_K^2)^*(y) = \sup_z \{\langle z, y \rangle - g_K^2(z)\}$"; try moving the definition to a displayed equation or use \allowbreak before "i.e."
...
\cite{HK2017} also states the dual problem
...
% FIXME(hbox): 62.20pt — line "\frac{\nu}{\alpha^2} z'(t) + \Bigl(...\Bigr) \in \partial(...)" too wide; break align* line after \Bigr) with \\ \quad\quad\in or use multline for that step
\begin{align*}
  \nu z(s) + b_0
  ...
  &\Rightarrow \quad
  \frac{\nu}{\alpha^2} z'(t) + \Bigl(\tfrac{b_0}{\alpha} - \tfrac{\nu b}{\alpha^2}\Bigr)
  \in \partial\bigl(\tfrac{1}{4}h_K^2\bigr)(-J_0\dot{z}'(t)).
\end{align*}
...
The main properties we want to highlight here that differ between the primal and dual problems are:
\begin{itemize}
\item The primal problem has a differential inclusion constraint, while the dual problem searches over curves with a fixed integral constraint on the action.
\item The primal problem has a surface constraint ($\gamma$ lies on $\partial K$), while the dual problem has no position constraint.
\item The dual functional $I_K$ depends only on the velocity $\dot{z}$, not on the position $z$.
\end{itemize}
These properties give us significantly more freedom to construct new minimizers from old ones in the dual problem.
To go from primal to dual, and from dual to primal, we need to prove the following equivalence result.
...
% FIXME(hbox): 10.65pt — inline formula "$-J_0\dot{\gamma} \in \partial g_K^2(\gamma)$" causes overflow; try \allowbreak before "at" or move the formula to a separate displayed equation
\medskip
\emph{Minimum equality.}
For any Reeb orbit~$\gamma$, the Fenchel equality
(Lemma~\ref{lem:fenchel-duality}) at $-J_0\dot{\gamma} \in \partial g_K^2(\gamma)$
gives pointwise:
\[
...
% [TODO: JÖRN - verify the base point recovery lemma, proof, and algorithm below.
%  Agent-written derivation based on tangency of Reeb vectors. The linear system
%  is straightforward but the inequality constraints and non-uniqueness discussion
...
Let $K$ be a polytope with $F$ facets.
Let $\sigma = (\sigma(1), \ldots, \sigma(m))$ be a cyclic
permutation of a subset of $\{1, \ldots, F\}$, and let
$\tau_1, \ldots, \tau_m > 0$ be positive dwell times.
...
The number of active facets $m = |S|$ determines
the dimension of the solution space:
\begin{itemize}
\item $m \geq 4$ with linearly independent normals:
  $b$ is uniquely determined (generically the case
  for orbits visiting $\geq 4$ facets).
\item $m < 4$:
  the affine subspace has dimension $4 - m$,
  and the inequality constraints select a
  convex subset.
\item $m > 4$:
  the system is overdetermined but consistent
  (a valid base point exists by
