...
- performance comparison against Rust exact or Rust `f64`.
...
| `experiments/hko-local-maximum/sage-validation/analyze.py` | Sage-driven validator and comparison script |
...
- the two HKO rows `hko_exact_winning_sigma` and
  `hko_exact_rank_deficient_sigma` were recovered with kernel dimension `1`.

## Next packet if this succeeds

Use the same export + Sage-check path for the HKO active-gradient linear-algebra
record behind the 15D flat-space claim, or spin out a separate numerics-methods
experiment for Sage feasibility and performance.
