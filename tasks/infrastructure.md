...
- Source surfaces: `AGENTS.md`, `.agents/skills/harness-engineering-gpt-55/`,
  old harness extraction,
...
| PM/research convention refactor | `[done]` | map input | current session | Added next-map layer, research index, task outcome conventions, and protocol-style skills. Those old skills are now deleted during GPT-5.5 harness migration. | `AGENTS.md`, `research/INDEX.md`, old harness extraction |
...
| Cached-map workflow skill | `[done]` | map input | current session | Added the protocol skill and used it for the first `crates/MAP.md` / `experiments/MAP.md` refresh. | old harness extraction: cached-map candidate, `crates/MAP.md`, `experiments/MAP.md` |
| Harness style baseline | `[map-input]` | map input | Jorn + harness discussion | Define the harness style convention before broad cleanup: which artifacts hold domain knowledge, procedure knowledge, temporary session state, durable instructions, and TODO workflow placeholders; then use that convention to decide what to extract, delete, rewrite, or leave alone. | `AGENTS.md`, old harness extraction, `tasks/README.md` |
...
| Chat communication style taxonomy | `[map-input]` | map input | Jorn + external evidence | Capture Jörn's concrete examples of chat-style failures and candidate repairs, then compare them against outside guidance before writing a durable chat-communication convention. | `AGENTS.md`, old harness extraction: harness-engineering candidate, old harness extraction: codex reference candidates |
| Difficult-interaction recovery | `[map-input]` | map input | Jorn + harness discussion | Define how agents should detect that frequent back-and-forth, accumulated errors, or unclear goals have pushed the session outside ordinary non-interactive coding mode; include an abort/restart path that carries forward only useful state when repair is cheaper than continuing. Use the chat-style taxonomy as input. | `AGENTS.md`, old harness extraction: harness-engineering candidate, old harness extraction: codex reference candidates |
| Goal-clarification mode | `[map-input]` | map input | Jorn + harness discussion | Define an explicit mode for complex goal clarification where agents do not start implementation, planning-by-inertia, or cleanup until the objective and intended artifact are settled. | `AGENTS.md`, old harness extraction: harness-engineering candidate, old harness extraction: codex reference candidates |
...
| Current-model harness prompt modernization | `[map-input]` | map input | Jorn + official-doc check | Audit prompt-engineering assumptions embedded in harness material; replace stale GPT-4 through GPT-5.3 and older-Claude-style advice only where it conflicts with current GPT-5.5, Claude Opus 4.6/4.7, or observed repo failures. | old harness extraction: harness-engineering candidate, old harness extraction: skill candidates, old harness extraction: codex reference candidates |
| Harness engineering skill revisit | `[future]` | future/follow-up | Jorn + agent review | Around 2026-05-07, reread old harness extraction: harness-engineering candidate after live use and add only harness-engineering practices Jörn remembered or agents encountered. Preserve the skill's objective/success-measurement structure; do not add proxy checks without a remembered/observed failure or official guidance. | old harness extraction: harness-engineering candidate |
...
- [stale 2026-05-03] The old rule "keep `AGENTS.md` short and put detailed
  rules in skills" was superseded by the GPT-5.5 harness migration. Rebuild
  target surfaces from the old harness extraction instead of assuming the old
  skill split.
  Refresh by: reading `AGENTS.md` and the old harness extraction.
