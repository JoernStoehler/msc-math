...
As in Lemma~\ref{lem:rank-deficiency-dismissal}, define the
walk-to-boundary quantities:
...
Then $\beta^*_k = 0$, $\beta^*_j \geq 0$ for all $j \in S$
(the walk-to-boundary and non-negativity arguments of
Lemma~\ref{lem:rank-deficiency-dismissal} are purely algebraic),
and—provided the feasibility correction below keeps
$\beta' \geq 0$—there exists $\beta' \in M(K)|_{S'}$ such that
the $(S', \sigma|_{S'})$ algorithm finds value at least
$\nu_0 - O(\varepsilon)$.
The explicit bound is derived in the proof.
\end{proposition}

\begin{proof}
The walk-to-boundary construction (Steps~1--4 of
Lemma~\ref{lem:rank-deficiency-dismissal}) carries over
unchanged: it uses only $\beta_0 > 0$, $\delta\beta$ having
mixed signs, and the definition of~$\alpha^*$.
The boundary point $\beta^* = \beta_0 + \alpha^*\delta\beta$
satisfies $\beta^*_k = 0$ and $\beta^*_j \geq 0$.
What changes is that $\beta^*$ is no longer an exact solution,
so we track the error.

\emph{Step~5${}'$: Action-value perturbation along the walk.}
...
$\delta\beta^\top H\,\beta_0 = 0$ and
$\delta\beta^\top H\,\delta\beta = 0$.
Here both are $O(\varepsilon)$ instead.
...
\[
  ...
  = (N^\top\delta\beta)^\top \lambda_0
    + (\eta^\top\delta\beta)\,\nu_0
  = r_\beta^\top \lambda_0 + r_\nu\,\nu_0,
\]
so
$|\delta\beta^\top H\,\beta_0|
  \leq \|r_\beta\|\,\|\lambda_0\| + |r_\nu|\,|\nu_0|
...
\[
  ...
  = (N^\top\delta\beta)^\top \delta\lambda
    + (\eta^\top\delta\beta)\,\delta\nu
    + \delta\beta^\top r_\lambda
  = r_\beta^\top \delta\lambda + r_\nu\,\delta\nu
    + \delta\beta^\top r_\lambda.
\]
Since $\|\delta\beta\|, \|\delta\lambda\|, |\delta\nu| \leq 1$
(unit vector) and
$\|r_\beta\|, \|r_\lambda\|, |r_\nu| \leq \varepsilon$:
\[
...
\emph{Step~6${}'$: Constraint violation of $\beta^*|_{S'}$.}
Since $N^\top\beta_0 = 0$, $\eta^\top\beta_0 = 1$,
and $\beta^*_k = 0$:
\begin{align*}
  {N'}^\top \beta^*|_{S'}
  &= N^\top\beta^*
  ...
  &= \eta^\top\beta^*
   ...
   = 1 + \alpha^*\,r_\nu.
\end{align*}
Both violations are bounded by $\alpha^*\varepsilon$.

\emph{Step~7${}'$: Feasibility correction.}
Let $C' = [N' \mid \eta'] \in \mathbb{R}^{|S'|\times 5}$
be the constraint matrix for $(S', \sigma|_{S'})$,
with smallest singular value $\sigma_C > 0$.
The minimum-norm correction
$\Delta\beta$ satisfying
${C'}^\top\Delta\beta
  = (\alpha^*r_\beta,\;\alpha^*r_\nu)^\top$
has
\[
  \|\Delta\beta\|_2
...
If $\|\Delta\beta\|_\infty < \min_{j \in S'}\beta^*_j$
(which holds for $\varepsilon$ small enough),
then $\beta'_j > 0$ for all $j \in S'$,
and ${N'}^\top\beta' = 0$, ${\eta'}^\top\beta' = 1$,
so $\beta' \in M(K)|_{S'}$.
...
\[
  ...
  \geq \nu_0
    - \underbrace{
        \varepsilon\,\alpha^*
        \bigl[2(\|\lambda_0\| + |\nu_0|) + 3\alpha^*\bigr]
      }_{\Delta_{\mathrm{walk}}}
    - \underbrace{
        \frac{2(\|\beta_0\| + \alpha^*)\,\|H\|\,
              \alpha^*\varepsilon}{\sigma_C}
      }_{\Delta_{\mathrm{proj}}}
  = \nu_0 - O(\varepsilon).
\]
\end{proof}
