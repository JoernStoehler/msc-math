...
\begin{enumerate}
  ...
  \item \textbf{LU fast path (evaluated but unhelpful).} We implemented an
    ...
    path (falling through to SVD on failure), accepting the modest
    overhead in exchange for handling edge cases where gap-based SVD
    rank detection over-truncates legitimate small singular values.
\end{enumerate}
