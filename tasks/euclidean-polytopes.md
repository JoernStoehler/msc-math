...
| Polar vertex enumeration and e2e pipeline performance | `[merged first optimization; not approved complete]` | mainline thesis | agents, Jörn review gate for completion | Branch `tracing-perf-integration` added opt-in tracing, restored checked polar origin validation, removed the unverified generic exact f64 rejection prefilter, and promoted the first exact optimization: BigRational common-denominator integer scaling for the origin-interior assertion plus polar determinant/gap enumeration. The production exact path no longer uses f64 or 5-set decisions. Random F=8 pruned HK2017 profile improved from exact geometry avg `40.407ms` and total avg `42.968ms` to exact geometry avg `4.438ms` and total avg `7.079ms` over 20 deterministic checked samples, with deterministic outputs unchanged. This is merged evidence, not approval that the e2e path is finished: the end-to-end optimization still needs a focused review of remaining bottlenecks, whether f64-certified decisions should be used anywhere on the hot path, and whether the F=8 benchmark is the right completion gate. Do not mark complete until that review is recorded and Jörn accepts the performance/quality tradeoff. | `formal/rational-integer-scaling.tex`, `formal/f64-orientation-sign-filters.tex`, `crates/symplectic/src/bin/profile_pruned_hk2017.rs`, `crates/euclidean-polytopes/src/polar.rs`, `crates/euclidean-polytopes/src/predicates.rs`, merged branch history `tracing-perf-integration` |
