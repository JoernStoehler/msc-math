...
measurements also enriches fresh samples, but a more extreme ridge filter did not
establish an additional gain. These observations support a coarse predictor;
they do not yet give a reliable rule for improving a selected body.

The distinction has a geometric counterpart. An edge--width formula explains
what the ridge measurement records on Lagrangian products. For rotated
regular pentagons it has an exact inverse-square relation to the systolic
ratio; see equation~\ref{eq:ds-pentagon-ridge-capacity}. Yet the cube has a
lower ridge score than HKO and a lower systolic ratio, which refutes a
universal inverse ordering. The empirical question is therefore why the
association is strong in the sampled populations, and which restrictions
could turn it into a mathematical statement. The search experiments test
its practical value alongside deformations and adaptive sampling.
