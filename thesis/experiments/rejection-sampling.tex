...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item \emph{Boundedness}: the normals must positively span~$\mathbb{R}^4$,
      i.e., no direction is left ``uncovered.'' For every triple~$(i,j,k)$,
      the three normals share a 1D kernel direction~$d$ (computed via the
      generalized cross product in~$\mathbb{R}^4$). The polytope is bounded
      only if some other normal blocks~$d$ (has positive inner product
      with~$d$) and some other normal blocks~$-d$.
