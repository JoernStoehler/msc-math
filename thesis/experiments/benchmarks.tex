...
\begin{enumerate}
  ...
  \item \textbf{LU fast path (evaluated but unhelpful).} We implemented an
    ...
    the overhead persists. We retain the LU path in code for future
    evaluation but the production solver is SVD-only.
\end{enumerate}
...
computes each capacity in ${\sim}850$\,ms (median) using the SVD-only
solver, compared to ${\sim}2{,}500$\,ms for HK2017.
