...
use super::{
    enumeration::{for_each_product_billiard_sigma, for_each_transition_pruned_sigma},
    orbit::{
        solve_f64_orbit_sigma_saddle_point, F64OrbitAdmissibility, F64OrbitKktData,
        F64OrbitSolveError,
    },
};
...
pub(crate) struct CandidateSolveSummary {
    ...
    admissible: Vec<F64OrbitKktData>,
    indeterminate: Vec<F64OrbitKktData>,
}

impl CandidateSolveSummary {
    pub(crate) fn best_admissible(&self) -> Option<&F64OrbitKktData> {
    ...
    fn record_sigma(&mut self, dual_vertices: &[Vector4<f64>], sigma: &[usize]) {
        ...
        let solved = solve_f64_orbit_sigma_saddle_point(dual_vertices, sigma);
        self.kkt_solve_ms += started.elapsed().as_secs_f64() * 1000.0;
        match solved {
            Ok(orbit) => self.record_orbit(orbit),
            Err(F64OrbitSolveError::Inadmissible) => self.inadmissible_count += 1,
            Err(F64OrbitSolveError::NumericalFailure) => self.numerical_failure_count += 1,
        }
    }

    fn record_orbit(&mut self, orbit: F64OrbitKktData) {
        match orbit.admissibility {
            F64OrbitAdmissibility::Admissible => self.admissible.push(orbit),
            F64OrbitAdmissibility::Indeterminate => self.indeterminate.push(orbit),
        }
    }
...
pub(crate) fn solve_transition_pruned_candidates(
    ...
    for_each_transition_pruned_sigma(transition_is_allowed, |sigma| {
...
pub(crate) fn solve_billiard_candidates_summary(
    ...
    let mut summary = CandidateSolveSummary::default();
    for_each_product_billiard_sigma(
        q_facet_indices,
        p_facet_indices,
