// api-extract: extract public API from crates/library/src/ into markdown reference files.
//
// For each .rs file with pub items, writes api-reference/library/src/<path>.md.
// Also writes api-reference/library/src/lib.md as an index of all modules.
//
...
//
// Path mapping:
//   crates/library/src/<x>.rs          -> api-reference/library/src/<x>.md
//   crates/library/src/foo/mod.rs      -> api-reference/library/src/foo/mod.md
//   crates/library/src/lib.rs          -> api-reference/library/src/lib.md  (also index)
...
fn render_module_md(module_path: &str, items: &ModuleItems) -> String {
...
fn render_item(
    ...
    match item {
        ...
        ExtractedItem::Function { name, sig, doc } => {
        ...
        ExtractedItem::Const { name, rendered, doc } => {
...
fn render_method(out: &mut String, type_name: &str, m: &ExtractedMethod) {
    let _ = writeln!(out, "### {type_name}::{}", m.name);
    out.push('\n');
    let _ = writeln!(out, "```rust\n{}\n```", m.sig);
    out.push('\n');
    if !m.doc.is_empty() {
...
fn main() {
    ...
    for rs_path in &rs_files {
        ...
        match extract_file(rs_path) {
            ...
            Ok(items) => {
                ...
                // Build output path
                let out_rel = rel.with_extension("md");
                let out_path = api_ref_root.join(&out_rel);
                std::fs::create_dir_all(out_path.parent().unwrap())
                    .expect("create output dir");

                let md = render_module_md(&module_display, &items);
    ...
    // Write index
    let index_path = api_ref_root.join("lib.md");
    let index_md = render_index(&index_entries);
    std::fs::create_dir_all(api_ref_root).expect("create api_ref_root");
    std::fs::write(&index_path, &index_md).expect("write index");
...
fn render_index(entries: &[(String, Vec<String>)]) -> String {
    let mut out = String::new();
    out.push_str("<!-- auto-generated by api-extract — do not edit -->\n");
    out.push_str("# symplectic — Library API Index\n\n");
    out.push_str("Path convention: `crates/<path>.rs` → `api-reference/<path>.md`\n\n");
