...
a successful certificate summary.
...
Only finite combinatorial choices are read from \path{witness.json}: the words
\(\sigma\), the selected five-column minors, and the beta coordinates that are
kept fixed in the feasible section. Floating-point numbers in the witness are
used only as hints for choosing among exact candidates. A row is accepted only
after exact checks: closure plus normalization, strict beta positivity in the
chosen real embedding, invertibility of the selected minor, equality with the
HKO action, the differentiated feasible-section equation, the derivative
formula for \(D\operatorname{sys}\), and annihilation of every symmetry tangent
column.
...
The successful run records exactly the values used above: 26 verified rows,
ambient dimension \(40\), row rank \(25\), symmetry rank \(15\), positive
coefficients \(\lambda_\sigma\), and
