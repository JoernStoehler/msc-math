...
- Landed in this worktree as the `sys-dataset` pipeline; verified current shared-table counts are `282` states / `282` capacity rows / `287` step events once the refreshed `experiments/sys-landscape/datascience/produce/shared-cache.jsonl` includes the fixed-`F` ascent endpoints.
