...
| `feature-block-regression` feature-block regression | Do cheap feature blocks predict `sys` on endpoint rows after fitting on random rows? | tabular random and endpoint rows | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze.py`; feature JSONL/PNG artifacts in the same directory; `research/sys-landscape-datascience/idea-ledger.md` (`feature-block-regression`, provisional `stat-sanity`); `research/sys-landscape.md`; `tasks/planning-notes.md` | Grouped CV by lineage/source; random-to-endpoint prediction is the load-bearing guard; the sanity spike adds cheap permuted-label nulls but still needs repo-owned source-truth repair before thesis use. | Provisional `stat-sanity` prose records above-null within-random and within-endpoint pockets, but best random-to-endpoint prediction was still `face_symplectic` at `R^2 = -9.1481` despite beating a weak null p95 of `-14.6865`. | Supports the bounded claim that the current feature packet did not produce a candidate-proposer; within-row-source associations must not be read as endpoint prediction. The null-strength detail stays caveated until the `stat-sanity` row is repaired or downgraded. | `ran with no candidate-proposer and no new validated row` / `validity caveat` | Main evidence for the feature-packet no-candidate-proposer result; sanity/null detail is supporting only until source-truth repair. | Reopen if refreshed feature packets show positive random-to-endpoint prediction, add local-maxima-specific prediction evidence, or fail the `stat-sanity` source-truth repair. |
