...
%   Speed is free: any reparametrization is again a closed characteristic.
% Downstream: a closed curve on bdry(K) with velocity in the characteristic
%   direction at each point. Geometric object (image), not a parametrized curve.
...
whose velocity lies in the characteristic line:
$\dot{\gamma}(t) \in \ell_{\gamma(t)}$ for a.e.\ $t$.

Any reparametrization of a closed characteristic is again a closed
characteristic. The action $A(\gamma)$ is reparametrization-invariant
...
boundary is
\[
  c_{\mathrm{EHZ}}(K) \coloneqq \min\{ A(\gamma) :
  \gamma \text{ is a closed characteristic on } \partial K \}.
\]
...
%   (iii) symplectic invariance: test c(A*K) = c(K) for random A in Sp(4).
%   (iv) normalization: test c(ball) = pi for unit ball.
The EHZ capacity satisfies, for all convex bodies $K, K'$
with $0 \in \operatorname{int}(K) \cap \operatorname{int}(K')$:
\begin{enumerate}
...
\item \emph{Conformality:} $c_{\mathrm{EHZ}}(\alpha K) = \alpha^2\, c_{\mathrm{EHZ}}(K)$
  for all $\alpha > 0$.
\item \emph{Symplectic invariance:} $c_{\mathrm{EHZ}}(AK) = c_{\mathrm{EHZ}}(K)$
  for all $A \in \mathrm{Sp}(4)$.
...
An \emph{irredundant $H$-representation} of $K$ is an unordered finite set
$\{(\hat{n}_i, \hat{h}_i)\}_{i=1}^{F}$ with $\hat{n}_i \in S^3$,
$\hat{h}_i > 0$, such that
\[
...
$\{(n_i, h_i)\}_{i=1}^{F}$.
The \emph{facets} of $K$ are the $(d{-}1)$-dimensional faces
\[
  F_i \coloneqq \{ x \in K : \langle x, n_i \rangle = h_i \},
  \qquad i = 1, \ldots, F.
\]
The vectors $n_i \in S^3$ are the \emph{outward unit normals} and
$h_i = h_K(n_i) > 0$ are the \emph{heights}.
The integer $F$ is the \emph{facet count}.
\end{definition}
...
\]
It is the unique vector tangent to $\partial K$ along $\operatorname{int}(F_i)$
that lies in $\ker(\omega_0|_{n_i^\perp})$ and satisfies
...
Since $\lambda_0|_x(R_i) = 1$ for every $x \in F_i$ and every $i$, the convex
combination preserves the normalization:
$\lambda_0|_{\gamma(t)}(\dot{\gamma}(t)) = 1$ for a.e.\ $t$.
