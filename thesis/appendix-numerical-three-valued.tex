...
\begin{align*}
  ...
  \begin{cases}
    ...
    \textsc{false}
      & \text{if no vertex } v \text{ comes close to both facets,} \\
    \textsc{unknown}
      & \text{otherwise (a vertex is near the boundary of incidence).}
  \end{cases}
\end{align*}
  ...
  \begin{cases}
    \textsc{true}
      & \text{if } \|\delta\beta\| > \varepsilon / \sigma_C
        \text{ (proposition applies, safe to skip),} \\
    \textsc{false}
      & \text{if } \sigma_{\min}
...
and $\beta_i = O(\|v\|)$ (the $\beta$-components scale with
the vertex coordinates since $\beta$ is determined by the
constraint $N^\top \beta = 0$, $\eta^\top \beta = 1$).
The period $T$ of the orbit is bounded below by the inradius
(determined by $\min_k h_k$) and above by the circumradius
(determined by $\max_k \|v_k\|$).
