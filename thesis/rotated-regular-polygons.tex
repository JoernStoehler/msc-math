...
This section is a draft integration of the executable certificate. The
certificate source and transcript are committed, but the endpoint, symmetry,
continuity, and exposition bridge to the theorem statement still need final
mathematical review before this result is used as final thesis source truth.
\end{remark}
...
% The referenced HK2019 section currently does not contain the
% finite-candidate/billiard reduction used here. Source material exists in
% formal/billiard-capacity-algorithm.tex and thesis/legacy/, and the missing
% thesis-facing wording is tracked in rotated-regular-polygons-content.md.
...
% Same unsatisfied dependency as above. This paragraph is not final until the
% finite-candidate theorem is written into active thesis prose or the reference
% is replaced by an explicit local theorem statement.
...
classifies every raw representative that survives this pruning. The exact
enumeration loop is in the committed Sage source; it keeps raw ordered
representatives instead of quotienting by cyclic shifts, reversals, or
symmetries, so it overcounts rather than undercounts.
