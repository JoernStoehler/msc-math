...
| Euclidean polytopes target | currently docs-only scaffold; proposed public surface is flat functions over `Vector4<T>`, with `f64` APIs returning values plus error bounds or indeterminate cases, and exact APIs resolving indeterminate cases exactly | `crates/euclidean-polytopes/README.md`, `crates/euclidean-polytopes/DEVELOPMENT.md` |
