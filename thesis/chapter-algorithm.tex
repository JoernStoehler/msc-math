...
\item \textbf{Filter.}
  ...
  % QC: beta^T H beta <= 0 filter: the bijection between (sigma, beta) and
  %   simple Reeb orbits (Theorem thm:main proof, Step 6) requires
  %   beta^T H beta > 0 for T = (beta^T H beta)^{-1} > 0.

\item \textbf{Evaluate.}
  \begin{equation}\label{eq:action-formula}
    A(S, \sigma) = (\beta^\top H \beta)^{-1}.
  \end{equation}
\end{enumerate}
...
\begin{proof}
For $i > j$,
$H_{ij} = \omega_0(n_{\sigma(i)}, n_{\sigma(j)})$
by definition of~$H$.
...
definition, and $V^* > 0$ by Step~1.
Hence the algorithm considers~$(\sigma^*, \beta^*)$ as a
candidate with
$A = (V^*)^{-1} = c_{\mathrm{EHZ}}(K)$.

\emph{Step~3: The algorithm returns it.}
The algorithm returns
$\min A = \min (\tilde{\beta}^\top H\, \tilde{\beta})^{-1}$
over all candidates. For any candidate with
$\tilde{\beta}^\top H\, \tilde{\beta} \leq V^*$,
we have
$A = (\tilde{\beta}^\top H\, \tilde{\beta})^{-1}
\geq (V^*)^{-1}$.
Since $(\sigma^*, \beta^*)$ achieves
$A = (V^*)^{-1}$, this is the minimum.
...
Non-maximizing critical points that pass the filter
($\beta > 0$, $\tilde{\beta}^\top H\, \tilde{\beta} > 0$)
