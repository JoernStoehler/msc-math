...
BIN="$REPO_ROOT/target/release/hko-neighborhood-sampling"
if [[ ! -x "$BIN" ]]; then
    cargo build --release -p exp-hko-local-maximum --bin hko-neighborhood-sampling
fi

N_PER_BUCKET="${N_PER_BUCKET:-20}"
OUT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/hko-neighborhood-m10-smoke.XXXXXX")"
cleanup() {
