...
use exp_sys_landscape::{
    capacity_auto_with_gap, compute_active_sys_state, compute_step_bound_detailed,
    compute_sys_from_capacity, dual_vertices_rational_strings, poly_id_from_dual_vertices,
    reference::exact_volume_as_f64, EventType, SysLandscapePolytopeCache,
};
...
mod tests {
    ...
    #[test]
    fn maximin_direction_rejects_mismatched_gradient_dimensions() {
        for malformed in [vec![], vec![Vector4::repeat(1.0); 2]] {
            let gradients = vec![vec![Vector4::repeat(1.0)], malformed];
            assert!(maximin_direction(&gradients).is_none());
        }
    }
