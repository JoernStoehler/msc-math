...
**Complete (2026-03-27).** 140 polytopes, 2661 successful midpoint constructions out of 2800 tests. Split into standalone experiment.
...
| Check | Failure rate |
|-------|-------------|
| Incidence change | 0.75% |
| omega_0 sign change | 54.4% |
| Transition matrix change | 58.5% |

(convexity JSONL, midpoint_construction_ok=true rows, cell_convexity.png)

**Cells are NOT convex.** 58.5% of successful midpoints have a different transition matrix. The transition matrix determines which cycles are feasible in HK2017.

Incidence is almost always preserved (99.25%) -- incidence boundaries are approximately hyperplanes.

**Failure rate increases sharply with F:** ~0% at F=5, ~50% at F=6-7, ~67% at F>=8.

**Non-convexity is entirely a product phenomenon in the committed data.** Random polytopes: 0/1103 transition failures. Lagrangian products: 1558/1558 transition failures. Products have special omega_0 relationships between cross-factor facet pairs (near-zero values that flip easily under perturbation).
