...
| Symplectic capacity and orbit search | certified four-dimensional scalar capacity and minimizing-word APIs for bounded exact-binary64 inputs, plus legacy/custom KKT, orbit, derivative, random, and persistence workflows with narrower contracts | `crates/symplectic/src/algorithms/capacity_4d/`, `crates/symplectic/README.md`, tests |
...
| QP/KKT numerical diagnostics | exact-vs-binary64 predicate, capacity, minimizer, adversarial, ablation, and profile evidence for the selected certified general and product routes; legacy/custom wrappers remain outside that certificate | `experiments/dev-quadratic-program/README.md`, general-ablation and product-closure packets |
