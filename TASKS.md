...
- Current concrete Sage surface: `experiments/hko-local-maximum/exact-clarke/build_widened_seed_witness.py` now emits `widened-seed-witness.json`, and `experiments/hko-local-maximum/exact-clarke/verify_widened_seed_witness.sage` replays exact field/geometry/symmetry/seed-row checks on that single artifact. This is a Packet 3 witness-contract milestone, not the final theorem checker.
- Current exact Sage finding: on the present widened seed surface, the seed rows annihilate the `Sp(4)` generators but not the current translation/scaling generators; the widened seed row matrix therefore has right-kernel dimension `29`, not the target `15`. Before treating the widened seed surface as provisional theorem input, resolve whether this comes from the affine/scaling tangent encoding, the exact row formula, or a convention mismatch between those two packets.
...
- Current reduced-route obstruction: one exact endpoint seed plus one exact midpoint seed give only `20` symmetry images, so that ultra-small reduced surface cannot reach active-matrix rank `25`. Current widening state: `5` six-facet seed orbits and `6` midpoint-style seven-facet seed orbits are exactified; only `2` asymmetric seven-facet seed orbits remain unresolved on the current numerical planning surface.
...
- Dedicated-session split:
  - Session A: keep exactifying the widened active-row seed surface and converge on a correct+trusted witness contract for the large exact route.
...
- Current cache evidence:
  ...
  - `experiments/sys-landscape/variable-f-ascent/cache.jsonl` is intentionally local and much larger than the shared cache: it stores many intermediate gradient-step polytopes so the shared family cache does not become a transient search log.
- Data-shape classes:
