...
- Underspecified or implicit objectives where the executing agent is allowed to pick the interpretation drift towards the laziest interpretation over time, due to the pressure to finish the goal, which often is a failure of the implicit real objective.
- Claims without epistemic status have ambiguous interpretation with regards to how much to question them, and so mere ideas can be misinterpreted as must-have constraints, or guesses can become assumed facts that propagate into future sessions via the repo.
...
- The charter fleshes out stopping conditions that are robust to interpretation drift, which e.g. comes from a the pressure to finish quickly or from boisterous overconfidence.
