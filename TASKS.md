...
- Problem: the public library API exposes capacity-focused entry points whose result stores one certified best permutation and beta. The root scalar family is now `ehz_capacity` (auto), `ehz_capacity_pruned`, `ehz_capacity_unpruned`, and `ehz_capacity_billiard`; several experiments still need richer algorithm output: all certified candidate orbits, all minimum-action simple orbits within tolerance, near-active witnesses, pruning/solver diagnostics, and recovered primal trajectories. Today that richer output exists only as copied experiment instrumentation or module-internal traversal.
...
- Candidate design surface: one shared orbit/result layer beneath the root scalar family `ehz_capacity` (auto), `ehz_capacity_pruned`, `ehz_capacity_unpruned`, and `ehz_capacity_billiard`, with separate search frontends for general search, pruning policy, and Lagrangian-product-specific enumeration/validation.
...
- Current code state: root `ehz_capacity` is the ordinary scalar/default entrypoint and returns one best permutation; the explicit HK2017 and billiard variants remain available for verification and algorithm-comparison work. Several experiments have copied instrumented enumeration code that collects all valid or near-optimal orbits. `orbit_recovery::recover_and_verify` validates one `(sigma, beta, capacity)` result at a time, so the validation session depends on either an experiment-local adapter or the library API design task above.
