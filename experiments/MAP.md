...
- Source truth is package manifests, entrypoints, local READMEs, helper crates,
  generated artifact locations, and owner-local notes.
...
- To refresh, update navigation and provenance facts; route interpretation to
  the owner future work should update.
...
- Source surfaces: `experiments/**/Cargo.toml`, `experiments/**/src/lib.rs`,
  local `README.md` files, experiment entrypoints, and owner-local notes.
...
| `FG/closed-word/exact` | Exact rational flow-graph closed-word/tube resolution. | `crates/symplectic/src/algorithms/flow_graph/exact_tube.rs` | Supported word-level control path. |
| `FG/capacity/exact` | Exact rational flow-graph capacity search. | `crates/symplectic/src/algorithms/flow_graph/exact_search.rs` | Selected F5/F6/F7 comparisons are implementation evidence. |
...
| Area | Current role | Related surfaces |
| --- | --- | --- |
| `experiments/hko-local-maximum/` | HKO local-maximality experiments: theorem certificate tooling under `theorem/`, empirical support checks under `empirical/`, singular-row diagnostics, and shared topic helpers under `src/` | `experiments/hko-local-maximum/README.md` |
| `experiments/local-maxima-check/` | Five-case comparison of theorem, exact, conjectural, and finite-screen local behavior at selected bodies | `experiments/local-maxima-check/README.md` |
...
| `experiments/dev-sys-prediction/` | active development packet for semi-local prediction of `sys(a0 + da)`: lower-envelope branch models, direction-cloud prediction error, candidate-window completeness, beta-domain diagnostics, and a local branch-equality continuation control before optimizer or thesis-evidence promotion | `experiments/dev-sys-prediction/README.md`, `experiments/dev-sys-prediction/branch-equality-continuation/README.md`, `experiments/dev-gradient-ascent/README.md`, `formal/sys-first-order-local-behavior.md` |
...
| `experiments/dev-flow-graph/` | active flow-graph algorithm-development packet: exact frontier counts, endpoint/closed-word representation spikes, and reproducible tube visualization | `experiments/dev-flow-graph/README.md`, `crates/symplectic/src/algorithms/flow_graph/README.md` |
...
- `experiments/dev-quadratic-program/numerics-audit/src/lib.rs`
- `experiments/verification/src/lib.rs`
- `experiments/sys-landscape/src/lib.rs`
- `experiments/dev-quadratic-program/src/lib.rs`

Current observed pattern:

- `experiments/dev-gradient-ascent/`, `experiments/dev-quadratic-program/`,
  and `experiments/dev-quadratic-program/numerics-audit/` are Rust-heavy
...
Check retained thesis claim usage and the owning experiment or owner-local note
before using them as evidence.

Freshness status is not owned by this map. The latest repo-status pass is
`scripts/repo-status/repo-status-smoke-and-core-2026-05-31.md`: selected
...
```bash
rg -n "<artifact-name>|Input Artifacts:|Output Artifacts:" experiments thesis formal crates
```
