% formal/f64-near-singular-vertices.tex -- bounded interpretation of near-singular
% 4-facet vertex systems.
%
% Labels defined here:
...
%
% Status: agent-written proof note, unapproved by Jorn.  The linear algebra
% statements are elementary.  The bridge to f64 diagnostics still needs a
% separate outward-rounded residual/minimum-norm implementation before it is a
% machine-checkable certificate.

\providecommand{\R}{\mathbb{R}}
\providecommand{\one}{\mathbf{1}}
\providecommand{\dist}{\operatorname{dist}}
\providecommand{\rowspan}{\operatorname{rowspan}}
...
\]
where the rows $a_i\in\R^4$ are the dual vertices used by the f64 capacity
experiment.  For a four-subset $I=\{i_1,i_2,i_3,i_4\}$, write $A_I$ for the
$4\times4$ matrix with rows $a_{i_j}$.

The f64 experiment currently treats a small determinant of $A_I$ as a
near-singular vertex event.  The determinant alone is not enough information:
a singular system can be inconsistent, consistent only far from the origin, or
consistent with bounded points.  The relevant proposition for capacity
enumeration is whether the active equations
\[
  A_Ix=\one
\]
...
\[
  r(A)=\dist(\one,\operatorname{im}A)
...
\begin{proof}
The equality $r(A)>0$ is exactly the statement that $\one$ is not in the image
...
Let $A,E\in\R^{4\times4}$, and suppose $\|E\|\le\varepsilon$.  If there is
$x$ with
\[
...
then
\[
  r(A)=\dist(\one,\operatorname{im}A)\le \varepsilon B.
\]
...
Moreover, if the input rows are allowed to move by spectral norm at most
...
\[
  \texttt{f64-capacity-near-singular}
\]
...
solution would require row perturbations on the order of at least $10^{-4}$ to
$10^{-3}$.  The closest ascent-product examples have much smaller residual
but least-norm solution scale about $10^{12}$, so they fall outside the
$B=10^3$ bounded region.
\end{remark}
