% hko-neighborhood.tex — \input'd from experiments/experiments.tex
%
...
%   - Plot script: experiments/hko-neighborhood/hko_neighborhood.py
...
($n_k \mapsto n_{k+1\bmod 5}$, $k = 0, \ldots, 4$)
...
versa: $\varphi(n_k) = n_{k+5\bmod 10}$ for $k = 0, \ldots, 9$.
...
It maps $n_k \mapsto n_{k+1\bmod 5}$ for $q$-facets and
$n_k \mapsto n_{k+1\bmod 5}$ (within the $p$-block) for $p$-facets.

Since $\Delta_{72^\circ}$ and $\varphi$ are both symplectomorphisms
preserving~$K$,
they map Reeb orbits to Reeb orbits of equal action
(Lemma~\ref{lem:capacity-axioms}\,(iii) applied at the orbit level).
...
\item \textbf{All gradient norms are identical}
  ($\approx 1.431$, spread $< 2 \times 10^{-14}$).
