...
Every datum in $\mathcal A(h_0)$ determines, after reversing the word to the
HK2017 displayed orientation and deleting zero entries, an action-minimizing
simple generalized characteristic.  Its dwell-time fraction associated with
facet $i$ is
...
\]
Put $\rho(i)=\sigma(F+1-i)$, the reversal from the project active-word order
to the HK2017 displayed order.  Place the constant velocity proportional to
$p_{\rho(i)}$ on an interval of length $T_{\rho(i)}$.  The resulting
piecewise-affine path closes.  Since
$\mathcal Q_\sigma(\alpha)>0$, it can be rescaled to the dual-action
normalization used in the proof of the Haim--Kislev formula.  The calculation
in that proof gives its dual value
$A_{\sigma,b}(h_0)/2$.  Because the datum is active, this is the minimum dual
value.  The dual-action correspondence therefore yields an action-minimizing
generalized characteristic with these velocity intervals.
...
\eqref{eq:facet-coverage-convex-balance} and is not sufficient for local
maximality.  Even an abstract family containing a row that sees every
coordinate can have its convex hull strictly separated from zero, producing a
common increasing direction.  The balance condition itself is not sufficient
when zero lies on the boundary: first-order flat directions require higher
order control (the one-function example $f(x)=x^2$ already has zero gradient
at a local minimum).
