...
fixed-\([0,1]\) curve \(w\) above. This \(w\) is a fixed-period minimizer:
every fixed-period competitor maps by the converse rescaling to a free-period
competitor, and the two objective values differ by the constant factor \(2\).
The fixed-period Clarke theorem therefore gives the nonsmooth Euler--Lagrange
inclusion for \(w\). In Haim--Kislev's notation this is
\[
  8I_K(w)w+a\in\partial h_K^2(-J_0\dot w)
\]
for a constant \(a\in\mathbb{R}^4\). Since
\(w(s)=z(Ts)/\sqrt{2T}\), the \(2\)-homogeneity of \(h_K^2\) gives
\[
  \frac{I_K(z)}{T}z(t)+\sqrt{\frac{2}{T}}\,\frac a4
  \in
  \partial\bigl(\tfrac14h_K^2\bigr)(-J_0\dot z(t)).
\]
Thus \(z\) is weak dual critical with \(\nu=I_K(z)/T\). The equivalent
formulation follows from Lemma~\ref{lem:preliminaries-fenchel-duality}.
\end{proof}
