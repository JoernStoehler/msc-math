...
approximate.  Its mixed wrapper checks that the exact and floating-point inputs
encode the same ordered facet data.  Exact fallback then classifies the error
words recorded by the floating-point run, while directly accepted words remain
floating-point outputs.  This path is not an exact capacity certificate.

On three deterministic rational reconstructions, exact flow-graph search
agrees with the current certified Haim--Kislev/QP aggregate.
