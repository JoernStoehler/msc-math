...
- `api-reference/library/`: Auto-generated from `crates/library/` — same `.rs` files but stripped of function bodies, tests, and private symbols, and thus significantly shorter and clearer to read. Path mapping: `crates/<path>.rs` → `api-reference/<path>.rs`.
...
- **API reference**: `api-reference/` is autogenerated to help agents understand code without needing to read complex implementations, long tests, or inaccessible private items. Agents can just 1:1 read `api-reference/library/src/**.rs` instead of `crates/library/src/**.rs`, if they don't care about the function signatures.
...
# API reference (automatically runs as pre-commit hook)
