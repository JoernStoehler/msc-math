...
Older `sys-landscape` experiment directories still exist outside
`experiments/sys-datascience/`, but this directory is the maintained producer
surface for the datascience pipeline.
...
SEED_TIME_BUDGET_SECS=1 \
  experiments/sys-datascience/pipeline.local.sh cache-benchmark
```
...
```bash
python3 experiments/sys-datascience/produce/merge-licca-ascent-shards.py
python3 experiments/sys-datascience/produce/merge-licca-ascent-shards.py --write
```
