...
fn evaluate(input: Input) -> Value {
    ...
    let value = match capacity_value(&result,1e-10) { Ok(v)=>v, Err(e)=>return failure("capacity_value",e) };
    let sys = value*value/(2.*volume);
    if !sys.is_finite() { return failure("sys","NonfiniteSys"); }
    let exact = match &result { Capacity4d::Product(p)=>Some(p.capacity_exact().to_string()), Capacity4d::General(_)=>None };
    json!({"status":"ok","capacity":value,"capacity_lower":result.bounds().lower(),
        "capacity_upper":result.bounds().upper(),"capacity_exact":exact,"capacity_route":result.route_name(),
        "volume":volume,"sys":sys,"geometry_ms":geometry_ms,"volume_ms":volume_ms,"capacity_ms":capacity_ms,
        "primal_vertex_count":primal.len()})
}
fn main() {
    let t=Instant::now();
