% math_prefilter.tex — f64 pre-filter error bound for vertex enumeration
% Mathematical results supporting crates/src/geom/vertex_enumeration.rs
%
% Labels defined here:
%   prop:prefilter-bound
%
% Source: /tmp/prefilter-bound-simple.tex (Jörn's algorithm specification)
% Status: Jörn is finalizing the proof; this file will be updated when
%   the final version is forwarded. Current content is a faithful
%   transcription of the working draft.
%
% \input'd from crates/src/geom/math.tex (or crates/src/math.tex)
% — do not add \documentclass or \begin{document}

% Local macros for this section
\providecommand{\one}{\mathbf{1}}
\providecommand{\emach}{\varepsilon_{\mathrm{mach}}}

\subsection{f64 pre-filter for vertex enumeration}
...
The following are standard results from numerical linear algebra,
stated without proof except where a short derivation is included.

\paragraph{IEEE rounding.}
Rounding a real number $x$ to f64 gives $\hat{x}$
with $|\hat{x} - x| \le \emach|x|$.

\paragraph{Casting a matrix.}
If $\hat{A}$ is obtained by rounding each entry of $A$
to f64, then $|\hat{A}_{ij} - A_{ij}| \le \emach|A_{ij}|$
entrywise. For the spectral norm:
$\|\hat{A} - A\| \le \sqrt{n}\,\emach\|A\|$.

(Proof: $\|\hat{A} - A\|^2 \le \|\hat{A} - A\|_F^2
= \sum_{ij}(\hat{A}_{ij} - A_{ij})^2
\le \emach^2 \sum_{ij} A_{ij}^2
= \emach^2\|A\|_F^2 \le \emach^2 n\|A\|^2$,
using $\|M\|_F^2 = \sum_i \sigma_i(M)^2 \le n\sigma_1(M)^2
= n\|M\|^2$.)

\paragraph{SVD backward stability.}
...
\]
with $C = 10^4$ for $n = 4$
(a tighter accounting gives $C < 1400$; see Remark below).
\end{proposition}

\noindent
\textit{Why this form?}
The solve $\hat{A}^{-1}\one$ is the only step that amplifies
errors, and $\kappa$ measures the amplification.
All other error sources --- rounding entries when casting to f64,
evaluating a length-$4$ dot product --- are $O(n)$ basic
operations on $O(1)$ data, contributing $O(n\emach)$ each.
Since $\kappa \ge 1$ (because $\|B\|\|B^{-1}\| \ge \|I\| = 1$),
these are absorbed.

\medskip
\noindent
\textit{Why $\hat\kappa \approx \kappa$?}
By SVD backward stability, the computed singular values
$\hat\sigma_i$ are the exact singular values of
$\hat{A} + E_{\mathrm{svd}}$ with
$\|E_{\mathrm{svd}}\| \le c\emach\|\hat{A}\|$.
By singular value perturbation (Weyl),
...
We number each use; the remark below enumerates all six.

\medskip\noindent\textbf{Step 1: Backward error of the solve.}
By SVD-based solve backward stability, there exist
...
$|T_1| \le \|y\|\|v - \hat{v}\|$
by Cauchy--Schwarz.
By the casting bound (applied to the vector $y$),
$\|y - \hat{y}\| \le \emach\|y\|$,
...
by the dot product rounding bound,
