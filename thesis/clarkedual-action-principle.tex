...
Equality holds iff $x$ attains the supremum defining $(g_K^2)^*(y)$,
...
\begin{align*}
  ...
  &\Rightarrow \quad
  \tfrac{1}{\alpha}[\nu z(s) + b_0]
  \in \partial\bigl(\tfrac{1}{4}h_K^2\bigr)
    \bigl(-\tfrac{1}{\alpha}J_0\dot{z}(s)\bigr)
  && \text{(1-homog.\ $\partial h_K^2$)} \\
  &\Rightarrow \quad
...
For any Reeb orbit~$\gamma$, since $-J_0\dot{\gamma} \in \partial g_K^2(\gamma)$,
the Fenchel equality (Lemma~\ref{lem:fenchel-duality}) gives pointwise:
\[
