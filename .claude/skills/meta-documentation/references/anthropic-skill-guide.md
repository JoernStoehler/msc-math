...
Converted: 2026-03-09 (PDF → text via PyPDF2)

## Relevance to this project

Most of the guide targets Claude.ai/MCP skills marketplace use cases. The relevant sections for our CLAUDE.md/SKILL.md writing are:

1. **Progressive disclosure** (p.5): Three-level system — frontmatter (always loaded), SKILL.md body (loaded on demand), linked files (discovered as needed). Matches our approach.

2. **Description field is critical** (p.10-11): Must include WHAT the skill does and WHEN to use it (trigger conditions). Under 1024 characters. Include specific tasks users might say. This is how Claude decides whether to load the skill.

3. **Instructions not followed** (p.25-26): Four causes and fixes:
   - Instructions too verbose → keep concise, use bullets/numbered lists, move detail to references/
   - Instructions buried → put critical instructions at top, use ## Important headers, repeat key points
   - Ambiguous language → be specific and actionable (they give a concrete good/bad example)
   - Model "laziness" → add explicit encouragement (but note: more effective in user prompts than SKILL.md)
   - **Advanced technique**: for critical validations, use scripts instead of language instructions. "Code is deterministic; language interpretation isn't."

4. **Keep SKILL.md under 5,000 words** (p.27), move detailed docs to references/

5. **Iterate on a single task before expanding** (p.15): Most effective skill creators iterate on a single challenging task until Claude succeeds, then extract the winning approach. Matches our trial-and-error pattern.

6. **Large context issues** (p.27): Too many skills enabled, all content loaded instead of progressive disclosure. Solution: reduce enabled skills, link to references instead of inlining.

## Full extracted text

Below is the full text extracted from the PDF slides. Formatting is approximate due to slide layout conversion.
...
preferences, processes, and domain expertise in every conversation, skill s let you 
...
• How to test, iterate, and distribute your skill s 
Who this is for: 
...
• Teams looking to standardize how Claude works across their organization Two Paths Through This Guide  
...
specialized expertise.Composability
...
(Notion, Asana, Linear, etc.)T eaches Claude how to use your service 
effectively
Provides real-time data access and tool 
invocationCaptures workflows and best practices
...
• What domain knowledge or best practices should be embedded?Common skill use case categories
...
• Error handling for common MCP issuesDefine success criteria
...
human users — see Distribution and Sharing.YAML frontmatter: The most important part
...
inject instructions.Writing effective skills
...
Clear explanation of what happens.Example:
```bash
...
coverage.Recommended T esting Approach
...
- 12,000 tokens consumedWith skill:
...
Solution: Add negative triggers, be more specificExecution issues:
...
members of the ecosystem on the standard, and we're excited by early adoption.Using skills via API 
...
2. Install in Claude:
   ...
   - Click "Upload skill"   - Select the skill folder (zipped)
3. Enable the skill:
...
you choose the right pattern below.Pattern 1: Sequential workflow orchestration
...
• Centralized error handlingPattern 3: Iterative refinement
...
• Fallback options
• Transparency about choicesPattern 5: Domain-specific intelligence
...
Cause: Name has spaces or capitals# Wrong
name: My Cool Skill
# Correct  
name: my-cool-skill
...
4. V erify tool names
 ...
 –Tool names are case-sensitiveInstructions not followed
...
• Contains Anthropic-created skill s you can customizeT ools and Utilities
skill-creator skill:
...
During development
   ...
   Examples provided
   References clearly linkedBefore upload
