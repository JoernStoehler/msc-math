...
\(\operatorname{sys}\geq1\).  That negative result is not interpretable on its
...
systolic-ratio gap and removed \(77.9\%\)--\(100\%\) of the distance to HKO.
...
the target reproduced the full scalar in every retained control row.  But the
...
\(1.57\cdot10^{-6}\) to \(4.61\cdot10^{-5}\), and none crossed \(1\).  The
...
recomputed scalar even though the target winner was represented and its
affine prediction was positive; \(40\) such losses retained determinate,
unchanged geometry.  All \(40\) current-branch gradient proposals decreased.
This is outcome-selected diagnostic evidence, not a prevalence estimate.
...
branch, not a missing target word, geometry predicate, volume derivative, or
sign error in the infinitesimal formula.  Euclidean distance alone is not an
adequate trust scale; the KKT spectral gap supplies a mechanistic warning,
although this three-case audit does not calibrate a universal cutoff.
...
solely to the geometry of \(\operatorname{sys}\): ordinary insufficient depth
and identified model failures both occur in the retained runs.
