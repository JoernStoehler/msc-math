...
- Execution status on `capacity-result-api-exec`: Packet 1 scaffold landed as `library/src/algorithms/orbit_search.rs` with shared enums/types for the new result layer. Packet 2 has started: there is now a shared `solve_orbit_sigma(...)` primitive on the saddle-point path, and the current HK2017/billiard bridges route through it. The remaining Packet 2 work is the shared collector/finalization surface above that primitive.
