#!/usr/bin/env bash
# Historical run commands. Existing frozen/run outputs are immutable.
set -euo pipefail
...
flock /workspaces/msc-math/.git/codex/ds-evaluator.lock /usr/bin/time -v -o "$PACKET/artifacts/run.time" timeout 180 python3 "$PACKET/run.py"
python3 "$PACKET/analyze.py"
python3 "$PACKET/costs.py"
