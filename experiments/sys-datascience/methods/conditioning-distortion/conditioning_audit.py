...
def diagnostics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "reason_counts": {k: counts[k] for k in (*REASONS, "bounded_attempt_exhaustion")},
        "rejection_reason_proportions": {k: counts[k] / len(rows) for k in (*REASONS, "bounded_attempt_exhaustion") if k != "accepted" and rows},
