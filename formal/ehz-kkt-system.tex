...
Then the QP
\[
  \min_{\beta}\; \tfrac{1}{2}\,\beta^\top H\,\beta
  \quad\text{s.t.}\quad
  A^\top \beta = 0,\quad
  \mathbf{1}^\top \beta = 1
\]
correctly computes the EHZ symplectic action for~$(S, \sigma)$.
In particular, the factorization $a_i = h_i\, n_i$
...
complex structure.
For a closed Reeb orbit visiting facets
$\sigma(1), \ldots, \sigma(|S|)$ with dwell times
$\beta_i > 0$ summing to~$1$:

\emph{Closure.}
The orbit closes iff
$\sum_i \beta_i\, R_{\sigma(i)} = 0$.
