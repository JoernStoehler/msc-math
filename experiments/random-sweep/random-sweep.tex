...
%   - Plot script: experiments/random-sweep/random_sweep.py
%   - Experiment notes: experiments/random-sweep/README.md
%
% Structure:
%   - Sampling setup
...
All 75 sampled polytopes satisfy~$\mathrm{sys} < 0.58$, well below the
Viterbo threshold~$\mathrm{sys} = 1$.
The median systolic ratio fluctuates between~$0.08$ ($F = 5$)
and~$0.48$ ($F = 12$) without a clear upward trend, and the
within-$F$ variance is large: at each facet count, the spread from
minimum to maximum is comparable to the median itself.
This suggests that polytope geometry (normal directions, height ratios)
matters more than facet count alone for determining~$\mathrm{sys}$.

For comparison, the only known counterexample ($\mathrm{sys} \approx
1.047$, Haim-Kislev \& Ostrover 2024) is a Lagrangian product of two
