...
| Polar vertex enumeration performance | `[implemented first optimization]` | mainline thesis | agents, Jörn only for further API/contract changes | Branch `tracing-perf-integration` added opt-in tracing, then promoted the winning spike: BigRational integer-scaled determinant/gap enumeration plus named trusted-origin APIs for already validated/generated inputs. Random F=8 pruned HK2017 profile improved from exact geometry avg `40.407ms` and total avg `42.968ms` to exact geometry avg `3.262ms` and total avg `5.954ms` over 20 deterministic samples, with deterministic outputs unchanged. Remaining follow-ups are only if larger-F or heterogeneous-denominator profiles show new bottlenecks. | `/tmp/msc-math-tracing-perf-2026-05-11/spike-comparison.md`, `crates/symplectic/src/bin/profile_pruned_hk2017.rs`, `crates/euclidean-polytopes/src/polar.rs` |
