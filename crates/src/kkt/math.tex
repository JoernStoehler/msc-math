...
\[
  \text{(I)}\ H\beta + N\mu + \eta\xi = 0, \qquad
  \text{(II)}\ N^\top \beta = 0, \qquad
  \text{(III)}\ \eta^\top \beta = 1.
\]
For any solution, left-multiply~(I) by~$\beta^\top$:
\[
  \beta^\top H\beta
  + \underbrace{(N^\top\beta)^\top}_{=\,0\text{ by (II)}}\mu
  + \underbrace{(\eta^\top\beta)}_{=\,1\text{ by (III)}}\xi
  = 0,
\]
...
\[
  \text{(I$'$)}\ H\,\delta\beta + N\,\delta\mu + \eta\,\delta\xi = 0, \qquad
  \text{(II$'$)}\ N^\top\delta\beta = 0, \qquad
  \text{(III$'$)}\ \eta^\top\delta\beta = 0.
\]
Left-multiply~(I) by~$\delta\beta^\top$:
\[
  \delta\beta^\top H\beta
  + \underbrace{(N^\top\delta\beta)^\top}_{=\,0\text{ by (II$'$)}}\mu
  + \underbrace{(\eta^\top\delta\beta)}_{=\,0\text{ by (III$'$)}}\xi
  = 0.
\]
Left-multiply~(I$'$) by~$\delta\beta^\top$:
\[
  \delta\beta^\top H\,\delta\beta
  + \underbrace{(N^\top\delta\beta)^\top}_{0}\delta\mu
  + \underbrace{(\eta^\top\delta\beta)}_{0}\delta\xi
  = 0.
\]
Therefore
\[
...
% Source: scratch-near-null-analysis.tex from kkt-lp-refactor branch
% (empirical analysis, not yet Jörn-verified).
% Code: saddle_point_solver.rs, beta_feasibility.rs
%
% [TODO: JÖRN - verify the O(|lambda|) argument for Type C impossibility:
%  eigenvalue equations give N^T v_beta = lambda * v_mu, eta^T v_beta = lambda * v_xi,
%  so constraint violation is O(|lambda|) = O(tau) for discarded eigenvectors.
%  Empirically confirmed: 0 Type C across ~4300 perms on 6 polytopes.]
...
See \texttt{beta\_feasibility::find\_max\_margin} for the implementation.
...
\item \textbf{Type~C} ($\|(\hat v_j)_\beta\| = O(1)$,
  ...
  Would break constraints.
  \emph{Never observed} across ${\approx}\,4300$~permutations
  on 6~polytopes.
\end{itemize}

\medskip\noindent\textbf{Known issue: LP cycling.}
The \texttt{microlp} simplex solver can cycle (infinite loop)
on degenerate instances of this LP
(observed for $k = 2$, $m = 5$ on the square$\times$square
product at $\theta = 0$).
The implementation guards against this with a 100\,ms timeout;
on timeout, the LP result is discarded and $\hat\beta_0$
is used directly.
\end{remark}
