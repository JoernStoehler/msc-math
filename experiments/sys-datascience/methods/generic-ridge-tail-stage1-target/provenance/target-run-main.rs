...
fn evaluate(args: &Args) {
    ...
    let u1 = usage();
    let manifest = EvaluationManifest { schema: "sys-datascience.generic-ridge-tail-stage1-target.evaluation.v1", status: "evaluated-frozen-200-target-free-panel", row_count: rows.len(), selected_count: selected, baseline_count: baseline, threshold: THRESHOLD, volume_definition: "euclidean_polytopes::volume_from_incidence_f64 on exact-derived incidence", capacity_route: "exp_sys_landscape::capacity_auto (billiard when Lagrangian-product classification succeeds; otherwise pruned HK2017)", preflight_sha256: sha256(&preflight_path), evaluator_source_sha256: sha256(Path::new(env!("CARGO_MANIFEST_DIR")).join("src/main.rs").as_path()), target_free_full_validation_sha256: FROZEN_VALIDATION_SHA256.to_string(), target_calls_for_new_population: true, rows_sha256: blake3_file(&output), wall_ms: started.elapsed().as_secs_f64()*1000.0, process_user_cpu_seconds: u1.user-u0.user, process_system_cpu_seconds: u1.system-u0.system, max_rss_kib: u1.rss };
