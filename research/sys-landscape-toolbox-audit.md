...
| Method | Question | Search surface | Data / artifacts | Validity guard | Observation | Inference | Rating | Thesis use | Reopen condition |
|--------|----------|----------------|------------------|----------------|-------------|-----------|--------|------------|------------------|
| `random-generic-sampling` random generic sampling | Does blind generic sampling find a new `sys > 1` example? | random generic polytopes | `experiments/sys-landscape/random-sample/`; `research/sys-landscape.md` | Exact artifacts are treated as producer-owned; no density guard. | Current note records 70 rows, max `sys=0.739`, no `sys>1`. | Supports only a bounded negative baseline for the sampled generic regime. | `attempted, negative vs random` | Main hostile-landscape evidence with the explicit seed-count caveat. | Reopen if larger committed samples or changed generator semantics land. |
...
| `visual-exploration` visual exploration | Did projection and picture inspection reveal a useful pattern? | exploratory geometry and dynamics views | `research/visualization.md`; `tasks/planning-notes.md` | Human visual inspection is exploratory and non-exhaustive. | Ledger records visual exploration as a negative exploratory result. | "Looking at pictures" produced no current search rule, but it is still research evidence about failed pattern discovery. | `attempted, negative vs random` / `supporting evidence only` | Supporting or standalone negative-exploration material, pending writing choice. | Reopen if figures become thesis-selected evidence or a visual pattern becomes an implemented heuristic. |
| `feature-block-regression` feature-block regression | Do cheap feature blocks predict `sys` well enough to transfer from random samples to endpoints? | tabular random and endpoint regimes | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze.py`; feature JSONL/PNG artifacts in the same directory; `research/sys-landscape-datascience/idea-ledger.md` (`feature-block-regression`, provisional `stat-sanity`); `research/sys-landscape.md`; `tasks/planning-notes.md` | Grouped CV by lineage/source; transfer between random and endpoint regimes is the load-bearing guard; the sanity spike adds cheap permuted-label nulls but still needs repo-owned source-truth repair before thesis use. | Provisional `stat-sanity` prose records above-null within-random and within-endpoint pockets, but best random-to-endpoint transfer was still `face_symplectic` at `R^2 = -9.1481` despite beating a weak null p95 of `-14.6865`. | Supports the bounded claim that the current feature packet did not produce a reusable global-search heuristic; within-regime signal must not be read as transfer. The null-strength detail stays caveated until the `stat-sanity` row is repaired or downgraded. | `attempted, negative vs random` / `validity caveat` | Main evidence for the feature-packet negative transfer result; sanity/null detail is supporting only until source-truth repair. | Reopen if refreshed feature packets show positive random-to-endpoint transfer, add local-maxima-specific transfer evidence, or fail the `stat-sanity` source-truth repair. |
| `regime-classification` regime classification | Can standard classifiers separate endpoint from random regimes, and is the separator geometric rather than provenance-heavy? | grouped tabular regime-separation task | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze_regime_classification.py`; `regime-classification-report.md`; `regime_classification_summary.md`; `regime_classification_bars.png`; `research/sys-landscape-datascience/idea-ledger.md` (`regime-classification`) | Grouped split; metadata/provenance fields are an explicit validity caveat; classification alone is not a search heuristic. | Current report found perfect provenance-metadata separation and above-null non-provenance separation. Best logistic geometry/orbit block was `skeleton` (`balanced_accuracy ~= 0.8728`, `ROC AUC ~= 0.9223`); best random-forest geometry/orbit block was `face_geometry` (`balanced_accuracy ~= 0.9275`, `ROC AUC ~= 0.9831`). | Current endpoint and random rows are distinguishable in the feature table, but the result is not a label-free generator or search rule for new `sys > 1` rows. | `attempted, negative vs random` / `validity caveat` | Supporting/caveat only. | Reopen if a larger/refreshed table changes the row guards, if Jörn wants permutation/bootstrap intervals for a thesis caveat, or if someone proposes a label-free sampling rule derived from the classifier. |
| `endpoint-residualized-regression` endpoint residualized regression | Do endpoint feature blocks add signal beyond metadata? | endpoint-only grouped tabular packet | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze_residual.py`; `feature_pattern_search_residual.png`; `research/sys-landscape-datascience/endpoint-residualized-regression-disposition-2026-05-31.md` | Intended guard is grouped endpoint CV after metadata-first residualization, but the current analyzer does not enforce endpoint-only loading and has no durable markdown report. | Review found that the committed analyzer calls `load_joined_rows(dataset_dir)` instead of `load_joined_rows(dataset_dir, endpoint_only=True)`, and `write_summary(...)` is not used to produce a committed report. | Current artifacts are not thesis-bearing. Repair narrowly before thesis use, or cut/future the row by Jörn decision. A scratch check suggested endpoint-side signal but no label-free search rule or `sys > 1` row. | `inconclusive: implementation bug` / `validity caveat` | Redo-before-thesis or cut/future; do not cite as main evidence now. | Reopen if the analyzer is fixed, a durable report is committed, and the ledgers record a terminal thesis-use verdict. |
...
| SVM family | Would support-vector classifiers/regressors change the supervised feature-table verdicts? | deferred supervised alternatives on feature tables | No committed SVM method packet found in the active ledger. | Would need the same grouped CV and random-to-endpoint transfer tests as `feature-block-regression` and `regime-classification`. | SVMs are not attempted in the current before-submission evidence set. | Their absence is a coverage caveat, not evidence for or against the negative transfer observation. | `skipped: expensive to implement` | Explicit omitted-family caveat only; do not cite as failed. | Reopen if a thesis reader expects SVM baselines or if current analyzers are extended cheaply. |
| Boosting / nearest-neighbor supervised alternatives | Would boosting or nearest-neighbor models change the regression/classification verdicts? | supervised alternatives on feature tables | `experiments/sys-landscape/datascience/methods/supervised-alternatives-spike/`; `research/sys-landscape-datascience/idea-ledger.md` (`supervised-alternatives`) | Same grouped-CV and random-to-endpoint transfer framing as the supervised feature-packet checks. | The committed supervised-alternatives spike included histogram gradient boosting, extra trees, and kNN; the best claim-bearing random-to-endpoint transfer result remained negative. | Cheap supervised alternatives do not change the current search-usefulness story. | `attempted, negative vs random` / `validity caveat` | Supporting/caveat only. | Reopen if refreshed feature tables show positive random-to-endpoint transfer or if an omitted model family becomes thesis-critical. |
