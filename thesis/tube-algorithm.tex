...
we define the \emph{direction} of $F_{ij}$:
write $F_i \to F_j$ if $\omega_0(n_i, n_j) > 0$,
and $F_j \to F_i$ if $\omega_0(n_i, n_j) < 0$.
By \cite{CH2021}, Lemma~{lem:EinEout},
a Reeb trajectory with velocity $R_i$
...
Let $T(\sigma(1), \ldots, \sigma(k))$ have data
$(\operatorname{Start}, \operatorname{End}, \varphi, a, \rho)$,
and let $\sigma(k+1)$ satisfy $F_{\sigma(k)} \to F_{\sigma(k+1)}$.
Write $\Phi \coloneqq \Phi_{\sigma(k-1),\sigma(k),\sigma(k+1)}$ for the step map
(Definition~\ref{def:step-data}).
...
Each tube extension strictly increases the rotation number:
if $T(\sigma(1), \ldots, \sigma(k))$ has rotation $\rho$,
then its extension to $T(\sigma(1), \ldots, \sigma(k+1))$ has rotation
\[
...
\item \textbf{Extend.}
  For each $l$ with $F_{\sigma(k)} \to F_l$
  and $l \notin \{\sigma(1), \ldots, \sigma(k)\}$
  (Lemma~\ref{lem:prune-simple}):
  compute the extended tube $T(\sigma(1), \ldots, \sigma(k), l)$
  (Definition~\ref{def:tube-extension})
  and recurse.
\end{enumerate}
