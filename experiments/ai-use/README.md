...
| `reports/project-efficiency-monthly-2026-07-12.md` | Reusable one-point-per-month resource table and durable qualitative value ledger. |
...
| `scripts/analyze_project_efficiency.py` | Joins selected `main` snapshots, token-analysis outputs, raw rollout interaction proxies, and completion-ledger state; does not assign scalar value. |
| `scripts/summarize_project_efficiency_monthly.py` | Aggregates monthly token/cost/model/lineage resources and month-end Git snapshots; leaves semantic value assessment to the report ledger. |
...
The project-efficiency pilot consumes those CSV outputs and adds a separate
Git/session-log packet. Its visible message counts and first-to-last log spans
are proxies only; they are not Jörn-hour or LICCA-accounting measurements.
