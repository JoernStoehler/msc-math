...
//! Output Artifacts: experiments/hko-local-maximum/empirical/neighborhood-sampling/m11/hko-neighborhood-splitting.jsonl
//!
...
//!
//! Architecture:
//! 1. `cargo run --bin hko-neighborhood-sampling --release -- m11 --smoke` runs a small
//!    smoke check and writes `hko-neighborhood-splitting-smoke.jsonl`.
//! 2. `cargo run --bin hko-neighborhood-sampling --release -- m11` generates the full
...
fn print_usage() {
    eprintln!(
        r#"Usage: hko-neighborhood-sampling m11 [options]
...
fn run_phase_b(base_dir: &std::path::Path, smoke: bool) {
    ...
    let splitting_path = if smoke {
        base_dir.join("hko-neighborhood-splitting-smoke.jsonl")
    } else {
        base_dir.join("hko-neighborhood-splitting.jsonl")
    };
    std::fs::create_dir_all(base_dir)
        .unwrap_or_else(|err| panic!("create m11 output dir {}: {err}", base_dir.display()));
...
pub fn run(raw_args: &[String]) {
    ...
    let base_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("empirical/neighborhood-sampling/m11");
