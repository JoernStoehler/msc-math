...
| `pca-projection` | PCA projection. Does a low-dimensional linear projection expose a reproducible rule for proposing high-`sys` candidates? | Current retained-dataset run completed locally. PCA axes were descriptive but did not produce a valid candidate-proposer or validated new row. | Current evidence for terminal state `ran with no candidate-proposer and no new validated row`. | `pca-projection/analyze.py` | `pca-projection/report.md` | Guard: PCA inputs exclude `sys`, capacity columns, endpoint labels, dataset identity, optimizer provenance, and post-hoc `sys` inspection. Thesis use: negative PCA projection row for the closed method table. |
