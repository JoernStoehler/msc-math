...
///
/// A 2D convex polygon is represented as outward unit normals (n̂ₖ ∈ S¹) and
/// positive heights (hₖ > 0): P = { x ∈ ℝ² | n̂ₖ · x ≤ hₖ for all k }.
///
/// **Angle convention**: Regular n-gon normals at angles π/2 + 2πk/n,
...
///
/// Outward unit normals at angles π/2 + 2πk/n for k = 0, ..., n-1.
/// Heights = R · cos(π/n) (the inradius).
///
/// # Panics
...
pub fn regular_polygon_2d(n: usize, circumradius: f64) -> (Vec<Vector2<f64>>, Vec<f64>) {
    ...
    assert!(circumradius > 0.0, "circumradius must be positive, got {circumradius}");
...
/// Rotate all normals of a 2D polygon by angle θ (radians, counterclockwise).
///
...
///
/// Generates n uniformly random normal directions on S¹ (sorted by angle),
...
pub fn random_polygon_2d<R: rand::Rng>(
    ...
    // Sample n angles uniformly in [0, 2π), sort them
...
/// intersections of adjacent halfplanes, then apply the shoelace formula.
///
/// Returns None if the polygon is degenerate (fewer than 3 vertices).
