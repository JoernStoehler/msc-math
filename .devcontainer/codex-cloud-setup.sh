#!/usr/bin/env bash
# codex-cloud-setup.sh — configure the Codex web environment for msc-math.
# Context: The default Codex web environment already provides Rust, uv, git-lfs,
...
This Codex web environment does not currently permit the repo-specific qhull
...
Next step:
- use a Codex web environment mode that already includes qconvex, or
...
(`crates/symplectic/src/geom/qhull.rs`). This environment is therefore not ready
