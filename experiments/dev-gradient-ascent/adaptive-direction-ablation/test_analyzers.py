...
def test_duplicate_summary_trajectory_is_rejected():
    with tempfile.TemporaryDirectory(prefix="adaptive-direction-summary-duplicate-") as tmp:
        packet = Path(tmp)
        shutil.copytree(HERE / "artifacts", packet / "artifacts")
        shutil.copytree(HERE / "inputs", packet / "inputs")
        summary_path = packet / "artifacts" / "summary.json"
        summary = json.loads(summary_path.read_text())
        summary["trajectories"].append(summary["trajectories"][0])
        summary_path.write_text(json.dumps(summary) + "\n")
        result = subprocess.run(
            ["python3", str(HERE / "analyze.py"), str(packet / "artifacts")],
            cwd=REPO,
...
if __name__ == "__main__":
    ...
    test_duplicate_summary_trajectory_is_rejected()
    print("summary mutation and duplicate trajectory rejected")
