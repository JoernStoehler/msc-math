...
using central finite differences with step size $\varepsilon = 10^{-7}$:
\[
  ...
  \approx \frac{c_{\mathrm{EHZ}}(h_k + \varepsilon) - c_{\mathrm{EHZ}}(h_k - \varepsilon)}{2\varepsilon},
\]
where $c_{\mathrm{EHZ}}(h_k \pm \varepsilon)$ is computed by re-solving
the KKT system for the top orbits from an instrumented HK2017 run (which
stores all valid orbits, not just the best).
...
Of the 140 starting polytopes, 128 (91\%) show an improved $\mathrm{sys}$
