...
prep_python() {
  # Only run uv sync if experiments has a pyproject.toml (may not exist yet).
  if [[ -f experiments/pyproject.toml ]]; then
    run_in_dir experiments uv sync --locked --extra dev
  else
    log "skipping uv sync (no experiments/pyproject.toml)"
  fi
}
