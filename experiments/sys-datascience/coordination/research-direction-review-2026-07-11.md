...
| O-PRODUCT-BOUNCE-PATTERN | Three-bounce winners are shifted toward higher `sys` across all 10 retained product buckets; two-bounce winners are overrepresented in every lower tail. | Reviewed full-table descriptive association. The earlier apparent `4`-versus-`1` cyclic-class contrast is forced `2^2` pair-order overcount, not four capacity-value branches. |
...
| H-TWO-BOUNCE-SHORTCUT | Low-action two-bounce witnesses often depress random-product capacity; high-tail rows more often have the three-bounce family win after the shortcut is lifted. | Strong retained-slice descriptive support, but `product_bounces` is derived from the same capacity optimization as `sys`. HKO has tied two-/three-bounce minimizers, so no universal three-bounce necessity or two-bounce `sys <= 1` claim is allowed. |
