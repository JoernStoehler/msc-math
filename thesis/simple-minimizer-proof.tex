...
Let $(z_n)$ be a sequence of closed, centered, simple piecewise-Reeb
curves with periods $T_n \to T > 0$.
Then a subsequence converges to a closed, centered, simple
piecewise-Reeb curve~$z_*$ with $A(z_*) = \lim A(z_n)$
and $I_K(z_*) = T$.
\end{lemma}
...
\item \emph{Rescale}
  ...
  $A(z_4) = \beta^2 A(z_3) = T^2 / A(z_3)$ and
  $I_K(z_4) = \beta\, T = T^2 / A(z_3)$.
\end{enumerate}
...
\emph{Compactness.}
By Lemma~\ref{lem:compactness}, a subsequence of the~$z_4$
converges to a closed, centered, simple piecewise-Reeb
curve~$z_*$ with $A(z_*) = T$ and $I_K(z_*) = T$.
...
Each $z_n$ is encoded by a permutation
$\sigma_n \in S_F$ and durations
$(t_1, \ldots, t_F) \in \mathbb{R}_{\geq 0}^F$
with $\sum t_i = T_n$.
Normalizing: $(t_i / T_n) \in \Delta^{F-1}$
(the standard simplex).
Since $S_F \times \Delta^{F-1}$ is compact,
a subsequence gives
a constant $\sigma$ (by pigeonhole) and
convergent normalized durations
$s_i \to s_i^\infty \in \Delta^{F-1}$.
Since $T_n \to T$, the durations
$t_i \to t_i^\infty = s_i^\infty T$.
