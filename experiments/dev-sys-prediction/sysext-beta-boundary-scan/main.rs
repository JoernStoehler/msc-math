...
fn parse_args() -> Cli {
    let mut cli = Cli {
        ...
        out: std::env::temp_dir().join("dev-sysext-beta-boundary-scan.jsonl"),
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                eprintln!(
                    "Usage: dev-sysext-beta-boundary-scan [--polytope-table PATH] [--out PATH] [--capacity-source SOURCE] [--facet-counts CSV] [--max-rows N]"
                );
                std::process::exit(0);
            }
