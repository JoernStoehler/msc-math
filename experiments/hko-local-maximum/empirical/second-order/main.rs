...
//! Output Artifacts: experiments/hko-local-maximum/empirical/second-order/second-order-base.jsonl
//!         experiments/hko-local-maximum/empirical/second-order/second-order-curves.jsonl
//!         experiments/hko-local-maximum/empirical/second-order/second-order-random.jsonl
//!
...
//!
//! Replaces the deleted broken Phase C LP route with clean a_i-space
//! computation. The old script read normals/heights fields that
...
fn main() {
    ...
    let out_dir = base_dir.join("empirical/second-order");
    let args = parse_args();
