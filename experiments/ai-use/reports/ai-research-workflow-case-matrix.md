...
| Projection-sign replay | Repair the known reduced-gradient sign defect from frozen base `f3d36cc9`, without receiving the historical fixture or answer. | All four agents repaired the production sign. Three produced discriminating regressions; the minimal GPT-5.6 run strengthened a symmetric fixture that remained sign-insensitive. | Independent mutation restored the bad sign. It failed the two verifier-first regressions and the GPT-5.5 minimal regression, but still passed the GPT-5.6 minimal test. | A plausible passing test was not evidence for the targeted repair when it could not detect the mutation. | The benchmark design required fail-before/pass-after evidence in verifier-first conditions. | Historical repair commits `94fee3e8` and `e56cf161`; replay threads and exact outcome hashes are recorded in the companion replay packet. One bug and four runs do not rank model generations or estimate a causal effect size. |
