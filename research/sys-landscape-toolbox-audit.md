...
| `feature-block-regression` feature-block regression | Do cheap feature blocks predict `sys` on endpoint rows after fitting on random rows? | tabular random and endpoint rows | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze.py`; feature JSONL/PNG artifacts in the same directory; `research/sys-landscape-datascience/idea-ledger.md` (`feature-block-regression`, downgraded `stat-sanity`); `research/sys-landscape.md`; `tasks/planning-notes.md` | Grouped CV by lineage/source; random-to-endpoint prediction is the load-bearing guard. The `stat-sanity` scratch check is not repo-owned source truth and is not needed for the main verdict. | Existing feature-block reports are pre-LICCA source material and should not be read as current `8445`-row evidence. | No current LICCA verdict yet for this row. | `not run within the stated compute/data bound` | Redo before thesis use as main data-science evidence, or record a terminal abandoned/not-applicable state. | Reopen by running or reclassifying this row on the current retained dataset. |
| `regime-classification` endpoint-vs-random classification | Can standard classifiers separate endpoint rows from random rows, and is the separator geometric rather than provenance-heavy? | grouped tabular endpoint-vs-random task | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze_regime_classification.py`; `regime-classification-report.md`; `regime_classification_summary.md`; `regime_classification_bars.png`; `research/sys-landscape-datascience/idea-ledger.md` (`regime-classification`) | Grouped split; metadata/provenance fields are an explicit validity caveat; classification alone is not a candidate-proposer. | Existing classification reports are pre-LICCA source material and should not be read as current `8445`-row evidence. | No current LICCA verdict yet for this supporting/caveat row. | `not run within the stated compute/data bound` | Redo before thesis use as an endpoint-vs-random caveat, or record a terminal abandoned/not-applicable state. | Reopen by running or reclassifying this row on the current retained dataset. |
| `endpoint-residualized-regression` endpoint residualized regression | Do endpoint feature blocks add endpoint-only grouped-CV association beyond metadata? | endpoint-only grouped tabular packet | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze_residual.py`; `feature_pattern_search_residual_summary.md`; `feature_pattern_search_residual.png`; `research/sys-landscape-datascience/endpoint-residualized-regression-disposition-2026-05-31.md` | Grouped endpoint CV after metadata-first residualization; endpoint-only loading was enforced in the pre-LICCA repair. | Existing residualized reports are pre-LICCA source material and should not be read as current `8445`-row evidence. | No current LICCA verdict yet for this supporting/caveat row. | `not run within the stated compute/data bound` | Redo before thesis use as an endpoint-side association caveat, or record a terminal abandoned/not-applicable state. | Reopen by running or reclassifying this row on the current retained dataset. |
| PCA / global dimensionality reduction | Would a global low-dimensional embedding become a candidate-proposer? | bounded feature-table spike | `research/sys-landscape-datascience/idea-ledger.md` (`pca-cluster-anomaly`) | The spike excluded metadata/provenance fields and required a rule specified before inspecting `sys`, not only correlation with `sys`. | Existing PCA reports are pre-LICCA source material and should not be read as current `8445`-row evidence. | No current LICCA verdict yet for this toolbox row. | `not run within the stated compute/data bound` | Redo before thesis use as PCA coverage, or record a terminal abandoned/not-applicable state. | Reopen by running or reclassifying this row on the current retained dataset. |
| Clustering / manifold learning | Would unsupervised grouping of landscape rows define a candidate-proposer? | bounded feature-table spike | `research/sys-landscape-datascience/idea-ledger.md` (`pca-cluster-anomaly`) | A valid result needs a cluster-to-search rule rather than rediscovery of producer families. | Existing clustering/anomaly reports are pre-LICCA source material and should not be read as current `8445`-row evidence. | No current LICCA verdict yet for this toolbox row. | `not run within the stated compute/data bound` | Redo before thesis use as clustering/manifold coverage, or record a terminal abandoned/not-applicable state. | Reopen by running or reclassifying this row on the current retained dataset. |
...
| Boosting / nearest-neighbor supervised alternatives | Would boosting or nearest-neighbor models change the regression/classification verdicts? | supervised alternatives on feature tables | `experiments/sys-landscape/datascience/methods/supervised-alternatives-spike/`; `research/sys-landscape-datascience/idea-ledger.md` (`supervised-alternatives`) | Same grouped-CV and random-to-endpoint prediction framing as the supervised feature-packet checks. | Existing supervised-alternatives reports are pre-LICCA source material and should not be read as current `8445`-row evidence. | No current LICCA verdict yet for this supporting/caveat row. | `not run within the stated compute/data bound` | Redo before thesis use as supervised-alternatives coverage, or record a terminal abandoned/not-applicable state. | Reopen by running or reclassifying this row on the current retained dataset. |
