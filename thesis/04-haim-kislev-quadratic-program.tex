...
The preceding block family preserves geometric information needed by later
exact branch comparisons.  The scalar capacity admits a stronger reduction.
Let \(I_q\) and \(I_p\) be the \(q\)- and \(p\)-positions in a feasible word,
and put
\[
...
\]
Positive objective value forces \(0<t<1\).  Closure splits between the two
factors, so \(\alpha\) and \(\gamma\) belong to the normalized closure
polytopes
\begin{align*}
  C_q&=\left\{\alpha\geq0:\sum\alpha_k=1,\
    \sum\alpha_k u_k=0\right\},\\
  C_p&=\left\{\gamma\geq0:\sum\gamma_k=1,\
    \sum\gamma_k v_k=0\right\},
\end{align*}
where \(u_k\) and \(v_k\) are the planar dual vertices selected by the word.
Because the symplectic form vanishes on each Lagrangian factor, the objective
factors as
\begin{equation}\label{eq:quadratic-program-product-factorization}
  Q_\sigma(\beta)=t(1-t)B_\sigma(\alpha,\gamma),
\end{equation}
where \(B_\sigma\) is bilinear.  Its coefficients are the signed pairings
\(\langle u_k,v_\ell\rangle\), with the sign determined by their order in
\(\sigma\).
...
\begin{proof}
Choose a global maximizer.  Its objective is positive, hence both factor
weights are nonzero and \eqref{eq:quadratic-program-product-factorization}
applies.  With \(\gamma\) fixed, the bilinear factor is linear in \(\alpha\),
so it has a maximizer at a vertex of \(C_q\).  Replacing \(\alpha\) by that
vertex preserves global maximality.  Fixing the new \(\alpha\) and repeating
the argument in \(C_p\) gives a global maximizer for which both normalized
factor weights are vertices of their closure polytopes.
...
Theorem~\ref{thm:quadratic-program-product-six-facet} is an existence and
capacity-value result.  It neither classifies all minimizing orbits nor says
that every maximizer has support at most six.  In particular, it does not
replace the twelve-facet block family when a later argument must compare a
specified family of geometric branches.
