...
thesis uses this fixed problem as an algorithmic interface.
...
not prescribe a single method for using it in later finite checks. In this
thesis, later arguments separate three choices: which words to visit, how to
solve one fixed active word, and what kind of evidence supports the output.
...
the whole word has a valid base point. The one active-word solve and, when
needed, the base-point recovery check from
...
Exploratory finite checks may use numerical solves with explicit rejection or
indeterminacy handling.

The theorem-bearing finite certificates do not rely on unchecked floating-point
answers. Selected branches are reconstructed with exact rational or algebraic
arithmetic, and the HKO and rotated-pentagon certificates are verified in
SageMath. The numerical behavior of the floating-point route is discussed in
Section~\ref{sec:numerics}; the proof-bearing SageMath role is recorded in
Appendix~\ref{app:sagemath-computations}.
