...
section first identifies the variables with dwell times of a simple dual curve
and then separates fixed-word stationarity from the global maximum which
computes the capacity.  For Lagrangian products of planar polygons, a billiard
argument gives a much smaller exhaustive word family; that specialization is
needed in Section~\ref{sec:rotated-regular-polygons}.  We call the resulting
finite optimization problem the quadratic program, or QP.
...
that already contains it.  This cannot increase the number of blocks of either
factor.  If the source block is a singleton, deleting it coalesces its two
neighboring opposite-type blocks and reduces both block counts by one.
...
Normalize its positive dwell times by their sum. The resulting finite weights
have QP value \(1/(2T)=Q_{\max}\), so
Proposition~\ref{prop:quadratic-program-global-maximizer-realization} translates
\(z'\) to a simple generalized Reeb orbit with the same active word. A simple
orbit cannot contain three consecutive facets of the same type. Indeed, along
...
This is impossible. Thus both types occur. Likewise, one \(q\)-block and one
\(p\)-block cannot close, by the same argument in each factor. Hence the common
number of alternating blocks is not one.
It is at most three by the construction, so it lies in \(\{2,3\}\). This proves
that the restricted family contains a capacity maximizer. It does not exclude
other minimum-action orbits with more blocks.
\end{proof}
...
The reusable Rust path has a narrower evidential role.  It enumerates unpruned
words, or cycles in a caller-supplied transition graph, and applies an f64
saddle-point solver to produce KKT candidates.  Selected candidates can be
resolved exactly over rational dual rows, but the current f64 filtering and
selective fallback do not certify that every globally relevant word was
resolved.  An exact returned payload is therefore an exact feasible KKT
witness for its word, not by itself an exact capacity computation.  The
theorem-bearing HKO and rotated-pentagon arguments use their separate exact
SageMath verification packets rather than this reusable f64 route.
