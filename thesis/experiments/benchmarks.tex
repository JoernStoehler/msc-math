...
\begin{enumerate}
  ...
  \item \textbf{LU fast path (evaluated but unhelpful).} We implemented an
    LU fast path: try full-pivoting LU before SVD, falling through on
    failure. Phase profiling showed this \emph{increases} total time by
    6--12\%, because most KKT systems yield $\beta \le 0$ even when LU
    reports the system as invertible: LU decomposes and solves wastefully,
    then SVD runs anyway. On the HK-O pentagon, LU reports 92\% of
    systems invertible but only 3\% produce valid $\beta > 0$. On random
    polytopes ($F = 7$--$8$), the success rate rises to~${\sim}9\%$ but
    the overhead persists. The production solver retains the LU fast
    path (falling through to SVD on failure). At $F = 7$, LU finds 26
    additional valid orbits (out of 23{,}650) that SVD rejects due to
    gap-based rank over-truncation, but none affect the computed capacity
    (all are non-optimal).
\end{enumerate}
