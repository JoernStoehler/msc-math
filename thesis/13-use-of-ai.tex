% Publication status (2026-07-13): provisional and not accepted by Jörn as
% final prose. See use-of-ai-content.md for known missing analyses and review.
...
In retrospect, an important benefit was a change in the order of work.  An
agent could implement from reference code or a sufficiently explicit contract
while I was still learning the underlying method.  I could then test, criticize,
and learn from an executable artifact before I was ready to write that
implementation myself.  This is not a measured comparison with a human-only
completion time.

Early in the project, an agent translated a public MATLAB implementation of the
...
and omitted important pruning.  Considerable later work was needed to obtain
the implementation used in this thesis.
...
These examples suggest a conditional change in workflow rather than a general
speedup: when a reference or explicit contract and relevant infrastructure
exist, implementation and testing need not wait for me to master every detail
needed to write the artifact myself.  The resulting artifact should be treated
as a question made executable, not as an answer certified by the cost or speed
...
or verifier-first contract.  The original prompt text is not recoverable,
...
run per cell, no randomization, and no verbatim prompt record, this does not
...
\begin{enumerate}
  ...
  derivations, symbolically tractable special cases, and adversarial inputs
