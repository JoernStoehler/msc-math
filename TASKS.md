...
- Follow-up packet landed: bounded `feature_face_symplectic` adds ridge-polygon symplectic-area summaries; it is strong within random (`R^2=0.4883` ridge, `0.8166` RF) and becomes the strongest non-metadata endpoint-side block so far (`R^2=0.3824` ridge, `0.2330` RF), but still fails transfer in both directions.
