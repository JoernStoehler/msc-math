...
def verify_manifest(input_path, augment_report_path, expected_revision=None, require_clean=False, design="disposable", allow_external_disposable=False):
    ...
    declared_output_path=output.get("path")
    if not isinstance(declared_output_path,str) or Path(declared_output_path).resolve() != Path(input_path).resolve(): raise AnalysisError("feature output path does not match analyzed input")
    if manifest.get("rows") != len(rows) or output.get("schema") != FEATURE_SCHEMA or output.get("rows") != len(rows) or output.get("sha256") != h: raise AnalysisError("feature output does not match augment report")
