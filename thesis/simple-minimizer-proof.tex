...
%   Since I_K depends only on velocity, I_K(z_{\alpha,b}) = I_K(z).
%   The family parameterized by (\alpha, b) \in \mathbb{R}_{>0} \times \mathbb{R}^4
%   has constant I_K, but varying period and action.
...
Let $z \in W^{1,2}([0,T_z], \mathbb{R}^4)$ be a closed curve that is a
...
\begin{equation}\label{eq:scaling-family}
  z_{\alpha,b}(t) := \alpha\, z\bigl(t/\alpha^2\bigr) + b,
  \quad t \in [0, \alpha^2 T_z],
\end{equation}
with period $T_{\alpha,b} := \alpha^2 T_z$.

Then:
\begin{enumerate}
\item[(a)] $z_{\alpha,b}$ is closed and is a critical point of the dual problem.
\item[(b)] $I_K(z_{\alpha,b}) = I_K(z)$ (independent of $\alpha$ and $b$).
\item[(c)] The action scales as $A(z_{\alpha,b}) = \alpha^2\, A(z)$.
\item[(d)] If $A(z) = T_z$, then $A(z_{\alpha,b}) = T_{\alpha,b}$
  (the dual constraint is preserved).
\end{enumerate}
\end{lemma}

\begin{proof}
\emph{Claim (a):} Closure follows from $z_{\alpha,b}(\alpha^2 T_z) = \alpha z(T_z) + b = \alpha z(0) + b = z_{\alpha,b}(0)$.

For the critical point property: $z$ is a critical point of $I_K$ subject to
$A(z) = T_z$. Both $I_K$ and $A$ are functionals depending only on the velocity
$\dot{z}$. Under the rescaling $z(t) \mapsto \alpha z(t/\alpha^2)$, the velocity
transforms as $\dot{z}(t) \mapsto (1/\alpha) \dot{z}(t/\alpha^2)$. Since $I_K$
is 2-homogeneous in velocity (via $h_K^2$) and $A$ is also 2-homogeneous in
velocity, the criticality condition (vanishing of the variation of the Lagrangian
$I_K - \lambda A$) is preserved under this rescaling. Translation by constant~$b$
is trivial since both functionals depend only on $\dot{z}$.

\emph{Claim (b):} Since $I_K(z) = \frac{1}{4}\int_0^T h_K^2(-J_0\dot{z})\, dt$
depends only on the velocity, $I_K(z + b) = I_K(z)$ for any constant~$b$.
For the scaling:
\[
  I_K(z_{\alpha,b})
  = \frac{1}{4} \int_0^{\alpha^2 T_z} h_K^2(-J_0 \dot{z}_{\alpha,b}(t))\, dt
  = \frac{1}{4} \int_0^{\alpha^2 T_z} h_K^2\bigl( -J_0 (1/\alpha) \dot{z}(t/\alpha^2) \bigr)\, dt.
\]
Since $h_K$ is 1-homogeneous:
\[
  = \frac{1}{4} \int_0^{\alpha^2 T_z} (1/\alpha)^2 h_K^2(-J_0\dot{z}(t/\alpha^2))\, dt
  = \frac{1}{\alpha^2} \cdot \frac{\alpha^2}{4} \int_0^{T_z} h_K^2(-J_0\dot{z}(s))\, ds
  = \frac{1}{4} \int_0^{T_z} h_K^2(-J_0\dot{z}(s))\, ds = I_K(z),
\]
using substitution $s = t/\alpha^2$ with $dt = \alpha^2\, ds$.

\emph{Claim (c):} Similarly, $A(z) = \frac{1}{2}\int_0^T \langle -J_0\dot{z}, z\rangle\, dt$
gives
\[
  A(z_{\alpha,b})
  = \frac{1}{2} \int_0^{\alpha^2 T_z} \langle -J_0(1/\alpha)\dot{z}(t/\alpha^2),\, \alpha z(t/\alpha^2) + b \rangle\, dt.
\]
Since $\int_0^{\alpha^2 T_z} \langle -J_0\dot{z}(t/\alpha^2), b\rangle\, dt = 0$ (using $J_0$ skew-symmetry
and closure of $z$), we get
\[
  = \alpha \cdot \frac{1}{2} \int_0^{\alpha^2 T_z} \langle -J_0\dot{z}(t/\alpha^2), z(t/\alpha^2)\rangle\, dt
  = \alpha^2 \cdot \frac{1}{2} \int_0^{T_z} \langle -J_0\dot{z}(s), z(s)\rangle\, ds
  = \alpha^2\, A(z).
\]

\emph{Claim (d):} If $A(z) = T_z$, then $A(z_{\alpha,b}) = \alpha^2 A(z) = \alpha^2 T_z = T_{\alpha,b}$.
\end{proof}

\begin{theorem}[Primal-dual equivalence (Clarke's Dual Action Principle)]%
\label{thm:primal-dual-equivalence}
% QC: Correspondence via z = gamma - center(gamma) (forward direction).
%   By Lemma~\ref{lem:dual-scaling-family}, dual critical points form
%   scaling families with constant I_K, so I_K(z) = T only for the special
%   representative z from a Reeb orbit. Minimum equality still holds.
...
\emph{(a) Forward correspondence.}
If $\gamma$ is a primal critical point (Hamiltonian orbit on $\partial K$)
with period~$T$, then
\[
  z := \gamma - \frac{1}{T} \int_0^T \gamma(t) \, dt
\]
is a dual critical point with period~$T$, and
\begin{equation}\label{eq:primal-dual-values}
  I_K(z) = T = A(\gamma).
\end{equation}

\emph{(b) Scaling families.}
By Lemma~\ref{lem:dual-scaling-family}, dual critical points form
families $\{z_{\alpha,b}\}_{\alpha > 0, b \in \mathbb{R}^4}$ with
$I_K(z_{\alpha,b}) = I_K(z)$ constant.
The equality $I_K(z) = T_z$ holds only for representatives arising
from~(a).

\emph{(c) Minimum equality.}
Consequently, $\min_\gamma A(\gamma) = \min_z I_K(z) = c_{\mathrm{EHZ}}(K)$.
\end{theorem}

\begin{proof}
% Jörn: text approved (3202a68)
...
\begin{align*}
  ...
  &= \nu^2 A(z) + \tfrac{\nu}{2}\!\int_0^T
    \langle -J_0 \dot{z},\, b \rangle\, dt
   = \nu^2 A(z) + \tfrac{\nu}{2}
    \bigl\langle -J_0\underbrace{[z(T) - z(0)]}_{= 0},\, b \bigr\rangle
   = \nu^2 A(z).
\end{align*}
...
\]
Since $A(\gamma) = T$ (the action equals the period for any Reeb orbit),
we have $I_K(\gamma) = T$. Since $\dot{z} = \dot{\gamma}$, we get $I_K(z) = I_K(\gamma) = T$.
Therefore $I_K(z) = T = A(\gamma)$.

% QC note: the proof shows the correspondence is surjective in both
%   directions and value-preserving (Parts 2--4).  Bijectivity is not
%   claimed: for polytopes, translates of gamma can remain on dK,
%   giving multiple primal orbits for one dual curve z.  This does not
%   affect the min-equality below (all such orbits share the same action).
Since the correspondence exists in both directions and preserves values,
minimizers correspond to minimizers:
$\min_\gamma A(\gamma) = \min_z I_K(z) = c_{\mathrm{EHZ}}(K)$.
\end{proof}

\begin{remark}\label{rem:primal-dual-nonunique}
The base point~$b$ in Part~3 need not be unique: for polytopes,
translates of~$\gamma$ can remain on~$\partial K$, giving
distinct primal orbits with the same~$z$.
The minimum-equality is unaffected since all such orbits share the same action.
\end{remark}
...
%   Core: W^{1,2} approximation preserving closure, centering, velocity constraint.
%   Corollary: A and I_K estimates by continuity.
\begin{lemma}[Piecewise affine approximation]\label{lem:pw-affine-approx}
Let $z \in W^{1,2}([0,T], \mathbb{R}^4)$ be closed and centered
with $\dot{z}(t) \in \mathrm{conv}\{R_1, \ldots, R_F\}$ a.e.
For any $\varepsilon > 0$, there exists a closed, centered,
...
Let $z \colon [0,T] \to \mathbb{R}^4$ be a closed, centered,
piecewise affine curve with velocities in
$\mathrm{conv}\{R_1, \ldots, R_F\}$.
There exists a closed, centered, piecewise affine curve~$z'$
...
Let $(z_n)$ be a sequence of closed, centered, simple
piecewise-Reeb curves with periods $T_n \to T > 0$.
Then a subsequence converges to a closed, centered, simple
piecewise-Reeb curve~$z_*$ with period~$T$,
$A(z_*) = \lim A(z_n)$, and $I_K(z_*) = T$.
\end{lemma}
...
\item \emph{Split}
  ...
  obtain a closed, centered $z_2$ with pure Reeb velocities,
  $A(z_2) \geq A(z_1)$, and $I_K(z_2) = T$.

\item \emph{Merge}
  (Lemma~\ref{lem:merging}):
  obtain a closed, centered $z_3$ with each Reeb vector
  used at most once,
...
\item \emph{Compactness}
  ...
  has periods $T_4 \to T$, so a subsequence converges
  to a closed, centered, simple piecewise-Reeb
  curve~$z_*$ with
  $A(z_*) = T$ and $I_K(z_*) = T$.
...
\emph{Centering restored:}
translate $z'$ by $-\frac{1}{T}\int_0^T z'\,dt$
to restore centering; this preserves $\dot{z}'$, closure, $A(z')$, and $I_K(z')$.
