...
Is HKO2024 (the only known Viterbo counterexample, sys ≈ 1.047) a local maximum
of the systolic ratio? In its 10-facet parameter space? Even when allowing
facet-splitting into 11-facet polytopes?
...
- `hko-neighborhood-{gradient,orbits,splitting}.png` — Figures

## Key observation

All ∂sys/∂h_k < 0: increasing any height *decreases* sys (normals fixed).
Decreasing heights shrinks K toward degeneracy. The normal gradient is nonzero
(|∇sys_n| ≈ 1.53), so HKO2024 is not a critical point on the (n,h) parameter
space. However, gradient ascent in the joint (h,n) direction converges
immediately (Δsys ≈ 5e-9), suggesting HKO2024 sits near a boundary of the
