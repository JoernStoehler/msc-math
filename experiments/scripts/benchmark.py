...
Goal: Derive timing model T(F) = a * b^F from the main polytope dataset,
      produce a figure, and update the timing model.
Input: experiments/data/polytopes.jsonl (generated by run_dataset.py)
...
DATA_PATH = REPO_ROOT / "experiments" / "data" / "polytopes.jsonl"

MODEL_PATH = PROFILING_DIR / "timing_model.json"


def load_dataset():
    """Load random polytopes from the main dataset JSONL."""
    rows = []
    with open(DATA_PATH) as f:
        for line in f:
            ...
            if row["source"] != "random":
                continue
            # Skip rows without valid capacity (NaN)
            cap = row.get("capacity")
            if cap is None or (isinstance(cap, float) and cap != cap):  # NaN check
                continue
            rows.append({
                "facets": row["facet_count"],
                "time_ms": row["time_capacity_ms"],
...
def update_timing_model(a, b, r_squared, stats):
    ...
    model = {
        "model": {
            ...
            "parameters": {"a": a, "b": b},
            "units": "seconds",
            "source": "benchmark.py (random polytopes from polytopes.jsonl)",
        },
        "fit_quality": {
            "r_squared": r_squared,
...
def main():
    if not DATA_PATH.exists():
        ...
        print("Run: python3 experiments/scripts/run_dataset.py", file=__import__("sys").stderr)
