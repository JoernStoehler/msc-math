...
\item[Maximum tracking] (``find the largest admissible $Q$''):
  ...
  dominates). Downstream consumers decide how to handle the
  remaining uncertainty (Section~\ref{app:error-tracking})---e.g.\
  accept the certified lower bound, or attempt more expensive tests.
\end{description}
...
Letting $\delta(v,i) \coloneqq |\langle n_i, v \rangle - h_i|$:
\[
  \tilde P_{\text{adj}}(i,j) =
  \begin{cases}
    \textsc{true}
      & \text{if } \exists\, v : \delta(v,i) < \varepsilon
        \text{ and } \delta(v,j) < \varepsilon, \\
    \textsc{false}
      & \text{if } \min_v \max(\delta(v,i),\, \delta(v,j))
        > \varepsilon', \\
    \textsc{unknown}
      & \text{otherwise.}
  \end{cases}
...
differs from the previous two predicates:
