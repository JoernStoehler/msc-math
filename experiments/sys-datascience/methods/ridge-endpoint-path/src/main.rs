...
const EXPECTED_IDS: [&str; 8] = [
    "ridge-endpoint-3x6-q01",
    "ridge-endpoint-3x6-q001",
    "ridge-endpoint-3x6-q0001",
    "ridge-endpoint-3x6-endpoint",
    "ridge-endpoint-4x4-q01",
    "ridge-endpoint-4x4-q001",
    "ridge-endpoint-4x4-q0001",
    "ridge-endpoint-4x4-endpoint",
];
...
fn implementation_manifest() -> CapacityManifest {
    ...
    let implementation_files = relative_paths
        .iter()
        .map(|relative| ImplementationFile {
    ...
    let repo_commit = String::from_utf8(
        Command::new("git")
            .args(["rev-parse", "HEAD"])
            .output()
...
/// Packet-local instrumentation of the current private MinimaSafe resolver.
...
fn instrumented_minimasafe_exact_fallback_counts(
    ...
    fn exact_replace(
        ...
        let beta_sum = exact
            ...
            .iter()
            .cloned()
            .fold(BigRational::zero(), |sum, beta| sum + beta);
        if beta_sum != BigRational::one()
            || !(0..4).all(|d| {
                orbit
                    ...
                    .iter()
                    .zip(exact.beta.iter())
                    .map(|(&facet, beta)| beta * &dual_vertices[facet][d])
                    .fold(BigRational::zero(), |sum, entry| sum + entry)
                    .is_zero()
            })
        {
            return None;
        ...
        let beta = exact
            .beta
            .iter()
            .map(|value| value.to_f64().expect("exact beta converts to f64"))
            .collect::<Vec<_>>();
    ...
    fn resolve_indices(
        ...
        *resolutions += indices.len();
        ...
        assert!(
            !orbits.is_empty(),
            "MinimaSafe exact fallback removed every orbit"
        );
    }
    ...
    loop {
        loop {
            ...
            if orbits[lower_index].admissibility == OrbitAdmissibility::IndeterminateF64 {
                needs.push(lower_index);
            }
            if orbits[upper_index].admissibility == OrbitAdmissibility::IndeterminateF64 {
                needs.push(upper_index);
            }
            if needs.is_empty() {
                break;
            }
        ...
        let needs = orbits
            ...
            .filter_map(|(i, orbit)| {
                (orbit.admissibility == OrbitAdmissibility::IndeterminateF64
                    && orbit.action_lower <= upper
                    && lower <= orbit.action_upper)
                    .then_some(i)
            })
            .collect::<Vec<_>>();
        if needs.is_empty() {
...
fn main() {
    ...
    let mut manifest_out = BufWriter::new(
        File::create(artifact_path("capacity-implementation-manifest.json"))
    ...
    let input =
        BufReader::new(File::open(artifact_path("candidates.jsonl")).expect("open candidates"));
    let mut output = BufWriter::new(
        File::create(artifact_path("target-evaluation.jsonl")).expect("create target output"),
    );
    ...
    for line in input.lines() {
        ...
        let f64_indeterminate_candidate_count_before_aggregation = candidates
            .iter()
        ...
        let instrumented_min_action = instrumented_orbits
            .iter()
            .filter(|orbit| {
                matches!(
                    orbit.admissibility,
            ...
            .expect("instrumented admissible orbit");
        assert!(
            (instrumented_min_action - ordinary.min_action).abs() <= ARITHMETIC_TOLERANCE,
        ...
        let result_is_finite = volume.is_finite()
            && capacity.is_finite()
            && sys.is_finite()
            && time_capacity_ms.is_finite()
            && ordinary.min_action_lower.is_finite()
            && ordinary.min_action_upper.is_finite();
        all_results_finite &= result_is_finite;
        no_sys_above_one &= sys <= 1.0;
        max_arithmetic_absolute_error =
            max_arithmetic_absolute_error.max(arithmetic_absolute_error);
        let target = TargetRow {
            schema: "ridge-endpoint-smoke.target-evaluation.v2",
            candidate_id: row.candidate_id.clone(),
            bucket: row.bucket.clone(),
            path_label: row.path_label.clone(),
            poly_id: poly_id.clone(),
            volume,
            capacity,
            sys,
            bounces,
            best_sigma: best_sigma.clone(),
            time_capacity_ms,
            result_is_finite,
            arithmetic_absolute_error,
            iterations: ordinary.iterations,
            min_action_lower: ordinary.min_action_lower,
            min_action_upper: ordinary.min_action_upper,
            returned_orbit_count: ordinary.orbits.len(),
            best_orbit_admissibility: format!("{:?}", best.admissibility),
            best_orbit_beta_margin: best.beta_margin,
            f64_indeterminate_candidate_count_before_aggregation,
            exact_fallback_resolution_count,
            exact_fallback_rejected_count,
            candidates_sha256: candidates_sha256.clone(),
            api_verification_sha256: api_verification_sha256.clone(),
            evaluator_source_sha256: evaluator_source_sha256.clone(),
            capacity_manifest_sha256: capacity_manifest_sha256.clone(),
        };
        writeln!(output, "{}", serde_json::to_string(&target).unwrap()).unwrap();
        output.flush().unwrap();
        assert!(
            sys <= 1.0,
            "trusted target result sys > 1 for {}; stopping immediately",
            row.candidate_id
        );
        if row.path_label == "endpoint" && row.bucket == "3x6" {
            assert!(
                (sys - 0.75).abs() <= ENDPOINT_TOLERANCE,
                "3x6 endpoint sys mismatch: {sys}"
            );
            endpoint_3x6_sys = Some(sys);
        }
        if row.path_label == "endpoint" && row.bucket == "4x4" {
            assert!(
                (sys - 0.5).abs() <= ENDPOINT_TOLERANCE,
                "4x4 endpoint sys mismatch: {sys}"
            );
        ...
        if row.candidate_id == "ridge-endpoint-3x6-q01" {
            ...
            assert!(
                agreement_absolute_error <= ARITHMETIC_TOLERANCE,
                "q01 ordinary/certified capacity mismatch"
            );
        ...
        assert!(
            observed_ids.insert(row.candidate_id),
            "duplicate candidate id"
        );
    }
    let certificate = certificate.expect("q01 certificate must be present");
    let mut certificate_out = BufWriter::new(
        File::create(artifact_path("q01-certified-minimizers.json")).expect("create certificate"),
    );
    writeln!(
        certificate_out,
        "{}",
    ...
    let expected_id_set_matches = observed_ids
        .iter()
        .map(String::as_str)
        .collect::<BTreeSet<_>>()
        == expected_ids;
    assert!(
        expected_id_set_matches,
        "expected exactly the frozen eight candidate ids"
    );
    assert!(
        max_arithmetic_absolute_error <= ARITHMETIC_TOLERANCE,
        "sys arithmetic mismatch"
    );
    let (q01_3x6_capacity, q01_3x6_sys) = q01_3x6.expect("q01 row must be present");
    let summary = TargetSummary {
        schema: "ridge-endpoint-smoke.target-summary.v2",
        status: "completed",
        expected_candidate_ids: EXPECTED_IDS.to_vec(),
        observed_candidate_ids: observed_ids.into_iter().collect(),
        expected_id_set_matches,
        unique_ids: true,
        row_count: 8,
        all_results_finite,
        ...
        endpoint_4x4_sys: endpoint_4x4_sys.expect("4x4 endpoint must be present"),
        endpoint_tolerance: ENDPOINT_TOLERANCE,
        q01_3x6_capacity,
        q01_3x6_sys,
        target_evaluation_sha256: sha256(artifact_path("target-evaluation.jsonl")),
        certificate_sha256: sha256(artifact_path("q01-certified-minimizers.json")),
        candidates_sha256,
        api_verification_sha256,
        cdf_placement_json_sha256: sha256(artifact_path("cdf-placement.json")),
        cdf_placement_tsv_sha256: sha256(artifact_path("cdf-placement.tsv")),
        capacity_manifest_sha256,
        evaluator_source_sha256,
    };
    let mut summary_out =
        BufWriter::new(File::create(artifact_path("target-summary.json")).expect("create summary"));
    writeln!(
        summary_out,
        "{}",
