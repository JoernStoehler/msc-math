...
The contributions grew out of a broad computational investigation. We
developed methods for evaluating capacity and used them to explore examples,
following up on questions that the computations suggested. This led to
several separate results, from a local maximality theorem to algorithms and
statistical observations. The following overview explains what each part
contributes to that investigation.
...
\cite{HaimKislevOstrover2024}. The equality of EHZ and cylindrical capacity
on general convex domains in four dimensions is still listed as open in
\cite[Remark~1.4(v)]{HaimKislevOstrover2024}. Edtmair instead characterizes
...
For each nearby direction, at least one of these bounds decreases. It follows
that the ratio itself decreases. The finite task is to construct enough
...
We expand the proof in Haim--Kislev's original paper into a self-contained
presentation for a master's-level reader. We explain the dual action
principle and the existence of a minimizing curve with a finite list of
distinct facet directions.
...
We pursued the flow-graph method of Chaidez and Hutchings~\cite{CH2021}
as an alternative way to compute capacity. Its performance in our tests
made it less useful for large searches, but it provides a reference
implementation for cross-checking the quadratic-program method. Agreement
between the two gives additional evidence that the implementations compute
the intended quantity.

The same finite methods yield a side result for the family
$P_5\times R(\theta)P_5$: its systolic ratio is
\[
 \frac{5+2\sqrt5}{10}\sec^2 d(\theta),
\]
where $d(\theta)$ is the distance to the nearest multiple of $\pi/5$.
An exhaustive comparison proves the formula, and its maximum is the HKO
value. This determines the whole relative-rotation profile while keeping
both factors regular.

\subsection{Looking for mathematical structure in data}

We also use data science to seek geometric relationships involving capacity.
The aim is to discover statistical patterns, interpret them geometrically,
and turn them into conjectures that can be proved or refuted. The experiments
find patterns below systolic ratio one, but do not establish their extension
above one. One geometric interpretation yields an exact edge--width formula
for a measured quantity on Lagrangian products; relating it to capacity
remains open.

\subsection{How to read the thesis}

The order follows the passage from geometry to computation and then to
applications. Sections 2--4 explain how nonsmooth boundary dynamics become
a finite quadratic program. Section 5 presents the alternative flow-graph
method, and Section 6 develops first-order variation. These foundations
prepare the local theorem in Section 7 and the further investigations in
Sections 8 and 9.

A reader primarily interested in the HKO theorem can start with its statement
and proof strategy in Section 7, returning to Sections 2--4 for the capacity
formula and to Section 6 for variation. The flow-graph and data-science
chapters are not prerequisites for that proof. Section 8 describes the
empirical investigation and can be read independently of the HKO certificate;
Section 9 gives the separate theorem for rotated pentagons. The later
sections document visualizations, numerical methods, code and data, and
reflect on the use of AI. The appendices contain computational detail that
can be consulted when checking or reproducing the results.
