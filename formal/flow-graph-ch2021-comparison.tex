...
\begin{description}
  \item[Main computational target.]
    CH2021 finds combinatorial Reeb orbits, actions, and rotation/CZ data, and
    ...
    targets full EHZ capacity only after a supported exact theorem scope, or a
    separate f64 exact-agreement acceptance scope, has been stated explicitly.
    Its correctness theorem is organized by
  ...
  \item[Finite search control.]
    CH2021 controls finiteness with rotation bounds and segment-count
    constraints.  The project search control is expressed through a finite
  ...
  \item[Numerical implementation.]
    ...
    statements are not this repository's exact/f64 API contract.  The Rust paths
    distinguish exact accepted output, f64 diagnostics, exact fallback, typed
    rejection, retained words, and profiling/evidence surfaces.
\end{description}
\end{remark}
...
\begin{enumerate}
  ...
  \item numerical accepted outputs agree with the exact semantic resolver, or
    all output-relevant numerical predicates are resolved exactly.
\end{enumerate}
