...
- Follow-up packet landed: bounded `feature_face_geometry` adds edge-length and facet-3-volume summaries from exact face geometry; it helps within-random strongly for RF (`R^2=0.6756`) and adds only a small endpoint-only signal (`R^2=0.0835` ridge, `0.0988` RF), with transfer still strongly negative.
- Follow-up packet landed: bounded `feature_face_symplectic` adds raw ridge-polygon symplectic-area summaries; it is strong within random (`R^2=0.4883` ridge, `0.8166` RF) and becomes the strongest non-metadata endpoint-side block so far (`R^2=0.3824` ridge, `0.2330` RF), but it remains scale-sensitive and still fails transfer in both directions.
