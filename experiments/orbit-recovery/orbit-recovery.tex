...
\end{itemize}
% [TODO: JÖRN - verify the following characterization lemma. Derived in
%   conversation: closure + on-facet + convexity of K reduce the defining
%   conditions of def:reeb-orbit-polytope to finitely many checks at breakpoints.]
\begin{lemma}[Finite verification of simple Reeb orbits]%
\label{lem:finite-orbit-verification}
Let $K$ be a polytope with facets $F_1, \ldots, F_F$ (outward normals
$n_i$, offsets~$h_i$, Reeb vectors~$R_i$).
A piecewise-affine closed curve $\gamma \colon [0, T] \to \mathbb{R}^4$
with breakpoints $0 = t_0 < t_1 < \cdots < t_m = T$ and constant
velocity $\dot{\gamma} = R_{\sigma(k)}$ on $(t_{k-1}, t_k)$
...
\begin{enumerate}
  ...
  \item \emph{Reeb velocity:}
    ...
    for some facet index $\sigma(k)$, for each $k = 1, \ldots, m$.
  \item \emph{On-facet:}
    $\langle n_{\sigma(k)},\, \gamma(t_{k-1}) \rangle = h_{\sigma(k)}$
    for each $k = 1, \ldots, m$.
  \item \emph{Inside $K$:}
    $\langle n_j,\, \gamma(t_k) \rangle \le h_j$
...
the inner product
$\langle n_{\sigma(k)}, \gamma(t) \rangle$ is constant on the $k$-th
segment. Hence $\gamma(t) \in F_{\sigma(k)}$ for all
$t \in [t_{k-1}, t_k]$ if and only if it holds at~$t_{k-1}$.
For~(4): on each segment, $\langle n_j, \gamma(t) \rangle$ is
