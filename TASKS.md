...
- Execution status on `capacity-result-api-exec`: Packet 1 scaffold landed as `library/src/algorithms/orbit_search.rs` with shared enums/types for the new result layer. Packet 2 is in progress: there is now a shared `solve_orbit_sigma(...)` primitive on the saddle-point path and a shared `collect_legacy_capacity(...)` seam for the current solve/classify/track/finalize loop. The remaining Packet 2 work is the actual `OrbitSearchResult`-returning collector surface above those seams.
