...
Figure `fig:flow-graph-tube-example` uses the deterministic generated
...
- The projection panel shows the polytope one-skeleton and orbit after radial
  projection to `S^3` and stereographic projection to `R^3`; it distorts metric
  geometry.
- The sequence panel shows only the visited facet-pair charts. Each panel has
  unrelated affine coordinates and scale, so cross-panel Euclidean shape or
  size comparisons are invalid.
