...
- Candidate design surface: one shared orbit/result layer beneath the root router family `ehz_capacity` (auto), `ehz_capacity_pruned`, `ehz_capacity_unpruned`, and `ehz_capacity_billiard`, with separate building blocks for sigma traversal, one-sigma solve, and aggregation.
...
- Current code state: root `ehz_capacity` is the ordinary scalar/default entrypoint and returns `OrbitSearchResult`; simple consumers use its scalar convenience accessors while validation code can still inspect the returned best orbit directly. Several experiments have copied instrumented enumeration code that collects all valid or near-optimal orbits. `orbit_recovery::recover_and_verify` validates one `(sigma, beta, capacity)` result at a time, so the validation session depends on either an experiment-local adapter or the library API design task above.
