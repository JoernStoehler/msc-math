...
% QC: action identity translates HK2017 Proposition 4.3.
\begin{lemma}[Action identity]\label{lem:action-identity}
Let $z \colon [0,T] \to \mathbb{R}^4$ be a closed piecewise affine
curve with piecewise constant velocity
$\dot{z}(t) = w_k$ on interval~$I_k$,
where $(I_1, \ldots, I_m)$ partitions~$[0,T]$.
Then
...
\item \emph{Rescale:}
  ...
  (by Lemma~\ref{lem:action-identity}, every term scales as
...
By Lemma~\ref{lem:action-identity}, $A$ is a polynomial
in the durations, so
...
\begin{proof}[Proof of Lemma~\ref{lem:action-identity}]
% QC: direct computation. Key steps: (a) closure kills z(0) term,
%   (b) self-pairing vanishes by omega_0 antisymmetry,
%   (c) cross terms give the double sum.
Write $z(t) = z(0) + \int_0^t \dot{z}(s)\, ds$.
Since $z(0) = z(T)$,
\[
  \int_0^T \langle -J_0 \dot{z}(t),\, z(0) \rangle\, dt
  = \langle -J_0[z(T) - z(0)],\, z(0) \rangle = 0,
\]
so
\[
  2A(z)
...
\]
For $t \in I_i$, the velocity is $\dot{z}(t) = w_i$ and
\[
  \int_0^t \dot{z}(s)\, ds
  = \sum_{j=1}^{i-1} |I_j|\, w_j + (t - \tau_{i-1})\, w_i,
\]
where $\tau_{i-1}$ is the left endpoint of~$I_i$.
The self-pairing $\omega_0(w_i, w_i) = 0$ eliminates the
last term, giving
\[
  \int_{I_i} \langle -J_0 w_i,\,
    \textstyle\sum_{j<i} |I_j|\, w_j \rangle\, dt
  = |I_i| \sum_{j=1}^{i-1} |I_j|\, \omega_0(w_j, w_i).
\]
...
by Lemma~\ref{lem:action-identity},
