...
For a convex polytope in $H$-representation with unit normals,
$\partial\,\mathrm{vol}/\partial h_k = S_k$,
where $S_k$ is the 3-dimensional volume of facet $F_k$.
This is the standard first variation of volume:
moving facet $k$ outward by $\delta$ adds a slab of thickness $\delta$ and
cross-section $F_k$, so the volume increases by $\delta\,S_k$ to first order.
Facet volumes are computed via ridge-based tetrahedralization
(centroid + ridge fan triangulation, 3D volume via the 4D cross product).
