...
| The idealized mathematical condition needs nonzero `omega_0` only on physical facet transitions, not on all facet pairs. | source-backed by `research/tube-algorithm.md` accepted clarification and formalized in `formal/flow-graph-real-algorithm.tex`, `def:fg-nondegenerate-facet-presentation` and `lem:fg-local-transition-regularity-positive-sign`; `exact_search.rs::validate_no_geometric_zero_omega_transition` and `f64_tube_search.rs::FlatTubeInput::validate_no_geometric_zero_omega_transitions` enforce the stronger implementation condition on every nonempty facet-pair candidate | theorem/code boundary: code may reject polytopes allowed by the idealized theorem, including nonempty-but-not-physically-transitioning zero-omega facet pairs | keep the thesis theorem and implementation scope separate unless validators are weakened to the local physical-transition condition |
