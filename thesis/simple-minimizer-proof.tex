...
Let $z \in W^{1,2}([0,T_z], \mathbb{R}^4)$ be a closed curve satisfying
the dual Euler--Lagrange equation~\eqref{eq:dual-euler-lagrange}.
...
\item $z_{\alpha,b}$ is closed and satisfies the dual Euler--Lagrange
  equation~\eqref{eq:dual-euler-lagrange}.
...
\begin{proof}
% [JÖRN: proof to be written based on your derivation]
\emph{Claim (a):} Closure follows from $z_{\alpha,b}(\beta T_z) = \alpha z(T_z) + b = \alpha z(0) + b = z_{\alpha,b}(0)$.
For the Euler--Lagrange equation: since the equation is obtained via
Lagrange multipliers for the constraint $A(z) = T$, and since both
$I_K$ and $A$ are homogeneous functionals in the velocity,
the rescaling $z \mapsto \alpha z(t/\beta)$ preserves the
subgradient relation. Translation by $b$ is harmless since
$I_K$ depends only on $\dot{z}$.
