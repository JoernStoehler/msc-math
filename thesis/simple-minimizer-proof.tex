...
\begin{proof}
Form the Lagrangian with multiplier $\alpha$ for the action constraint:
\[
  L(z, \dot{z}) = \tfrac{1}{4}h_K^2(-J_0\dot{z}) + \alpha\langle -J_0\dot{z}, z\rangle.
\]
For such Lagrangians (convex in $\dot{z}$, linear in $z$), the Euler-Lagrange
inclusion states: there exists $\xi(t) \in \partial_{\dot{z}} L$ a.e.\ such that
\[
  \dot{\xi}(t) = \nabla_z L(z(t), \dot{z}(t)) = -\alpha J_0 \dot{z}
  \quad\text{a.e.}
\]
The subdifferential in $\dot{z}$ is
\[
  \partial_{\dot{z}} L
  = J_0\, \partial\bigl(\tfrac{1}{4}h_K^2\bigr)(-J_0\dot{z}) + \alpha J_0 z,
\]
so $\xi(t) = J_0 p(t) + \alpha J_0 z(t)$ for some $p(t) \in \partial(\tfrac{1}{4}h_K^2)(-J_0\dot{z}(t))$.
The Euler-Lagrange inclusion gives:
\[
  J_0 \dot{p} + \alpha J_0 \dot{z} = -\alpha J_0 \dot{z}
  \quad\Rightarrow\quad
  \dot{p} = -2\alpha \dot{z} \quad\text{a.e.}
\]
Integrating: $p(t) = c - 2\alpha z(t)$ for constant $c \in \mathbb{R}^4$.
...
\begin{align*}
  ...
  \quad \text{(2-homog.)} \\
  &\Rightarrow \quad
  \frac{\nu}{\alpha^3} z'(t) + \biggl(\frac{b_0}{\alpha^2} - \frac{\nu b}{\alpha^3}\biggr)
...
\[
  ...
  \quad \text{a.e.}
\]
Writing $\nu = -2\alpha$ and $b = c$, this is exactly the Euler-Lagrange
condition~\eqref{eq:dual-euler-lagrange} from Lemma~\ref{lem:dual-euler-lagrange}.
...
This is exactly the Euler-Lagrange condition~\eqref{eq:dual-euler-lagrange} with $\nu = 1$.
...
\emph{Step 1: Converting the Euler-Lagrange condition.}
Since $\dot{\gamma} = \nu\dot{z}$, the Euler-Lagrange condition becomes
\[
  \gamma \in \partial\bigl(\tfrac{1}{4}h_K^2\bigr)(-J_0\dot{\gamma}/\nu)
  \quad\text{a.e.}
\]
