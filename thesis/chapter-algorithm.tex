...
% Approval: proof structure approved by Jörn. Text not yet reviewed.
...
\medskip
\noindent\textbf{Algorithm} (restated from
Section~\ref{sec:main-result}).

\smallskip\noindent
For each nonempty subset $S \subseteq \{1, \ldots, F\}$
and each cyclic ordering~$\sigma$ of~$S$:

\begin{enumerate}
\item \textbf{Define}
  $N \in \mathbb{R}^{|S| \times 4}$,
...
\item \textbf{Solve}
  system~\eqref{eq:linear-system} for
  $(\beta, \lambda, \nu)$.

\item \textbf{Filter.}
  Discard if $\beta_i \leq 0$ for any $i$,
  or if $Q(\beta) \leq 0$.
...
%   non-maximizing critical points of Q need not correspond to
...
$Q$ is continuous, and the set of cyclic orderings~$\sigma$
is finite. Hence the maximum is attained at
some~$(\sigma^*, \beta^*)$ with value $Q^* > 0$
...
definition, and $Q^* > 0$ by Step~1.
Hence the algorithm considers~$(\sigma^*, \beta^*)$ as a
candidate with $A = \frac{1}{2Q^*} = c_{\mathrm{EHZ}}(K)$.

\emph{Step~3: The algorithm returns it.}
The algorithm returns $\min A = \min \frac{1}{2Q}$ over all
candidates. For any candidate with value~$Q \leq Q^*$,
we have $A = \frac{1}{2Q} \geq \frac{1}{2Q^*}$.
Since $(\sigma^*, \beta^*)$ achieves
$A = \frac{1}{2Q^*}$, this is the minimum.
Hence the returned value is
$\frac{1}{2Q^*} = c_{\mathrm{EHZ}}(K)$.
\end{proof}

\begin{remark}\label{rem:non-maximizer-warning}
% Approval: proof structure approved by Jörn. Text not yet reviewed.
% QC: important for agents/readers who might assume all critical
%   points of Q correspond to Reeb orbits.
The proof uses only that the global maximizer of~$Q$ corresponds
to a minimum-action Reeb orbit
(Theorem~\ref{thm:main}, Step~6).
Non-maximizing critical points of~$Q$ that pass the filter
($\beta > 0$, $Q > 0$) need not correspond to Reeb orbits:
the bijection between $(\sigma, \beta)$ and simple orbits is
established only at the optimum.
The algorithm may evaluate~$A$ at such spurious critical
points, but they cannot affect the returned minimum
(Step~3 above).
\end{remark}
