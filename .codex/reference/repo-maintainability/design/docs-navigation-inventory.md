...
- Date: 2026-04-16.
- Scope checked: `AGENTS.md`, `TASKS.md`, `crates/symplectic/src/lib.rs`,
  `crates/symplectic/src/database.rs`, `experiments/<topic>/src/lib.rs`, and the
  maintainability note at `.codex/reference/repo-maintainability/design/main.md`.
- Output rule: `question -> current answer source -> gap -> likely future home`.
- Supersession note: `TASKS.md` was the live tracker when this inventory was
  written. Current task routing lives in `ROADMAP.md` and `tasks/*.md`.
...
- `nl -ba .codex/reference/repo-maintainability/design/main.md | sed -n '1,260p'`
- `nl -ba crates/symplectic/src/lib.rs | sed -n '1,120p'`
- `nl -ba crates/symplectic/src/database.rs | sed -n '1,140p'`
...
| Where is active maintainability work tracked, and what is the current phase? | Historical: `TASKS.md:445-495` and `.codex/reference/repo-maintainability/design/main.md:12-20, 95-107, 230-237`. Current routing should be refreshed from `ROADMAP.md` and `tasks/*.md`. | No gap for status in the 2026-04-16 snapshot. Refresh before reusing. | Tracker packet / no new doc needed |
| What is the public library surface for routine use? | `crates/symplectic/src/lib.rs:1-66` | The simple re-exports are listed, but the file does not separate stable simple surfaces from expert-only deep paths or accidental internals. | `ARCHITECTURE.md` |
| How does the JSONL cache/data layer work? | `crates/symplectic/src/database.rs:1-54, 114-140` and historical `TASKS.md:468-482` | The storage contract is clear, but the canonical shared catalog versus mirror versus transient-search distinction is still open. That is an architecture decision, not a wording gap. | Tracker packet first; `ARCHITECTURE.md` after the policy lands |
...
| Which deep imports are okay for experiments to use? | `.codex/reference/repo-maintainability/design/main.md:44-78` and `crates/symplectic/src/lib.rs:23-36` | The note records examples of deep imports, but the classification into stable/public, expert/public, accidental internal, or unclear is not finished. This is an architecture decision. | D1 import-surface packet, then `ARCHITECTURE.md` |
| Where does the repo explain library-internal architecture and the math/code boundary? | `crates/symplectic/src/lib.rs:6-36` and `AGENTS.md:45-52` | Mostly answered already. The current docs explain submodules, dependency direction, and the formal-file linkage. | No new doc needed |

## Observed Strengths

- `AGENTS.md` already gives a repo-wide orientation map, operating rules, and quick commands.
- In the 2026-04-16 snapshot, `TASKS.md` separated program status from
  execution packets and named the maintainability work explicitly.
- `crates/symplectic/src/lib.rs` already compresses the library into a readable submodule map plus dependency graph.
- `crates/symplectic/src/database.rs` already states ownership: experiment-owned JSONL storage with no canonical shared mutable cache path.
...
- `.codex/reference/repo-maintainability/design/main.md` already separates observed facts, open decisions, discovery packets, and the Jörn decision surface.
