...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item \emph{Boundedness}: for every triple~$(i,j,k)$ of normals, compute
      ...
      triple has positive inner product with~$d$, and some normal outside the
      triple has positive inner product with~$-d$.
...
Since the boundedness check depends only on the normal directions and not on
the heights, this suggests that boundedness is the dominant rejection cause at
low facet count.
...
check becomes the bottleneck: extreme height ratios make it more likely that
some facet's incident vertices fail the affine rank test.
