...
| Data-science search | empirical/data-science exploration feature-complete; triangle--hexagon path bound available | `experiments/sys-datascience/coordination/final-research-account-2026-07-12.md`, reviewed method/gradient packets and producer contracts | reproducible demonstration readiness under retained claim boundaries; optional exact-interior profile work is separate |
...
| C3a | settled for current exploration | Ridge sum/mean redundancy is algebraic; endpoint paths park ridge-only optimization without proving a Goodhart law; product-bounce and residual examples expose structure outside the original ridge interpretation. | `experiments/sys-datascience/coordination/final-research-account-2026-07-12.md`; broader invariant work reopens only for a named claim | bounded empirical breadth; not thesis-level data closure under FACTSHEET 34.1 |
| C3b | settled for current exploration | Existing-data scouts, reviewed bounce/class-minimum evidence, two prospective pool-screening rules, and bounded optimizer results leave S0, ridge-pressure work, generator transfer, and scheduler selection parked at current marginal value. | reopen only when a named outcome can change a retained claim/decision | later research opportunity, not current completion gate |
| C3c | settled for integration | The random/product table supports a bounded negative benchmark; ridge and covariance-rho support named same-generator sub-threshold pool-screening claims; the gradient/optimizer packets support bounded method claims, not endpoints, maxima, or a default policy. | `experiments/sys-datascience/coordination/final-research-account-2026-07-12.md`; reopen only for a stronger named claim | data-science feature-completion is bounded; demonstration selects sources under those limits |
| C3d | Jörn wording, later | What thesis term should describe the two independently validated same-generator sub-threshold pool-screening rules? | choose terminology after evidence stabilizes; this does not route research | final data-science wording |
