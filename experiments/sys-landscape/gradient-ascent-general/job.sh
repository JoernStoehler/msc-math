...
N="${N:-10000}"
EXP_DIR="experiments/sys-landscape/gradient-ascent-general"
BINARY="$CARGO_TARGET_DIR/release/sys-gradient-ascent-general"

if [[ ! -x "$BINARY" ]]; then
    echo "Missing executable: $BINARY" >&2
    echo "Build it first on LICCA with:" >&2
    echo "  cd ~/msc-math && CARGO_TARGET_DIR=$CARGO_TARGET_DIR cargo build --release -p exp-sys-landscape --bin sys-gradient-ascent-general" >&2
    exit 1
fi
