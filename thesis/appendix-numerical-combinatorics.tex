...
\end{itemize}
Condition~(1) is decided exactly;
condition~(2) is decided numerically
with the three-valued framework.
\end{remark}
...
\item \emph{Determinant margin:}
  $\delta_{\min} \coloneqq
  \min_j |\!\det(N_{S_j})| > 0$.
\item \emph{Symplectic margin}
  (defined when some $\omega_0(n_i, n_k) \neq 0$):
  $\omega_{\min} \coloneqq
...
The normalization and the rational-to-floating-point
