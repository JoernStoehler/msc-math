...
| O-RIDGE-QUOTIENT | Sum and mean are the same within-bucket selector whenever ridge count is fixed. | Algebraic identity `sum = ridge_count * mean`, not empirical replication. Max/std/q95 remain distinct measured quantities. |
...
| O-PRODUCT-BOUNCE-PATTERN | On the post-target panel, both lower-`sys` product matches use two-bounce words with two same-type pair blocks, while the higher members use three-bounce all-singleton words. | The apparent `4`-versus-`1` cyclic-class contrast is forced `2^2` pair-order overcount, not four capacity-value branches. Population association remains untested. |
...
| H-BOUNCE-REGIME | Product `sys` distributions differ between two-bounce vertex-hit and three-bounce all-singleton minimizers. | Mathematically meaningful and cheaply testable from the retained producer field; current support is only two post-target-selected pairs. |
| H-BRANCH | After quotienting cyclic rotation and same-type product-block permutations, high sys may depend on genuine near-active branch competition or action gaps. | High mathematical value; the current product `4`-class observation supplies no support because it collapses structurally. |
...
| --- | --- | --- |
| M0-ACTION-SPECTRUM | Do product bounce/block type, quotient-aware action gaps, near-minimizer multiplicity/entropy, support sizes, beta margins, or branch-window visibility organize sys? | Tiny post-target panel exposed a two-bounce versus three-bounce contrast. The raw four-class count is structurally forced pair-order overcount. Test the existing bounce field across retained product rows before any new action-spectrum computation. |
