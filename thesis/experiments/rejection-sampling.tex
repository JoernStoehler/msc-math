...
If not, $(\operatorname{span}\{n_\ell : \ell \in S_0\})^\perp$ has
dimension~$\ge 2$ and contains~$d$, so there exists~$v$ in this orthogonal
complement, not parallel to~$d$.  The path $d + tv$ stays
in~$\operatorname{rec}(K)$ for small~$|t|$: the~$S_0$ constraints remain tight
(since $v \perp n_\ell$ for $\ell \in S_0$) and the strict inequalities
$n_\ell \cdot d < 0$ remain negative by continuity. (Such strict inequalities
exist: since the normals span~$\mathbb{R}^4$, $d$ cannot be orthogonal to all
normals, so $S_0 \neq \{1, \ldots, F\}$.)
