...
def main() -> None:
    ...
    polytopes = {
        row["poly_id"]: row for row in load_jsonl(args.dataset_dir / "polytope-table.jsonl")
    }
    ...
    for observation in observations:
        ...
        sys_by_dataset[dataset].append(float(poly["sys"]))
        x_by_dataset[dataset].append(float(poly["facet_count"]))
        y_by_dataset[dataset].append(float(poly["sys"]))
        dual_vertex_counts.append(int(poly["dual_vertex_count"]))
