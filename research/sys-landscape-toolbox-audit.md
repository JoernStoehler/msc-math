...
| `endpoint-residualized-regression` endpoint residualized regression | Do endpoint feature blocks add signal beyond metadata? | endpoint-only grouped tabular packet | `experiments/sys-landscape/datascience/methods/feature-pattern-search/analyze_residual.py`; `feature_pattern_search_residual.png` | Grouped endpoint CV; metadata-first residual design tests incremental feature signal, but no current-contract report has been reviewed. | Analyzer tests ridge and random forest residual models after subtracting metadata predictions. The active idea ledger now marks this row `current-review` because the existing artifact must be resolved before its thesis role is classified. | Current artifacts are not enough for a terminal negative or thesis-facing claim. | `inconclusive: experiment/design failure` / `validity caveat` | Current review target; thesis use still needs a current-contract report or Jörn decision. | Reopen by running the `endpoint-residualized-regression` packet and recording a reviewed report/ledger disposition. |
